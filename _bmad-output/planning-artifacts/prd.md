...
*   **NFR4: Security & Compliance (NDPA-Aligned)**
    ...
    *   **NFR4.4 Defense-in-Depth:** Rate Limiting (Redis), Honeypots, strict Content Security Policy (CSP), and IP Throttling with the following thresholds:
        *   **Login Attempts** (covers `/auth/staff/login`, `/auth/public/login`, `/auth/login/mfa`, `/auth/login/mfa-backup`) — stratified defence with four layers; each layer counts a specific subset of responses:
            *   **Burst limiter (`loginRateLimit`):** **5 FAILED attempts per IP per 15 minutes** (Story 9-13 close-out 2026-06-03 amendment via `skipSuccessfulRequests: true` — successful 2xx logins do NOT count). Counts 4xx + 5xx responses including 401 Invalid Credentials, 403 Account Locked / Suspended, 429 from downstream limiters. Codified at `apps/api/src/middleware/login-rate-limit.ts`.
            *   **Sustained limiter (`strictLoginRateLimit`):** 10 attempts per IP per 1 hour, **all responses counted** (catches sustained activity including successful brute-forces). Same file, lines 60-87.
            ...
            *   **Cumulative block:** temporary 30-minute block on the IP after 10 cumulative failed attempts (handled in `AuthService`).
