# Oyo State Labour & Skills Registry (OSLSR) - Product Requirements Document (PRD) - V8.2
...
*   **NFR9: Production SSH Access Control (Tailscale Overlay).** Production VPS SSH (port 22/tcp) must be **restricted to the Tailscale overlay network CGNAT range (`100.64.0.0/10`)** at the DigitalOcean Cloud Firewall boundary. Public-internet SSH must be unreachable. Node-level hardening: `PasswordAuthentication no` (enforced in `/etc/ssh/sshd_config` main file **and** every drop-in under `/etc/ssh/sshd_config.d/*.conf`), `PermitRootLogin prohibit-password`, `PubkeyAuthentication yes`, fail2ban active on the sshd jail. DigitalOcean web console access is retained as the break-glass path and documented in `docs/emergency-recovery-runbook.md`. Realised by Story 9-9 Tailscale + SSH hardening subtask (completed 2026-04-23 per SCP-2026-04-22 §7.1); rationale: Monday 2026-04-20 distributed SSH brute-force incident from 14+ IPs (see `new_err.txt` evidence block).
...
*   **Operator Access (per NFR9, SCP-2026-04-22):** Production VPS SSH is reachable **only over Tailscale overlay network** (`100.64.0.0/10` CGNAT). Public-internet SSH is closed at the DigitalOcean Cloud Firewall. Break-glass access via DigitalOcean web console documented in `docs/emergency-recovery-runbook.md`. Cloudflare WAF/CDN is planned but **domain-gated** on the `oslrs.com` purchase and is not a prerequisite for field-survey start.
