# Oyo State Labour & Skills Registry (OSLSR) - Product Requirements Document (PRD) - V8.4
...
| 2026-04-23   | 8.2     | **SCP-2026-04-22: Multi-Source Registry, API Governance, Security Hardening, Field-Survey UX.** Adds Epic 10 (API Governance & Third-Party Data Sharing, 6 stories) and Epic 11 (Multi-Source Registry & Secondary Data Ingestion, 4 stories). Scopes FR21 to NIN-present rejection (imported and pending-NIN records excluded). Softens FR5 to allow deferred NIN capture via `pending_nin_capture` status. Updates NFR8.1 to partial unique index (`UNIQUE WHERE nin IS NOT NULL`). Adds alternative Right to Erasure verification (phone + DOB + LGA) for NIN-absent records. New FR24 (partner API), FR25 (secondary-data ingestion), FR26 (audit log viewer), FR27 (public wizard with deferred auth), FR28 (pending-NIN capture). New NFR9 (Tailscale-restricted SSH), NFR10 (API token hashing + rotation). Technical Assumptions extended with Tailscale, nullable-NIN, magic-link-primary / SMS-OTP-infra-only, multi-source ingestion, partner API substrate. See `_bmad-output/planning-artifacts/sprint-change-proposal-2026-04-22.md` for full trigger context. MVP unchanged. | John (PM) |
| 2026-04-25   | 8.3     | **NFR9 + Operator Access bullet rewrite — alignment with as-deployed dual-source SSH firewall.** Triggered by drift between original SCP §A.4 narrative (tailnet-only firewall) and as-deployed state per sprint-status.yaml §10-11 (2026-04-25 entries) + Architecture V8.2-a1 follow-up amendment. NFR9 rewritten end-to-end to reflect the layered access model: **sshd-level key-only authentication as the primary access control**, Tailscale overlay (`100.64.0.0/10`) as the operator daily-driver path, DO Cloud Firewall permitting **dual-source** SSH inbound (`0.0.0.0/0` + `100.64.0.0/10` — the `0.0.0.0/0` source is intentional and load-bearing, required for GitHub Actions CI deploys and DO Console which is itself SSH-based via DOTTY/`droplet-agent` from DO infrastructure IPs), fail2ban elevated from "insurance" to **load-bearing second-line defence** against public-surface brute-force, monthly OS reboot pre-flight checklist documented. Operator Access bullet under Technical Assumptions §Infrastructure & Deployment correspondingly updated. Realisation history captured: original tailnet-only firewall deployed 2026-04-23, amended dual-source 2026-04-25 after empirical discovery of DO Console SSH dependency (supersedes original WebSocket-filtering hypothesis). Future state (self-hosted GHA runner inside tailnet → re-narrow firewall) flagged as Story 9-9 follow-up. **Authoritative as-deployed source remains Architecture ADR-020 V8.2-a1.** No new FRs/NFRs added; no scope change; this is a documentation-fidelity correction. | John (PM) |
| 2026-05-08   | 8.4     | **Story 5.2 AC#5 — PDF export row-cap policy.** Adds acceptance criterion to Story 5.2 (Customizable Reporting & Query Builder) referencing **ADR-021** (architecture, Winston-pending) and `docs/decisions/2026-05-08-pdf-export-row-cap.md` (decision memo, Awwal-ratified 2026-05-08). Establishes the project-wide pattern for **tiered caps on expensive synchronous operations**: PDF exports honour a server-side hard cap (HTTP 413 at 5,001+ rows) plus a UX modal in the soft band (2,001–5,000 rows); CSV remains the supported format for large datasets (existing per-endpoint caps unchanged); domain-bounded exports (e.g. LGA enumerator productivity rollups, ≤200 rows by domain logic) are exempt via per-endpoint suppression flag. Originating evidence: Story 9-10 close-out 2nd-pass code-review (2026-05-08) lifted the BENCHMARK gate that had previously hidden the real cost of `ExportService.generatePdfReport` at scale (PDF 10K = 14.6s/93MB; CSV 100K = 0.34s/6.3MB). Iris (UX product) and Sally (UX design) unavailable in Operate-phase window — Awwal ratified per follow-up brief escalation rule. No new FRs/NFRs added; no scope change. ADR-021 + UX Spec Component #18 (`ExportRowCapModal`) authoring will be re-routed to Winston + Sally respectively (current drafts authored under PM-overreach during the same session; routing-correction logged in decision memo Propagation Log). Story creation routes to Bob (SM) via canonical `*create-story --yolo` once Winston + Sally edits land. | John (PM) |
...
*   **FR5:** The system shall **prompt** public users to provide their National Identity Number (NIN) during registration. When NIN is provided, it prevents multiple completed registrations by the same individual (per FR21). When NIN is unavailable at the point of submission (e.g., respondent does not have their NIN card on hand, or NIMC issuance is delayed), the record **may be saved with `status = pending_nin_capture`** and a completion path (return-to-complete via magic link; enumerator follow-up in the field) must be offered. Records in `pending_nin_capture` are excluded from NIN-keyed downstream pipelines (fraud detection NIN dedupe, marketplace enrichment requiring NIN) until NIN is captured and verified. **Note:** Only NIN is accepted for identity verification to ensure NDPA compliance (BVN is explicitly excluded per NFR4.1). **Reference:** `pending_nin_capture` status model defined in FR28 and realised by Story 11-1 schema foundation.
...
*   **FR21:** The system shall enforce **Global NIN Uniqueness at the Respondent Record Level — when NIN is present** via a **partial UNIQUE constraint** on `respondents.nin` (`UNIQUE WHERE nin IS NOT NULL`; see NFR8.1). One respondent record exists per NIN-carrying individual across all submission sources (Enumerator, Public Self-Registration, Paper Entry, Secondary-Data Import). **Scope — rejection applies only when the incoming submission carries a non-null NIN.** When a submission contains a NIN matching an existing respondent, the system shall **reject** the submission with an error message including the original registration date and source (e.g., "This individual was already registered on 2026-02-10T14:30:00.000Z via enumerator"). Rejected submissions are preserved in the `submissions` table (not deleted) with `processed: true` and the error stored in `processingError`. **NIN-absent records — excluded from rejection.** Records with `status = pending_nin_capture` (respondent did not provide NIN at submission time, FR28) and `status = imported_unverified` (secondary-data imports without NIN, FR25) are **not** subject to NIN-based deduplication. Candidate-merge detection for NIN-absent records is handled by the Import Service (Epic 11) via probabilistic matching (phone + full name + LGA + trade), not by UNIQUE-constraint rejection. **Cross-table uniqueness:** The system also checks the `users` table for staff NIN conflicts - if a submission NIN matches a staff member, it is rejected with error code `NIN_DUPLICATE_STAFF` (prevents staff from being double-registered as respondents). **Pre-submission NIN check:** All submission channels (Enumerator native form, Public registration, Clerk data entry) call `POST /api/v1/forms/check-nin` **when NIN is populated** before submission to detect duplicates at the point of entry, disabling the Submit button when a duplicate is found. In offline mode, the pre-check is skipped and the ingestion worker catches the duplicate on sync. **Race condition defense:** If two simultaneous submissions with the same NIN arrive, the PostgreSQL partial UNIQUE constraint catches the conflict and the second submission is rejected. **Duplicate detection audit:** When a NIN duplicate is rejected, the system logs `{ event: 'respondent.duplicate_nin_rejected', existingRespondentId, rejectedSubmissionId, source, channel }` for supervisor review. **NIN format validation:** When a NIN is provided, all submission channels must validate NIN format using the Modulus 11 checksum algorithm on the client side before submission, ensuring only structurally valid NINs enter the pipeline.
*   **FR24: Partner API (Third-Party Data Sharing).** The system shall expose a **scoped partner API** at `/api/v1/partner/*` for authenticated third-party MDA consumers (ITF-SUPA, NBS, NIMC, future integrations), delivered by Epic 10.
    *   **Authentication:** scoped API keys (not OAuth2, not mTLS — per SCP-2026-04-22 §4.1 Story 10-1 decision). Keys stored as SHA-256 hashes; plaintext is shown once at provisioning and cannot be recovered.
    *   **Scopes (initial set):** `aggregated_stats:read`, `marketplace:read_public`, `registry:verify_nin`, `submissions:read_aggregated`, `submissions:read_pii`. Each scope may carry a time-bounded expiry (per-scope `expires_at`).
    *   **Per-key controls:** LGA-scoping (via `api_key_scopes` join table), IP allowlist (`allowed_ip_cidrs TEXT[]`), per-consumer per-scope rate limits and quotas (Redis-backed, Story 10-2).
    *   **Rotation:** default 180-day rotation with a 7-day overlap window where both old and new keys validate, to allow zero-downtime rollover.
    *   **Audit:** every partner-API call is logged in the audit stream with a `consumer_id` principal (distinct from human `actor_id`), surfaced via FR26 audit viewer and Story 10-6 consumer audit dashboard.
    *   **Provisioning policy (post-Transfer):** Ministry ICT lead approval plus two-person sign-off is required for any key granted `submissions:read_pii` scope.
    *   **Legal precondition:** no key is provisioned with a PII-bearing scope until a signed Data-Sharing Agreement (Story 10-5) is on file.
*   **FR25: Secondary-Data Ingestion (Multi-Source Registry).** The system shall ingest **secondary data sources** (public registries, partner MDA exports) into the canonical respondent registry with source-labelled provenance, delivered by Epic 11.
    *   **Formats supported:** PDF (parsed via structured extractor, reference implementation against ITF-SUPA public artisan PDF), CSV, XLSX.
    *   **Source labelling:** each imported row is stamped with `source` ∈ `{imported_itf_supa, imported_other}` (existing `source` enum extended), `external_reference_id`, `import_batch_id`, `imported_at`, and retains `status = imported_unverified` until cross-referenced against a field-verified submission.
    *   **Batch-level lawful basis:** each `import_batches` row captures `lawful_basis` (NDPA Article 25 category), `data_controller_consent_reference`, uploaded-by `actor_id`, and file-hash for provenance.
    *   **Dry-run preview → confirm → rollback workflow:** `POST /api/v1/admin/imports/dry-run` returns a per-row decision preview (match candidates, skip reasons, lawful-basis requirements) without committing; `POST /api/v1/admin/imports/confirm` persists inside a transaction; `POST /api/v1/admin/imports/:id/rollback` soft-deletes the batch within a **14-day window**.
    *   **Duplicate policy:** email/phone match against existing records triggers auto-skip with a logged reason (per SCP-2026-04-22 §4.1 Story 11-2 decision); NIN-match is routed through FR21's partial-UNIQUE pathway.
    *   **Exclusion from NIN-keyed pipelines:** `imported_unverified` rows are excluded from fraud-detection NIN dedupe and marketplace enrichment (see FR21 scope clause and FR5 deferred-NIN clause) until NIN is captured and verified.
    *   **Source badges + filters:** source provenance is surfaced across Registry Table, Respondent Detail, Marketplace cards, and Assessor Queue (Story 11-4).
*   **FR26: Super-Admin Audit Log Viewer.** The system shall expose the existing write-side audit infrastructure (Epic 6) via a **super-admin UI** for read-side investigation, delivered by Story 9-11.
    *   **Capabilities:** list, paginate (stable constant-time plan at page 1 / 100 / 1000), filter by principal (human `actor_id` OR `consumer_id`, per FR24), action, target resource, and time range.
    *   **Performance:** list query p95 < 500ms with any single filter applied; p95 < 800ms with two filters combined, verified against 1,000,000 seeded `audit_logs` rows using the Story 11-1 seeder (Akintola-risk AC#X).
    *   **Export:** CSV export of filtered result sets (field-level PII redaction depends on caller role).
    *   **Navigation:** new "Audit Log" sidebar item for the super_admin role.
    *   **Prerequisite for Epic 10 PII release:** no `submissions:read_pii` partner-API scope may be provisioned before FR26 is live in production (DPIA credibility gate).
*   **FR27: Public Self-Registration Single-Wizard with Deferred Authentication.** The system shall collapse the current four-hop public registration flow (register → login → form → submit) into a **single 5-step wizard**, delivered by Story 9-12.
    *   **Flow:** survey content (consent captured at step 3, not at end) → NIN / pending-NIN branch → review → optional auth setup at completion.
    *   **Auth model:** **email magic-link is the primary authentication channel**; password login is a fallback. **SMS OTP infrastructure is built but inactive (budget-gated)** — toggled by super-admin flag when a Nigerian-SMS provider agreement lands.
    *   **Return-to-complete:** respondents who start the wizard and leave mid-flow receive a magic-link email that resumes them at their saved step (for `pending_nin_capture` follow-up, for profile activation, for verification status updates).
    *   **Trust surface:** trust badges (ministry seal, NDPA data-protection statement, "your data stays in Nigeria") rendered at the bottom of every wizard step (ITF-SUPA-inspired trust UX).
    *   **Accessibility:** step-indicator is persistently visible and keyboard-navigable (WCAG 2.1 AA, per NFR5.1).
*   **FR28: Deferred-NIN Capture via `pending_nin_capture` Status.** The system shall offer a **deferred-NIN path** for respondents who do not have their NIN on hand at submission, delivered by Story 9-12.
    *   **Status model:** `pending_nin_capture` is a first-class value in the `respondents.status` enum (alongside `active`, `nin_unavailable`, `imported_unverified`), defined in Story 11-1 schema foundation.
    *   **NIN retrieval hint:** inline `*346#` USSD hint is surfaced at the NIN input ("Don't know your NIN? Dial \*346# on any phone linked to your NIMC record.") and repeated in follow-up reminder emails / SMS.
    *   **Reminder cadence:** scheduled reminder notifications at T+2d, T+7d, T+14d. After T+30d without NIN capture, record transitions to `nin_unavailable` and enters a supervisor-review queue.
    *   **Downstream exclusion:** `pending_nin_capture` and `nin_unavailable` records are excluded from NIN-keyed fraud dedupe, marketplace enrichment requiring NIN, and the official-to-partner `registry:verify_nin` scope response set (FR24).
    *   **Right-to-erasure path:** NIN-absent records remain fully erasable via the alternative verification path documented in Epic 2 Story 2.1 (phone + DOB + LGA match).
...
*   **NFR4: Security & Compliance (NDPA-Aligned)**
    ...
    *   **NFR4.4 Defense-in-Depth:** Rate Limiting (Redis), Honeypots, strict Content Security Policy (CSP), and per-IP flood ceilings. Thresholds are governed by the identity-axis rules below (*restructured 2026-09-17, Lane C, per PM ruling `pm-ruling-2026-09-16-nfr4-4-rate-limit-axis.md`*). The previous revision's blanket licence — *"and IP Throttling with the following thresholds"* — is **withdrawn**: it permitted any author to bound any route by `req.ip` without stating whose traffic that bounded, and four production incidents came from limiters it licensed but never named.
        *   **NFR4.4.a — Identity axis (NORMATIVE).** Every rate limit MUST declare the identifier it counts against — its **axis** — and MUST appear in the NFR4.4.c register. Two axes, and almost every protected route needs **both**:
            *   **Person axis** — keyed on a per-person identifier the request already carries: `email` (normalised lowercase + trimmed, stored as a digest), `nin`, `userId` (JWT `sub`), or a single-use `token`. Sized for **one person's** legitimate use.
            ...
            *   A person axis MUST be used wherever the request carries a person identifier. Where it carries none, the limiter is flood-axis only and MUST record a population model per NFR4.4.b.
            ...
            *   **A person-axis limiter that silently falls back to IP is a flood-axis limiter in disguise.** Any `user?.sub ?? ip` fallback MUST log a warning and increment a metric on every IP-keyed request (reference implementation: `apps/api/src/middleware/audit-log-rate-limit.ts`). Silent fallback is prohibited.
            *   ⭐ **Mount order is part of the control, not an implementation detail** (*ratified by adversarial review 2026-09-16; generalised to all routes 2026-09-17*). A limiter's budget means nothing without knowing which requests reach it. The ordering rules are: (i) a **flood ceiling mounts first**, so it bounds every request including those later refused, and with them the cost of any external verification call behind it; (ii) **proof-of-humanity (CAPTCHA) mounts ahead of any person-axis budget**, so an unauthenticated stranger cannot spend a named person's allowance; (iii) a **person-axis budget mounts ahead of the sustained flood budget**, so one person's retries cannot exhaust a budget shared with strangers. Any route whose limiters are reordered MUST be re-assessed against NFR4.4.c, because the same numbers in a different order are a different control.
        *   **NFR4.4.b — Population constraint, exposure tiers, response mode (NORMATIVE).** **In the Oyo State operating environment an IP address does not identify a person.** Nigerian carriers use CGNAT, and Opera Mini proxies users through a small pool of servers. Measured: **244 activation refusals from 6 addresses** with reverse DNS `opera-mini.net` (2026-09-07/08); **36 citizens blocked across 5 carrier ranges** in one morning (2026-08-05). A per-IP budget is therefore shared between strangers, and a small one is an availability defect.
            *   ⛔ **The failure mode is invisible in the database by construction, and this is now measured, not argued.** A refused or misdirected request carries no user id, so the victim's row reads "invited, never activated" or "activated, never logged in" — which an operator reads as apathy. In the two days after the 13-68 deploy, **32 `user_not_found` login failures** were logged across **seven** enumerators typing a non-provisioned address, including the enumerator who has captured 21 people (9 attempts) — and **two of them typed a bare `+` with nothing after it**, which is a person who knows a plus exists and cannot guess the suffix. **None of these increment `failed_login_attempts`**, because a wrong address never reaches a password check: production still shows exactly **one** account with any failure count at all. A cohort failing 32 times left no trace in the `users` table. Any control in this class MUST therefore be assessed against its logs, never against the absence of database evidence.
            *   **Disclosure boundary is response-side, not log-side.** `user_not_found` and `invalid_password` MUST return an identical generic error, so account existence is not disclosed to the caller. The **logs** MUST distinguish them, and both branches MUST log the same normalised identifier field — that asymmetry is what makes per-IP cardinality monitoring possible, and a monitor reading a field present on only one branch measures half the traffic.
            *   Flood budgets are constrained by **what a refusal costs the person refused**:
                *   **Tier 1 — public, unauthenticated, one-shot or irreversible.** Activation, registration submit, magic-link request and redemption, password reset request and completion, registration status, NIN check, public verification, login. **Minimum flood budget: 100 per window. No exemption.** A refusal ends the person's journey, cannot be meaningfully retried, and is invisible in the database. All four recorded incidents are Tier 1.
                *   **Tier 2 — public, unauthenticated, retryable and idempotent.** Marketplace search, marketplace profile, edit-token use. **Minimum 100 per window, OR a recorded population model** stating (i) assumed strangers behind one address and (ii) one person's legitimate volume per window, with `budget ≥ (i) × (ii)`, plus a dated review.
                *   **Tier 3 — authenticated.** **Person axis mandatory** (`user.sub` exists by definition); any IP budget is a loud fallback path, exempt from the floor.
            *   **Response-mode clause.** A flood budget's floor depends on what consumes it. A budget counting **all responses** is spent by ordinary successful use and carries its tier's fixed floor. A budget counting **failures only** is spent only when someone errs, and instead carries a **derived floor: ≥ (assumed strangers behind one address) × (one person's per-window failure allowance)**, both figures recorded in its register row. **A failure budget may never be set at or below one person's own failure allowance** — that is the defect class by definition. *Worked example: `strictLoginRateLimit` = 3 strangers/address × 20 failures/person/hour = **60/hour**.*
        ...
        *   **NFR4.4.e — Login Attempts** (covers `/auth/staff/login`, `/auth/public/login`, `/auth/login/mfa`, `/auth/login/mfa-backup`) — stratified defence in layers on two axes (per person, per IP); each layer counts a specific subset of responses. **Mount order is part of the control** (*adversarial review 2026-09-16*): IP flood ceiling → CAPTCHA → burst limiter → sustained limiter → (MFA gate on step 2) → controller. The CAPTCHA sits before the per-email budget so a request with no CAPTCHA cannot spend anyone's budget; the sustained limiter sits last so it is charged only for CAPTCHA-passing attempts the person's own budget allowed:
            *   **Burst limiter (`loginRateLimit`):** **5 FAILED attempts per submitted EMAIL per 15 minutes** (normalised lowercase + trimmed, stored as a SHA-256 digest so the Redis key is fixed-size and holds no address; falls back to the IP when the body carries no email, i.e. the two MFA step-2 routes) — *axis amended 2026-09-16, Story 13-68; was per IP*. Successful 2xx logins do NOT count (Story 9-13 close-out 2026-06-03 amendment via `skipSuccessfulRequests: true`). Counts 4xx + 5xx responses that reach it — 401 Invalid Credentials, 429 Account Locked, 403 Account Suspended / Not Activated, 429 from the sustained limiter behind it — but never a CAPTCHA failure, which is refused before it. Codified at `apps/api/src/middleware/login-rate-limit.ts`.
            *   **IP flood ceiling (`loginIpFloodLimit`):** 100 requests per IP per 15 minutes, **all responses counted except a 429 emitted by a limiter behind it** (*added 2026-09-16, Story 13-68; the refusal exclusion added 2026-09-20, Story 13-70 FR1, per NFR4.4.d*), mounted **first**. **Load-bearing:** it is the only login limiter that counts successful responses, so it is the only bound on successful request volume from one IP (for example, bulk validation of credentials an attacker already holds); being first, it also bounds CAPTCHA-refused traffic and so the hCaptcha verification call. It is **not** the bound on a stream of failures — the sustained limiter refuses that at 61 per hour first. Same file.
            *   **Sustained limiter (`strictLoginRateLimit`):** **60 FAILED requests per IP per 1 hour**, successful logins not counted (*amended 2026-09-16, Story 13-68; was 10 per IP per hour, all responses*). **Combined per-IP login budget: 60 failures per hour, and at most 100 requests of any outcome per 15 minutes.** The 60 is **derived**, not chosen: strangers behind one address × one person's failure allowance = 3 (17 enumerators observed across 6 Opera Mini addresses) × 20 (5 per email per 15 min) — amend the population model, not the constant, and never set it at or below one person's own allowance. The derivation holds only because it is mounted **last**: behind the burst limiter, one person can spend at most their own 20 failures an hour of it (mounted first, their retries into their own 429 and every refused CAPTCHA were charged too, and one person could lock a whole proxy out). Counting failures only is an **availability** change, not a security one: an attacker generates no successes, so failed-only at 60 gives an attacker the same capacity as all-responses at 60 — but **6× the 10 per IP per hour it replaced**, a trade-off ruled for the shared-proxy population, not a tightening. It is the per-IP bound on credential spray, on enumeration of accounts that do not exist (which increments nothing account-side), and on mass account-lockout runs (≤ 6 new locks per IP per hour). It does not stop a successful brute force (that is the account lockout and MFA). Same file.
            *   **MFA gate (`mfaRateLimit`):** 10 attempts per IP per 1 minute, all responses counted — gates burst attacks at the edge before they reach the per-user lockout. Codified at `apps/api/src/middleware/mfa-rate-limit.ts` (Story 9-13 AC#7).
            *   **Per-user lockout (`users.mfa_locked_until`):** 5 failed TOTP verifies per user per 15 minutes → 15-minute lockout (separate from per-IP rate limits). Final stop on the per-user axis via the `mfa.service.ts` `recordFailure` helper.
            *   **Cumulative block:** temporary 30-minute lock on the **account** (`users.locked_until`) after 10 cumulative failed password attempts, with a warning logged from the 5th (handled in `AuthService`; *corrected 2026-09-16 — this line previously said "on the IP", which the code has never done*). **The failure counter is cleared when the lock expires** (*added 2026-09-16, Story 13-68*): previously it was reset only by a successful login or a password reset, so an account that had once reached 10 was re-locked by every single further failure — two requests an hour held it locked indefinitely. Lock duration does not yet escalate on repeat (needs a lock-count column; tracked residual); plain decay leaves ~10 guesses per 30 minutes against bcrypt, the password policy and MFA, assessed as immaterial by the PM ruling of 2026-09-16. Every `auth.login_failed` failure branch logs the normalised `email`, so distinct identifiers per IP is a single-field aggregation.
            *   **Rationale for `skipSuccessfulRequests` (2026-06-03 amendment):** UAT-driven. Story 9-13 close-out (2026-06-02/03) surfaced that the original `loginRateLimit` config counted ALL requests, blocking the operator on the 4th–5th successful TOTP verify during legitimate iteration on the MFA-pending skeleton bug retest cycle. Attacker defence is preserved end-to-end via the three remaining layers (sustained / edge / per-user) — brute-force attackers spam wrong codes that all return failed responses and continue to count against the burst limiter. See Architecture §"ADR-015 → Rate-limit semantics post-9-13 close-out (2026-06-03)" for the full attacker / legitimate-operator response-counting matrix.
            *   **Rationale for the identity-axis amendment (2026-09-16, Story 13-68):** field-driven. In this market an IP is not a person — carrier CGNAT and Opera Mini put many users behind one address (the activation limiter logged **244 refusals from six `opera-mini.net` addresses** on 2026-09-07/08). A per-IP burst budget was therefore shared by strangers, and the sustained limiter refused the 11th successful login behind one proxy in an hour, with 17 field enumerators about to log in for the first time. Keying the burst limiter on the email caps a **burst** against one account (rotating IPs no longer buys a fresh budget, and at most 5 failures per 15 minutes reach the non-atomic account counter however many run in parallel) while the two per-IP ceilings keep a spray bounded. **What loosened, stated as ruled (adversarial review 2026-09-16):** (a) a patient attacker against ONE account gets ≈ 320–480 guesses a day instead of ≈ 58 — lockout decay means each lock costs ten fresh failures instead of one; the PM ruling accepts 480/day against bcrypt, the password policy and MFA; (b) a spray gets 60 failures per IP per hour instead of 10. Known and accepted: anyone who knows an email and solves a CAPTCHA per attempt can spend its five failures and hold its login for up to 15 minutes — the same capability, at a comparable CAPTCHA cost, as ten wrong passwords triggering the 30-minute account lock. (Before the review this needed no CAPTCHA at all, because the burst limiter ran ahead of it.) Sources: SCP `sprint-change-proposal-2026-09-16-rate-limit-identity-axis.md` Lane A; PM ruling `pm-ruling-2026-09-16-nfr4-4-rate-limit-axis.md` incl. Addendum A (mass account-lockout DoS; failed-only derived 60; decay). The normative two-axis rule for all limiters is Lane C.
        *   **NFR4.4.c — Threshold register (NORMATIVE).** Every rate control in the system appears below with its axis, key, budget, window, counting mode and tier. **No control may exist outside this register.** The five free-standing thresholds this clause replaces (Marketplace Search, Contact Reveal, API General, Password Reset, Profile Edit Token) are carried into it unchanged in value; what is added is the axis each was always implicitly asserting, and the 28 controls that were never named at all. The register is **enforced in code** by the coverage map in `apps/api/src/middleware/__tests__/rate-limit-coverage.test.ts`, which is the authority on current values; this clause is the authority on what a legal row looks like.
            *   **Login** (see NFR4.4.e for the full rationale) — burst `loginRateLimit`: person / email digest · 5 · 15 min · FAILED-EXCEPT-REFUSALS · T1 *(Story 13-70 R3, 2026-09-20: failed-only as before, and a 429 emitted by `strictLoginRateLimit` or `mfaRateLimit` is handed back rather than charged to the refused person — its OWN 429s are still counted.)*. Flood `loginIpFloodLimit`: flood / IP · 100 · 15 min · ALL-EXCEPT-REFUSALS · T1 *(Story 13-70 FR1, 2026-09-20: still counts every SUCCESS — that is the load-bearing property — but not a 429 emitted by another limiter on the route, per NFR4.4.d. A refused CAPTCHA (400) is still counted.)*. **Cost bounded, as this clause requires of any diverging ceiling: total request VOLUME per IP, and the bcrypt work behind it. It no longer bounds the hCaptcha `siteverify` call on the captcha-passing-then-refused path — Story 13-70 R6, DATED 2026-10-15.** **Population model, re-derived 2026-09-20 (Story 13-70 R1) because the one in the code was wrong twice over:** prod holds **28** enumerator accounts of which **nine are the operator's own harness logins**, so **19 real field accounts** — the old "17" mixed field staff with the test harness. Per person per window this ceiling counts 5 failures + 1 success = **6** (not 5; it is the only login limiter counting successes). Measured concurrency behind one shared address is ≈3 (17 enumerators across SIX Opera Mini proxies, 2026-09-07/08) → **18**; absolute worst case 19 × 6 = **114**; 100 buys ≈16 concurrent people behind one address. ⭐ **100 is ACCEPTED — evidence by the Story 13-70 adversarial review, ruling by Awwal at adjudication, both 2026-09-20 (Story 13-70 R1; two-part attribution per handoff §2al) — because FR1 changed who can spend it:** until FR1 one person could reach 100 alone — five real attempts plus 95 of their own refusals — and shut their whole proxy for fifteen minutes. Sustained `strictLoginRateLimit`: flood / IP · 60 · 1 h · FAILED-EXCEPT-REFUSALS · T1 (derived 3 × 20) *(Story 13-70 R3, 2026-09-20: `mfaRateLimit` is mounted behind it on the two MFA step-2 routes, so there IS a limiter whose 429 it would otherwise charge to the shared per-IP failure budget.)*. Account lockout `users.locked_until`: person / user · 10 failures → 30 min, counter cleared on expiry.
            *   **MFA** — `mfaRateLimit`: person / **MFA challenge token** · 10 · 1 min · ALL · T1 *(currently keyed on IP — CHANGE REQUIRED; `userId` is not available at middleware time, the challenge token is)*. Per-user lockout `users.mfa_locked_until`: person / user · 5 · 15 min → 15-min lock.
            *   **Session** — `refreshRateLimit`: person / **session (token-family) id** · 10 · 1 min · ALL · T3 *(currently keyed on IP — CHANGE REQUIRED; the token value is NOT a legal key because refresh tokens rotate on use)*. `reauthRateLimit`: person / `user.sub` · 5 · 15 min · ALL · T3 *(currently IP — CHANGE REQUIRED)*.
            *   **Activation** — `activationRateLimit`: person / invitation token · 20 · 15 min · T1. `activationIpFloodLimit`: flood / IP · 300 · 15 min · ALL-EXCEPT-REFUSALS · T1 *(Story 13-70 R7, 2026-09-20: a 429 from `activationRateLimit` is no longer counted here.)*.
            *   **Password reset** — request (service layer, `PasswordResetService.checkRateLimit`): person / email · **3 · 1 h** · T1 *(the threshold this clause replaces; unchanged)*. Request flood `passwordResetRateLimit`: flood / IP · 200 · 1 h · ALL · T1. Completion `passwordResetCompletionRateLimit`: person / reset token · 20 · 15 min · T1. Completion flood `passwordResetCompletionIpFloodLimit`: flood / IP · 300 · 15 min · ALL-EXCEPT-REFUSALS · T1 *(Story 13-70 R7, 2026-09-20: a 429 from the person-keyed limiter on the same route is no longer counted here.)*.
            *   **Magic link** — `magicLinkRateLimit`: person / email on the request route · 3 · 1 h · T1. ⚠️ **SPLIT REQUIRED:** the same limiter and Redis prefix is mounted on seven further routes that carry no `email` and therefore key on **IP at 3 per hour, pooled** — including magic-link redemption, which FR27 makes the **primary public authentication channel**. Measured 2026-09-03→16: 3 refusals, all on the email branch, one enumerator; the IP branch has **zero** refusals at ≈2 redemptions/day. A **latent scaling risk, not a live incident** — 6 proxy addresses × 3/hour = 18 statewide against a projected ≈9/hour. Redemption routes MUST move to their own prefix keyed on the **magic-link token** with a Tier 1 IP ceiling ≥ 100/hour, scheduled **before any marketing or radio push**.
            *   **Registration & wizard** — `registrationRateLimit`: flood / IP · **100** · 15 min · ALL-EXCEPT-REFUSALS · T1 *(Story 13-70 R7, 2026-09-20: a 429 from the person-keyed limiter on the same route is no longer counted here.)* *(currently 50 — CHANGE REQUIRED; the per-email limiter below is the abuse control, so raising the ceiling improves capture headroom and weakens nothing)*. `registrationEmailRateLimit`: person / email digest · 3 · 15 min · T1 *(Story 13-70 FR3, 2026-09-20: `e:<sha256>` via the shared `buildLoginRateLimitKey`, not the raw address)*. `wizardDraftRateLimit`: flood / IP · 1,200 · 15 min · ALL-EXCEPT-REFUSALS · T1 *(Story 13-70 R7, 2026-09-20: a 429 from the person-keyed limiter on the same route is no longer counted here.)*. `wizardDraftEmailRateLimit`: person / email digest · 300 · 15 min · T1 *(Story 13-70 FR3, 2026-09-20: same shared builder)*. `registrationStatusRateLimit`: flood / IP · **100** · 15 min · T1 *(currently 10, via a hand-rolled `keyGenerator` that the limiter census misreported as a person key for five weeks — CHANGE REQUIRED)*. `ninCheckRateLimit`: person / `user.sub` with **silent IP fallback** · 20 · 1 min · T1 *(mounted on a public route, so per-IP in practice — fallback MUST be made loud)*.
            *   **Public verification & notification** — `publicVerificationRateLimit`: flood / IP · **100** · 1 min · T1 *(currently 30 — CHANGE REQUIRED; an officer verifying an ID card in the field is a one-shot task)*. `unsubscribeRateLimit`: flood / IP · 120 · 1 min · T1.
            *   **Marketplace** — `marketplaceSearchRateLimit`: flood / IP · **30 · 1 min** · T2 *(the threshold this clause replaces; unchanged)*. Population model: no person axis is available on a public unauthenticated search; refusals are visible, immediately retryable and destroy nothing. **Measured 2026-09-03→16: 0 refusals across 16,825 marketplace log lines, with a passing positive control** (the same query finds the known 244 activation and 12 reset-completion refusals). **Review 2026-12-31**, or immediately on either trigger: >20 refusals in a day, or any marketing/radio/partner push driving marketplace traffic — in which case raise to 300/min **before** the push. `marketplaceProfileRateLimit`: flood / IP · 100 · 1 min · T2. Contact reveal (`reveal-rate-limit.ts`): person / `userId` · **50 · 24 h** · T3 *(the threshold this clause replaces; unchanged)*. Edit-token request (service layer): person / NIN · **3 · 1 day** · T2 *(the threshold this clause replaces; unchanged)*. `editTokenRequestRateLimit`: flood / IP · **100** · 1 h · T1 *(currently 10 — CHANGE REQUIRED)*. `editTokenUseRateLimit`: person / **edit token** · 30 · 1 min · T2 *(currently IP — CHANGE REQUIRED)*. `revealStepUpRateLimit`: person / `user.sub` · 15 · 5 min · T3 *(currently IP — CHANGE REQUIRED, and it is a **re-order plus a re-key, not a re-key alone**: the limiter is presently mounted **ahead of** `authenticate`, so `user.sub` does not yet exist when it computes its key. Re-keying without moving it behind `authenticate` would silently fall back to IP and change nothing — the defect class in NFR4.4.a's mount-order rule, and the reason that rule is normative)*.
            *   **Authenticated back-office (all Tier 3, person axis on `user.sub`)** — `auditLogReadRateLimit` 60/min *(reference implementation: the only limiter whose IP fallback logs)*; `auditLogExportRateLimit` 10/h; `exportRateLimit` role-based/h; `messageRateLimit` 30/min; `operationsReadRateLimit` 60/min; `profileUpdateRateLimit` 10/min — the last four have **silent** IP fallbacks that MUST be made loud. `settingsListRateLimit` 60/min and `settingsWriteRateLimit` 30/min are **currently keyed on IP on authenticated routes — CHANGE REQUIRED**.
            *   ⚠️ **Flood-ceiling divergence, ruled 2026-09-17.** Tier 1 flood ceilings are **300/15 min** (activation, password-reset completion) but **100/15 min** for login, against the same shared-proxy population. **The divergence is permitted and login stays at 100**, because login's ceiling is mounted ahead of the CAPTCHA and therefore also bounds the external hCaptcha `siteverify` call and the bcrypt work behind it — a cost the other two do not carry per attempt. **But the justification is currently unmeasured**, and an unverified assumption may not do normative work indefinitely: neither hCaptcha's own rate limit at 400 verifications/IP/hour nor bcrypt p95 on the 2 GB VPS has been measured. **Both MUST be measured by 2026-10-15.** If neither constrains, login's ceiling harmonises to 300/15 min and this exception is deleted. ⛔ **AMENDED 2026-09-20 (Story 13-70 R6) — THE JUSTIFICATION ITSELF MOVED, AND NOT IN ONE DIRECTION.** FR1 stopped the ceiling counting a 429 emitted by a limiter behind it, and the mount order is flood → CAPTCHA → burst → strict — so a request the burst limiter refuses has ALREADY passed the captcha and already cost a `siteverify` call. **bcrypt is now bounded BETTER** (a refused request never reaches the controller, so the budget is no longer spent on requests bcrypt never sees), **and `siteverify` WORSE**: measured in `login-rate-limit.binding.test.ts`, 40 captcha-passing requests refused downstream cost **40 siteverify calls while the counter peaked at 6**, against 40 forged-captcha requests which cost 40 calls and moved the counter to 40. The *"400 verifications/IP/hour"* figure this exception rested on is therefore no longer an upper bound on that path. Scoped honestly: a production token is single-use, so a reused one returns `success:false` → 400 → counted, and the residual exposure is one solved captcha per unbounded call. ⭐ **This makes the 2026-10-15 measurement MORE load-bearing, not less**, and if the answer is that hCaptcha does constrain, the fix is a counter of its own mounted with the captcha — one counter cannot both exclude refusals (NFR4.4.d) and count every request that reached the captcha. Until then the rule is: **a flood ceiling may diverge from its tier's peers only where its register row records the cost it is bounding.** Three different numbers with three recorded derivations is sound engineering; three different numbers with none is drift.
        *   **NFR4.4.d — Governance and observability (NORMATIVE).**
            *   **Enforcement.** A limiter without an NFR4.4.c row fails `rate-limit-coverage.test.ts`. The coverage map carries required `axis`, `key`, `tier`, `mode` and `budget` fields, and the suite **asserts** — not documents — that: (a) every limiter reachable from a mounted router has a map entry; (b) every Tier 1 all-response flood entry has budget ≥ 100; (c) every failed-only flood entry exceeds one person's failure allowance and records its population model; (d) every Tier 2 entry below 100 records a population model and a review date; (e) every person entry names a non-IP key; (f) every declared IP-fallback path logs. **Assertions, not comment tables:** the previous reviewer-facing comment table recorded `registrationRateLimit` as 5 against code of 50 for five weeks, and named `googleAuthRateLimit` — a limiter that exists nowhere in `src`.
            *   ⛔ **A refusal is not a request.** A 429 emitted by one limiter MUST NOT be counted by any other limiter on the same route. Mounted first and counting all responses, a flood ceiling otherwise charges itself for the refusals its own downstream limiters produce: one person exhausting a 5-failure personal budget and continuing to retry spends 5 real attempts plus up to 95 of their own 429s, reaching a 100 ceiling and **refusing their entire proxy for 15 minutes**. Stated in user terms deliberately — every incident in this family was a legitimate person, never an attacker. CAPTCHA refusals (400) remain counted, so the ceiling still bounds the external verification call. ✅ **IMPLEMENTED — Story 13-70 FR1 on the login routes, R3 across every limiter on them, and R7 on the other four limiter pairs, all 2026-09-20. Eight route families, twelve limiters.** `loginRateLimit` and `strictLoginRateLimit` stamp `res.locals.rateLimitRefusedBy` in their `handler`; `loginIpFloodLimit`, mounted FIRST and therefore the only limiter whose response listener observes every other one's 429, skips on `requestWasSuccessful: (_req, res) => !res.locals.rateLimitRefusedBy` paired with `skipFailedRequests` (the predicate is consulted only when a skip flag is set, so the flag is what makes it execute). `verifyCaptcha` deliberately does not stamp, so its 400 stays counted. Residual **A1** (Story 13-68 adjudication, 2026-09-16) is closed against this. ⛔ **The subtle failure mode named below was the reason the STATUS-FILTER form was rejected** (`res.statusCode !== 429`, Option A, adjudication 2026-09-20): express-rate-limit applies the predicate in a RESPONSE listener, so a ceiling that skips 429s skips its OWN and its counter stops climbing at the limit — each request is still refused, but `attempts` in `auth.login_ip_flood_limit_exceeded` stops meaning "requests from this IP" and the clause's reopen trigger becomes undiagnosable exactly when it fires. The marker form has no self-skip by construction, because the ceiling's own 429s never set the marker. Which behaviour shipped is established in both directions by `login-rate-limit.binding.test.ts`, which reads the ceiling's COUNTER VALUE (not merely whether a request was served) and asserts it does not advance on a downstream 429 AND does advance on a genuine 401. ⭐ **THE CLAUSE IS MET ON EVERY LEG OF THE LOGIN ROUTES, and the rule is uniform there rather than special-cased.** A first pass implemented only the ceiling's half and MEASURED the remainder open: `loginRateLimit` is mounted before `strictLoginRateLimit` and `mfaRateLimit`, so it was still charging their 429s to the refused person's own per-email budget — once strict's hourly ceiling was spent, an address that had never been given a single login attempt on that IP lost all five of its failures to refusals it did not cause. Ruled and closed the same day (Story 13-70 R3): **every limiter on these routes stamps its own name when it refuses, and no limiter counts a request a DIFFERENT limiter refused.** That is why the marker carries a NAME and not a boolean — a boolean forces a limiter to choose between discounting its own 429s (Option A's trap) and counting everyone's, while the name lets it ignore every refusal but its own. `mfaRateLimit`, mounted LAST on the two MFA step-2 routes and therefore behind all three login limiters, is covered by the same rule; mount order alone could never have reached it. ⛔ **AND FOUR MORE ROUTE FAMILIES WERE FOUND VIOLATING THIS CLAUSE, by the adversarial review of Story 13-70 on 2026-09-20, AND CLOSED THE SAME DAY (R7).** The clause is route-general and says *any other limiter on the same route*; FR1 and R3 had implemented it on the four login routes only, and the annotation above had already been flipped to IMPLEMENTED — the drift class this arc exists to close, at a smaller radius. Every pair below had the identical shape the login pair had — a flood ceiling mounted FIRST counting all responses, with a person-keyed limiter behind it — and not one carried a skip predicate: `activationIpFloodLimit` → `activationRateLimit` (`auth.routes.ts:27,29`); `passwordResetCompletionIpFloodLimit` → `passwordResetCompletionRateLimit` (`auth.routes.ts:107-109,114-116`); `registrationRateLimit` → `registrationEmailRateLimit` (`registration.routes.ts:74-75`); `wizardDraftRateLimit` → `wizardDraftEmailRateLimit` (`registration.routes.ts:60-61`). ⭐ **The registration one was residual A1’s own incident on another route:** a citizen who spent their 3-per-email budget and pressed the button again charged their own 429s to the 50-per-IP ceiling their whole carrier gateway shares. **Now:** all eight stamp `res.locals.rateLimitRefusedBy`, and the four ceilings carry `skipFailedRequests` + `requestWasSuccessful: (_req, res) => !refusedByAnotherLimiter(res, SELF)`. ⚠️ The four person-keyed limiters stamp but take no predicate, deliberately: every mount of all eight was enumerated and each of them is LAST on every route it appears on, and giving them the flag too would register R4’s `close`/`error` listeners on BOTH limiters of a route, leaving nothing to count an aborted request. Mount a third limiter behind one of them and it needs the predicate — `rate-limit-coverage.test.ts` asserts each route’s limiter list, so that reds. **Pinned in both directions per family by `refusal-not-a-request.binding.test.ts`**, which reads the ceiling’s COUNTER VALUE from a probe in the real route stack: it does not advance on a downstream 429 and does advance on a served request. ⚠️ The in-memory-store limit of **R2** applies to these four exactly as it does to login, and R2’s post-deploy read should cover a registration refusal too.
            *   **Attribution.** Every 429 logs `keyedBy: 'email' | 'nin' | 'user' | 'token' | 'ip'`. Without it an investigation must reverse-DNS a handful of addresses to discover a limit was per-proxy — which is literally how the activation defect was found.
            ...
            *   **Spray is detected by cardinality, not volume.** 200 login requests from one address is three enumerators retrying **or** 200 accounts sprayed once each; identical counts, opposite meanings. The monitored signal is **distinct person-identifiers per IP per window with the failure ratio** — alert at > 20 distinct identifiers from one IP per hour with > 80% failures. Delivery is tracked as a dated follow-up (**2026-10-15**); until it exists, the detection capability is the operator query in `docs/runbooks/`, and that dependency MUST be stated rather than assumed away.
            *   **The proxy signature is a documented non-event.** High refusals concentrated on *few* distinct IPs is a proxy pool, not an attack. Six addresses producing 244 refusals alerts a human; it does not block.
            *   **Account lockout must decay.** A per-account failure counter never reset converts a brute-force defence into a standing denial of service: an attacker holds an account locked indefinitely at two requests an hour. Lockout counters reset when the lock expires.
            *   ⭐ **The enforcement tooling is subject to the defect class it polices.** A census classifying limiters by syntax rather than by the key actually returned will under-report; a monitor that cannot fail loudly will report a clean zero it never measured. Every such tool MUST carry a **positive control** — a known non-zero it can still find — and MUST be verified against `git show HEAD:<path>` rather than the working tree, so that repairing a defect does not erase the evidence that it existed. *On record: the limiter census reported 19 per-IP when the answer was 20; a repair then reported 23 (three false positives, because `keyGenerator: keyByUser` resolves to a person key); a further repair matched nothing at all, because `\s` and `\b` degrade inside a template literal — and every one of those runs emitted confident green output.*
        *   **NFR4.4.f — Known gap.** **"API Endpoints (General): 100 requests per user per minute"** — carried forward from the threshold list this clause replaces — **is specified but NOT IMPLEMENTED.** No general per-user API limiter exists in `apps/api/src`. It remains an **open requirement with an owning story**, not a satisfied threshold, and MUST NOT be restated as "unchanged" in any future revision.
...
*   **NFR8: Advanced Security & Concurrency (Anti-Fragility):**
    *   **NFR8.1: Race Condition Defense:** Critical uniqueness checks (e.g., NIN, Email) must rely on **Database-Level Unique Constraints**, not just application logic. The `respondents` table must enforce a **partial UNIQUE index on the `nin` column — `UNIQUE WHERE nin IS NOT NULL`** — to prevent duplicate registrations at the database level when NIN is present, while allowing multiple `NULL` NIN rows for `pending_nin_capture` and `imported_unverified` records (implements FR21 scope clause; realised by Story 11-1 migration). Partial-index semantics are preferred over full UNIQUE + application-layer NULL-handling because the constraint remains authoritative at the DB boundary and does not require service-layer branching to preserve race-safety.
...
*   **NFR9: Production SSH Access Control (Tailscale Daily-Driver + sshd-Hardened Public Surface).** Production VPS SSH (port 22/tcp) is governed by a **layered access model** with the **sshd-level key-only authentication as the primary access control**, not the network firewall:
    *   **Primary control (sshd):** `PasswordAuthentication no` (enforced in `/etc/ssh/sshd_config` main file **and** every drop-in under `/etc/ssh/sshd_config.d/*.conf` — drop-in alignment is load-bearing; a single `yes` in any drop-in defeats the control), `PermitRootLogin prohibit-password`, `PubkeyAuthentication yes`. Drift in this area is a P0 incident.
    *   **Operator daily-driver path:** Tailscale overlay network (`100.64.0.0/10` CGNAT range). Operator laptop + VPS are tailnet members; MagicDNS (`oslsr-home-app`) is the canonical SSH target. Future operator devices (phone, secondary laptop) join the same tailnet.
    *   **Network firewall (defence-in-depth + DDoS attenuation):** DigitalOcean Cloud Firewall permits SSH (22/tcp) inbound from **both** `0.0.0.0/0` (+ `::/0`) **and** `100.64.0.0/10`. The `0.0.0.0/0` source is **intentional and load-bearing** — required for two non-Tailscale access vectors that are operationally necessary: (a) GitHub Actions CI deploys (`appleboy/ssh-action` over public IP — runners are not Tailscale-eligible); (b) DigitalOcean Web Console + Recovery Console (SSH-based via DOTTY/`droplet-agent` from DO infrastructure IP ranges, e.g. `162.243.0.0/16` — empirically confirmed 2026-04-25, supersedes the original WebSocket-filtering hypothesis). The firewall is not the access control; it is defence-in-depth.
    *   **Active second-line defence:** fail2ban with the sshd jail (default `maxretry 5`, `bantime 10m`). With public-internet SSH reachable at the firewall layer, fail2ban is **active brute-force attenuation against the public attack surface** — not nominal insurance. Steady-state ban-list non-emptiness is health, not anomaly. Tuning is a real operational lever; revisit cadence per quarterly recovery drill.
    *   **Break-glass path:** DigitalOcean Web Console + Recovery Console retained as documented break-glass paths; both depend on the firewall permitting DO infrastructure IPs (currently covered by the dual-source rule). Documented in `docs/emergency-recovery-runbook.md`.
    *   **OS patching cadence:** monthly reboot with mandatory pre-flight checklist (tailscaled enabled-on-boot, PM2 startup hook + `pm2 save` executed, Docker `restart: unless-stopped`, sshd drop-in alignment unchanged, DO snapshot taken before reboot). Realised first time at 2026-04-25 (Story 9-9 subtask #2 done).
    *   **Realisation history:** Original tailnet-only firewall deployed 2026-04-23 (Story 9-9 subtask #1); amended to dual-source 2026-04-25 after empirical discovery that DO Console depends on public-internet SSH reachability. **Authoritative source for the as-deployed state is Architecture ADR-020 (V8.2-a1 follow-up amendment, 2026-04-25)** + `docs/emergency-recovery-runbook.md`. Rationale: Monday 2026-04-20 distributed SSH brute-force incident from 14+ IPs (see `new_err.txt` evidence block) — the incident drove the original tailnet-only design; the 2026-04-25 amendment refined the architecture without weakening the security posture (sshd hardening + fail2ban replace what the firewall narrowing originally provided).
    *   **Future state (Story 9-9 follow-up):** if a self-hosted GitHub Actions runner is brought up inside the tailnet, the firewall could re-narrow to `100.64.0.0/10` + DO published IP ranges (preserving Console as break-glass). Tracked in Story 9-9 follow-up subtasks; not MVP.
*   **NFR10: Partner API Token Storage, Hashing, and Rotation.** Partner API tokens (FR24) must be stored as **SHA-256 hashes** in `api_keys.token_hash`; **plaintext tokens are displayed exactly once** at provisioning and are not recoverable from the system thereafter. Token comparison at authentication time must be **timing-safe** (constant-time comparison of the hashed submitted value against the stored hash). Default rotation window is **180 days** with a **7-day overlap** during which both the superseded and successor tokens validate, permitting zero-downtime rollover by the consumer. A consumer may request an emergency rotation at any time; the superseded token is immediately invalidated (no overlap) and the event is audit-logged with principal `consumer_id` and `reason = 'emergency_rotation'`.
...
*   **Operator Access (per NFR9, SCP-2026-04-22 + V8.2-a1 follow-up amendment 2026-04-25):** Production VPS SSH operates under a **layered access model** — **sshd-level key-only authentication is the primary access control**; Tailscale overlay (`100.64.0.0/10` CGNAT) is the operator daily-driver path; the DigitalOcean Cloud Firewall permits SSH inbound from **both** `0.0.0.0/0` (+ `::/0`) **and** `100.64.0.0/10` (dual-source — `0.0.0.0/0` is intentional and required for GitHub Actions CI deploys + DO Console which is itself SSH-based via DOTTY/`droplet-agent` from DO infrastructure IPs); fail2ban is the load-bearing second-line defence against public-surface brute-force. DigitalOcean Web/Recovery Console retained as break-glass paths (documented in `docs/emergency-recovery-runbook.md`). Cloudflare WAF/CDN is planned but **domain-gated** on the `oyoskills.com` purchase and is not a prerequisite for field-survey start. **Authoritative as-deployed reference: Architecture ADR-020 (V8.2-a1 amendment) + Story 9-9 Change Log entries 2026-04-23 + 2026-04-25.**

#### Identity, Access & Data Sharing (Amended per SCP-2026-04-22)
*   **Nullable-NIN Registry Model:** `respondents.nin` is nullable. Identity uniqueness is enforced via a **partial UNIQUE index** (`UNIQUE WHERE nin IS NOT NULL`; NFR8.1) rather than a total UNIQUE. Records without NIN (`pending_nin_capture`, `imported_unverified`, `nin_unavailable`) coexist in the canonical `respondents` table with provenance captured via extended `source` enum, `external_reference_id`, `import_batch_id`, `imported_at` columns. Candidate-merge detection for NIN-absent rows is Import-Service-owned (Epic 11), not UNIQUE-constraint-owned.
*   **Public Authentication Channels:** Email **magic-link is primary**; password login is the fallback. SMS OTP infrastructure is built (service interfaces, provider adapter, audit wiring) but **inactive / feature-flagged** until a Nigerian-SMS provider agreement lands (budget-gated). No partner-API scope depends on SMS OTP.
*   **Third-Party Data Sharing Substrate:** Partner integrations (ITF-SUPA, NBS, NIMC, future MDAs) are served by the `/api/v1/partner/*` namespace gated by scoped API keys (FR24, NFR10), Redis-backed per-consumer rate-limits (Story 10-2), IP allowlists, per-scope time-bounded grants, and SHA-256 token storage. OAuth2 and mTLS are **explicitly deferred** — not MVP; revisitable post-Transfer if a partner requirement forces it.
*   **Secondary-Data Ingestion Substrate:** A batch-oriented Import Service (Epic 11) handles PDF / CSV / XLSX ingestion with dry-run preview, transactional confirm, and a 14-day rollback window. Per-batch `lawful_basis` capture is mandatory and linked to the DPIA (Baseline Report Appendix H + standalone D1).
...
*   **Epic 10: API Governance & Third-Party Data Sharing** _(Added 2026-04-23 per SCP-2026-04-22 — 6 stories: scoped API consumer auth, per-consumer rate limiting, consumer admin UI, public developer portal, Data-Sharing Agreement template + consumer onboarding SOP, consumer audit dashboard. Realises FR24 + NFR10. Transfer-phase prerequisite.)_
*   **Epic 11: Multi-Source Registry & Secondary Data Ingestion** _(Added 2026-04-23 per SCP-2026-04-22 — 4 stories: schema foundation (nullable NIN + partial UNIQUE + `import_batches` table + extended `source` enum), import service with PDF/CSV/XLSX parsers and dry-run/confirm/rollback endpoints, admin import UI, source badges + filter chips across Registry / Respondent Detail / Marketplace / Assessor Queue. Realises FR25. Reference implementation against ITF-SUPA public artisan PDF.)_
...
6.  **Right to Erasure:** Public Users can request complete data deletion via email to Super Admin. Admin must verify identity and manually purge data, logging the action in audit trail with justification.
    *   **Primary verification path (NIN-present records):** NIN + Phone validation against the `respondents` row.
    *   **Alternative verification path (NIN-absent records — `pending_nin_capture`, `imported_unverified`, `nin_unavailable` per FR28 / FR25):** **Phone + Date of Birth + LGA match** against the `respondents` row, plus one of: (a) the magic-link email address on file, or (b) an in-person supervisor attestation countersigned by the assigned LGA Supervisor. The alternative path closes the NDPA erasure gap opened by the nullable-NIN model (SCP-2026-04-22) without requiring the respondent to obtain a NIN purely to be erased. All verification attempts (successful or failed) are logged to `audit_logs` with principal = super-admin `actor_id`, target = respondent record, and verification-path metadata, surfaced via FR26 audit viewer.
...
5.  PDF exports honour a server-side row cap (HTTP 413 above the hard threshold; UX modal in the soft band) with format-appropriate defaults — CSV is the supported format for large datasets. Domain-bounded exports (e.g. LGA enumerator productivity rollups) are exempt where the row count is small by design. See **ADR-021** + `docs/decisions/2026-05-08-pdf-export-row-cap.md` for thresholds and rationale.
...
2.  **Enhanced Bot Protection:**
    *   **Rate Limiting:** 30 search queries per IP per minute — flood axis, Tier 2, governed by the NFR4.4.c register (which is authoritative; do not restate the number here). ⚠️ **"CAPTCHA required after 10 queries in 5 minutes" is NOT IMPLEMENTED** (*verified 2026-09-17*): `verifyCaptcha` is mounted on contact-reveal and edit-token-request, but `GET /marketplace/search` carries `marketplaceSearchRateLimit` alone. Per NFR4.4.f this is an open requirement with an owning story, not a satisfied threshold — and per NFR4.4.c it MUST NOT be assumed when sizing the search ceiling.
