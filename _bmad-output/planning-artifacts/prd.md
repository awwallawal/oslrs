...
*   **NFR4: Security & Compliance (NDPA-Aligned)**
    ...
    *   **NFR4.4 Defense-in-Depth:** Rate Limiting (Redis), Honeypots, strict Content Security Policy (CSP), and per-IP flood ceilings. Thresholds are governed by the identity-axis rules below (*restructured 2026-09-17, Lane C, per PM ruling `pm-ruling-2026-09-16-nfr4-4-rate-limit-axis.md`*). The previous revision's blanket licence — *"and IP Throttling with the following thresholds"* — is **withdrawn**: it permitted any author to bound any route by `req.ip` without stating whose traffic that bounded, and four production incidents came from limiters it licensed but never named.
        ...
        *   **NFR4.4.e — Login Attempts** (covers `/auth/staff/login`, `/auth/public/login`, `/auth/login/mfa`, `/auth/login/mfa-backup`) — stratified defence in layers on two axes (per person, per IP); each layer counts a specific subset of responses. **Mount order is part of the control** (*adversarial review 2026-09-16*): IP flood ceiling → CAPTCHA → burst limiter → sustained limiter → (MFA gate on step 2) → controller. The CAPTCHA sits before the per-email budget so a request with no CAPTCHA cannot spend anyone's budget; the sustained limiter sits last so it is charged only for CAPTCHA-passing attempts the person's own budget allowed:
            ...
            *   **IP flood ceiling (`loginIpFloodLimit`):** 100 requests per IP per 15 minutes, **all responses counted**, mounted **first** (*added 2026-09-16, Story 13-68*). **Load-bearing:** it is the only login limiter that counts successful responses, so it is the only bound on successful request volume from one IP (for example, bulk validation of credentials an attacker already holds); being first, it also bounds CAPTCHA-refused traffic and so the hCaptcha verification call. It is **not** the bound on a stream of failures — the sustained limiter refuses that at 61 per hour first. Same file.
        ...
        *   **NFR4.4.c — Threshold register (NORMATIVE).** Every rate control in the system appears below with its axis, key, budget, window, counting mode and tier. **No control may exist outside this register.** The five free-standing thresholds this clause replaces (Marketplace Search, Contact Reveal, API General, Password Reset, Profile Edit Token) are carried into it unchanged in value; what is added is the axis each was always implicitly asserting, and the 28 controls that were never named at all. The register is **enforced in code** by the coverage map in `apps/api/src/middleware/__tests__/rate-limit-coverage.test.ts`, which is the authority on current values; this clause is the authority on what a legal row looks like.
            *   **Login** (see NFR4.4.e for the full rationale) — burst `loginRateLimit`: person / email digest · 5 · 15 min · FAILED · T1. Flood `loginIpFloodLimit`: flood / IP · 100 · 15 min · ALL · T1. Sustained `strictLoginRateLimit`: flood / IP · 60 · 1 h · FAILED · T1 (derived 3 × 20). Account lockout `users.locked_until`: person / user · 10 failures → 30 min, counter cleared on expiry.
        ...
        *   **NFR4.4.d — Governance and observability (NORMATIVE).**
            ...
            *   ⛔ **A refusal is not a request.** A 429 emitted by one limiter MUST NOT be counted by any other limiter on the same route. Mounted first and counting all responses, a flood ceiling otherwise charges itself for the refusals its own downstream limiters produce: one person exhausting a 5-failure personal budget and continuing to retry spends 5 real attempts plus up to 95 of their own 429s, reaching a 100 ceiling and **refusing their entire proxy for 15 minutes**. Stated in user terms deliberately — every incident in this family was a legitimate person, never an attacker. CAPTCHA refusals (400) remain counted, so the ceiling still bounds the external verification call. ⚠️ **NOT YET IMPLEMENTED — this clause states the TARGET, not the current build.** No limiter in `apps/api/src/middleware/` carries a predicate excluding 429s; `loginIpFloodLimit` counts them today, so the 15-minute proxy refusal described above is live behaviour, not a hypothetical. Tracked as residual **A1** (Story 13-68 adjudication, 2026-09-16) and routed to its own story: the change is a security-behaviour change to a limiter, which per `feedback_code_review_no_story_dev` takes a dev-story and an adversarial review, not an inline edit. ⛔ **It also has a subtle failure mode the story must pin:** express-rate-limit applies the skip predicate in a RESPONSE listener, so a ceiling that skips 429s would skip its OWN 429s too and its counter would stop climbing at the limit. That still refuses each request, but it is not what the clause says, and a test must establish which behaviour shipped.
