...
**Source Documents:** `_bmad-output/planning-artifacts/epic-12-measurement-honesty-and-dashboard-refresh-brief.md` (full brief + per-page drift inventory + story shells); `_bmad-output/wireframes/*.excalidraw` (Track-C north-star); `9-59-unified-registry-export.md`.
...
**Tiering:** 🚦 Pre-spend (this week) — 13-1, 13-3, 13-4. Monday (zero-cost, no gate) — sheet **frozen** (spec doc) + umbrella meeting + distribution. Fast-follow (post-spend) — 13-2 importer (on the 11-2 backbone), 13-5 Yoruba comprehension layer, 13-6 channel + coverage dashboard. 🚦 Launch-deploy-reliability — **13-7 Web-test IndexedDB polyfill** (added 2026-06-27, emergent from the 13-1 deploy: a non-deterministic `MissingAPIError` unhandled rejection blocks CI `test-web` at random and can stall any launch-week deploy; web-only fake-indexeddb setupFile, api untouched — pure test-infra, no scope change). 🚦 Launch-resilience — **13-8 Launch resilience hardening** (added 2026-06-27, emergent from the 13-3 load test: the prod box is a CPU-bound 1-vCPU droplet; bundles the droplet resize to 2vCPU/4GB [#1 lever, operator], a p95 alert-threshold tune so a slowdown pages Telegram, the `tsx`→`node dist` compiled-runtime fix [careful, prod-boot risk], and the failover dry-run — documented so the hardening isn't only in git). 📊 Campaign-measurement (added 2026-06-27, from John's PM brief) — **13-9 Campaign engagement tracking** (🚦 pre-blast, must ship before Jul-1: campaign-tagged magic-links → 13-1 attribution + bounce/complaint suppression are one-way doors; Resend webhook → email_events; **opens OUT** — privacy + unreliable) + **13-10 Campaign dashboard** (post-launch: per-campaign funnel + one-click retarget cohorts + channel comparison on the existing surface — *the retarget loop is the product*).
...
**Tier:** 🚦 pre-spend (target live before Monday 2026-06-29) · **Status:** done. _(Parity-corrected 2026-07-30 — this line still read `ready-for-dev` while `sprint-status.yaml` has read `done`; the board is canonical.)_
...
**Tier:** sheet=Monday (zero-cost, no gate); importer=fast-follow (cascade is async) · **Status:** ready-for-dev. _(Parity-corrected 2026-07-30 — this line still read `backlog`; `sprint-status.yaml` has read `ready-for-dev`. Sequencing caveats live on the board entry.)_
...
**Tier:** 🚦 pre-spend (sibling of Story 9-20; REUSE the DONE monitoring stack — net-new = ONLY the load test + the fallback) · **Status:** done. _(Parity-corrected 2026-07-30 — this line still read `ready-for-dev` while `sprint-status.yaml` has read `done`.)_
...
**Emergent expansion 13-7 … 13-65 (parity note, 2026-07-18; 13-37/13-38 added 2026-07-19; 13-39/13-40 added 2026-07-20; 13-41 added 2026-07-22; 13-42 added 2026-07-23; 13-43/13-44 added 2026-07-24; 13-46 added 2026-07-30; **13-49 … 13-55 added 2026-08-08 (John/PM) — this index had lapsed SEVEN stories behind `sprint-status.yaml`, every one of them created between 2026-08-01 and 2026-08-07** — `13-45` is RESERVED for `docs/adjudication-agent-handoff.md` §8 D3, the story-status CI guard, and is deliberately unallocated until 13-41 lands the shared `ci-guard` toolkit — indexed here 2026-07-30):** Epic 13 grew well past the original six as launch prep surfaced defects and mechanisms. These emergent stories are tracked canonically in `sprint-status.yaml` + their own `13-*.md` files (full ACs there — NOT re-documented here to avoid drift); this is the index:
...
- **Public-honesty + send-coordination track (2026-07):** 13-22 skills vocabulary/extraction reconciliation (done+deployed — JSONB-vs-space-split) · 13-23 public-submission form-binding (done+deployed — questionnaire_form_id sentinel) · 13-24 coordinated welcome-send + anti-whiplash blast sequencing (**done+deployed** — parity-corrected 2026-07-30, this line still read `ready-for-dev`; ⚠️ **but its operator Task 5** — the welcome backfill to ~116 emailable, Resend-Pro-gated — **is still an unchecked `[ ]` under a story marked `done`**, and it is step 2 of the launch sequence; see `docs/adjudication-agent-handoff.md` §4. **CODE story after the 2026-07-23 re-scope** — implemented inherited email-keyed contact-dedupe closing the verified auto-send↔blast double-send gap, not ops-only; Resend-Pro-gated) · 13-25 public /insights registry-count honesty (done+deployed — LAUNCH-SLICE of Epic 12's 12-4 count-core for the public surface) · 13-26 shared `loadEnv()` (ready-for-dev, POST-LAUNCH hygiene, boot-path/SEC-3 risk).
...
- **CI + test-infra hardening (2026-07):** 13-30 reauth-routes test-teardown FK-race helper (done+deployed — shared audit-safe teardown) · 13-31 osv-scanner audit-gate migration (done+deployed — npm retired `pnpm audit`, 410) · 13-32 osv-scanner prod-scope + vuln remediation (done+deployed — two-tier prod/report gate + turbo 1.13→2.x; removed 13-31's 2026-08-15 grandfather-expiry cliff at the root) · 13-36 E2E messaging determinism + workflow reliability (**done+deployed 2026-07-27 `830dcf7`**, final pass `be207c1` 2026-07-30 — test-hygiene; deterministic socket.io waits + a converging `db:seed:dev`; the E2E workflow is now a TRUSTWORTHY signal, green on push and on a `repeat_each: 3` burn-in, so a red there is triaged, never reflexively re-run; the last "accepted residual" was overturned and root-fixed by pinning `workers: 1`) · 13-37 registry-read-drift CI guard (**done** — ⚠️ *this entry read `review` until 2026-08-08; `sprint-status.yaml` has carried `done` since the push that discharged AC6. Its DISCHARGE-ON-PUSH note below is preserved as the record of HOW it closed, not as its current state.* Post-launch tooling; fails `lint-and-build` on a NEW hand-rolled respondent⟕submission read outside the sanctioned modules, enforcing 13-33's canonical `registryUnifiedSource`; emergent from the 13-33 backlog-harmonization sweep. Implemented + adversarially reviewed 2026-07-30 (12 findings: 3 High/4 Med/5 Low, all fixed in-pass). **Stays `review` deliberately: AC6's CI leg is DISCHARGE-ON-PUSH** — High finding H1 was that the named guard step could never execute because `- name: Lint` ran first and already invokes the guard via the api `lint` chain, so the fix *changed the CI wiring* and only a push can confirm both the green and the step ORDER. It closes on push, not on the review.).

  _(Parity-corrected 2026-07-30 — this bullet listed BOTH 13-36 and 13-37 as `ready-for-dev`. 13-36 had been done+deployed since 2026-07-27 and 13-37 was `review`; `sprint-status.yaml` is canonical.)_
- **Convergence + pre-blast polish (2026-07):** 13-33 canonical unified registry read + honest density map (**done+deployed 2026-07-19** — respondent-anchored single read; banded density disclosure; parity smoke view≡inline≡count-core≡export; live /insights density now populated) · 13-34 pre-blast form-content (remove the live public GPS question + clarify the "Main Occupation/Job Title" label; XLSForm re-upload+re-pin + a public-wizard geopoint-suppression guard) — ⚠️ **no longer form-only: the 2026-07-22 adversarial review surfaced a live 22P02 500 class** (a `::uuid` cast on the TEXT `submissions.questionnaire_form_id`, which product code fills with sentinels like `supplemental-survey`/`self-edit`; the `~ regex AND ::uuid` idiom does not guard it) affecting the staff registry list/search/detail + fraud-detection detail reads — fixed at 4 sites + 1 prophylactic (`supervisor.controller` `submitter_id`), so **13-34 carried an API deploy** and shipped before the blast (the blast itself grows the sentinel population that trips it) — **done+deployed 2026-07-23**. ⚠️ **Correction 2026-07-30:** 13-34's claim that "every `::uuid` cast on a TEXT column in `apps/api` is accounted for" is **disproven** — `controllers/form.controller.ts:276` and `:325` both cast `${submissions.submitterId}::uuid` unguarded (`db/schema/submissions.ts:51` declares `submitterId: text('submitter_id')`), the same class 13-34 fixed prophylactically two files away at `supervisor.controller.ts:144`. Risk is prophylactic today (every writer writes `null` or a UUID; `NULL::uuid` is safe) so 13-34 stays `done`; the fix is owned by 13-41 AC10.4 · 13-41 CI guard against the unsafe-SQL-cast class **wide** (any stricter-type cast on a TEXT column incl. `raw_data->>'…'`, plus the false-guard idiom itself — ready-for-dev, sequences after 13-34's deploy). **Materially RE-SCOPED 2026-07-30 (John/PM, triggered by the 13-37 review):** it no longer MIRRORS 13-37's plumbing, it **EXTRACTS** it into a shared `apps/api/src/lib/ci-guard/` toolkit with an AST source model (new **AC11**) consumed by thin `scripts/lint-*.ts` runners, and repoints 13-37 onto it under an equivalence gate; new **AC12** makes 13-37's "fixtures only encode the evasions their author imagined" lesson testable; new **Task 0**; AC10's "zero hits" premise replaced by a measured baseline (≥55 live sites) + per-hit disposition. **NEW HARD DEPENDENCY: 13-37 must be `done`, not merely `review`** — Task 0 refactors 13-37's shipped files, so starting earlier makes any red ambiguous between the two stories. Still **POST-LAUNCH, not launch-gating** — settled 2026-07-30 by a read-only prod query: `registry_unified` has 82 rows with a `dob`, **0 unparseable, 0 empty strings**, so the `(raw_data->>'dob')::date` exposure on the public /insights page is prophylactic today, not a live defect · 13-42 ops-digest data-integrity watch (ready-for-dev, POST-LAUNCH observability — standing digest signals for the sentinel population + self-edit path liveness, each shaped to fire only on the real defect delta (a NEW sentinel value; audit/submission divergence), not on blast volume or the benign no-traffic zero; WRITE-side sibling of 13-41's read-side guard). 📋 **Epic-13 RETRO INPUT is recorded in the 13-34 story file** ("Retro Input (Epic 13)" section): a flaky-test heuristic that nearly buried a prod 500, reproduce-by-construction over reproduce-by-repetition, and four review-surviving writings of one broken idiom · 13-35 wizard UX polish (**done+deployed 2026-07-25 `eee2877`** — skills-picker "Done" close affordance + positive framing of prefilled/"collected-earlier" questions) · 13-38 marketplace "[Association] — confirmed member" badge (ready-for-dev — two-tier association-confirmed→member-verified trust badge, honest-naming; renders 13-2's association provenance; emergent from Awwal's verification reframe).
...
- **Post-launch debt + observability track (added to this index 2026-07-30 — 13-43/13-44 had been on `sprint-status.yaml` since 2026-07-24 but were never indexed here):** 13-43 react-router 6→7 migration (ready-for-dev — retires the accepted-risk debt: removes the bounded override + the 3 `osv-scanner.toml` entries; **KEEP `apps/web/src/lib/safe-redirect.ts`**, the app-layer open-redirect mitigation, after the migration. Pull trigger: another 6.x advisory) · 13-44 super-admin campaign observability (ready-for-dev — wires the built-but-unwired `getCampaignFunnel` from 13-9 + a `campaign_sends` contact log + a ledger-liveness banner, because `recordCampaignSend` is FAIL-SOFT; reuse the 9-11 Audit Log Viewer pattern incl. its PII precedent).
- **Public-burst readiness (added 2026-07-30 — the RADIO channel of this epic's own 5-channel push finally gets its WRITE-side controls):** 13-46 public-burst readiness — cap the SEND, not the SIGNUP (**ready-for-dev, LAUNCH-ADJACENT — it gates the jingle, which precedes the blasts; NOT post-launch hygiene**). Emergent from Awwal's decision to run a radio jingle straight at the public wizard. The code does not block the jingle; the CONTROLS are wrong in both directions at once. **(a)** `registrationRateLimit` is 5/IP/15min with the docblock *"Prevents mass account creation"* (`middleware/registration-rate-limit.ts:20-24,31-32`) mounted on the **accountless** public wizard submit (`routes/registration.routes.ts:48-51` records the transplant from the legacy `/auth/public/register` discipline); under Nigerian carrier NAT that key is a whole carrier's subscribers — `realIpMiddleware` resolves `CF-Connecting-IP` **before** every limiter (`app.ts:127-133`) — so the 6th listener behind a carrier IP in 15 min takes a 429 traced only by one `logger.warn` (`registration.rate_limit_exceeded`). `wizardDraftRateLimit` fails *earlier and more silently*: 120/IP/15min sized on the stated assumption *"Shared NATs (~5 wizards concurrently) stay well inside budget"* (`wizard-draft-rate-limit.ts:20-28`), an assumption a state-wide jingle breaks by construction. **(b)** The asset actually at risk — **sending reputation** — has no ceiling: every public registration fires the thank-you **synchronously in-request** (`registration.controller.ts:855-863` → `submission-processing.service.ts:1048` `thankyou-referral-auto` → `email.service.ts:100,115`; never queued), while `NotificationMeter` holds only TTL constants, is documented **FAIL-OPEN** and runs *after* the provider with its result discarded (`notification-meter.service.ts:23-25,60,62,150-153`; `email.service.ts:116-122,143`) — so a cap is a new **pre-send** check, and it must be **category-aware** because fail-open is *correct* for magic links. Amplifier: the auto-send marker is per-**respondent** while there is no email dedupe on the respondent side (no `email` column on `respondents` at all; NIN dedupe is gated `if (ninValue)` so the **pending-NIN path has none**) ⇒ one address, N registrations, N emails. Resend Pro's 50k/mo is a **ceiling on damage, not a protection** — a larger quota makes the cap more important, not less. **Fix order (Awwal's accepted leverage ordering, load-bearing):** cap the send → per-address throttle reusing `MARKETING_CONTACT_GAP_DAYS` → a global burst breaker that **alerts rather than blocks** (composing 9-52's `cf-traffic-watch` dispatch + cooldown, because Awwal wants to know the jingle *worked*) → **only then** rescope the per-IP limits → publish **registered vs verified** on `/insights` via Axis 3 of `registry-data-status-taxonomy.md`, extending 13-25's `getRegistryCountCore` (today the public headline summary carries **no WHERE clause at all**, `public-insights.service.ts:132`) → an operator CF WAF rate-limit rule scoped **TO** the registration path (the inverse of `pre-viral-push-checklist.md:27,50`'s *"exclude `/api/*`"*, which excludes the exact path at risk) → a before/after count of the **already-logged** `registration.rate_limit_exceeded` as the "how many did we turn away" evidence. **Preserved, not reopened:** no captcha on the wizard (the magic link + captcha-gated login is the real gate) and **Bot Fight Mode stays OFF** (9-20). **EXTENDS three existing artifacts and creates no new runbook** — `docs/runbooks/13-3-cutover-and-failover.md` (the radio-jingle runbook that already exists; note 13-3's load test measured a **read** endpoint, so its green verdict says nothing about the write path), `pre-viral-push-checklist.md`, and `roadmap-to-launch.md:115-124`'s paid-spend pre-flight gate. Write-side sibling of 13-42; consumes 9-52's alert path; AC5 follows 13-25's launch-slice precedent. **Not dependent on 13-37 or 13-41.** 📻 **Attribution addendum (2026-07-30) — note for anyone reading this index: 13-1's "How did you hear about us?" capture is NOT missing.** It is live on the Review step (`Step5ReviewAndSave.tsx:229`), `ATTRIBUTION_ENABLED = true` (`attribution.ts:12`), **Radio is the first option** (`attribution.ts:15-25`), and the chain runs end to end into `submissions.raw_data.campaign_source` (`registration.controller.ts:103-115,735-738`) and back out via `ReportService.getCampaignBreakdown` (`report.service.ts:86-95`). The gaps are narrower: the path has **executed zero times on prod** (`campaign_source` on **0 of 82** submissions), so the pre-jingle gate is a **liveness dry run, not a form change** (AC9, asserting the *read* side too so it cannot become another built-but-unconsumed `getCampaignFunnel`); `"Prefer not to say"` is the **pre-selected first option**, so *declined* and *ignored* are the same stored value and the channel denominator is unrecoverable (AC10a); and **first-position bias sits on Radio itself** (AC10b). Station-level attribution is deliberately absent (`attribution.ts:14`, 13-1 AC2.4) — a **media-buy** decision, with per-station UTM links (already wired, `WizardPage.tsx:171`) as the no-code alternative that attributes even when the listener skips the question. **Making the question mandatory is an explicit OPEN DECISION for Awwal, not an AC** — it reverses 13-1's recorded "prominence ≠ mandatory" guardrail, and forced choice with Radio first would manufacture the signal the jingle is meant to measure.
- **🔴 Live prod defect found by a dry run (2026-07-30):** 13-47 wizard draft-step cap + attribution durability (**`done`** — ⚠️ *this entry read `review`/"awaiting deploy" until 2026-08-08; it shipped and `sprint-status.yaml` has carried `done` since.*). `saveDraftSchema.currentStep: .max(5)` was correct for Story 9-12’s FIXED five-step wizard and was never re-derived once the step count became FORM-DRIVEN (`3 head + one per form SECTION + 1 review`). **13-34’s 2026-07-23 re-pin published a six-section public form → N=10 → every draft autosave from step 6 up 400’d for seven days**: 232 of 293 drafts frozen at step 4-5 (`MAX(current_step)=5` in a ten-step wizard), draft-resume silently discarding answers past the cap, and Story 13-1 attribution — written on the REVIEW step — structurally undeliverable (`campaign_source` on **0 of 84** submissions). Near-miss only because 291/293 drafts predate the re-pin and submissions still succeeded via 13-23’s payload precedence; the exposure was ahead, since those drafts ARE the blast audience. Fixed the cap, moved attribution into the submit payload (payload→draft precedence, bounded schema), added server-side rejection logging + an honest client alert, minted **Pitfall #46**, and extended all 292 live drafts by a month. Found by 13-46 AC9’s liveness dry run, not by any test — every automated signal was green throughout (the e2e resume harness only exercises steps 0-2).
- **Test-environment fidelity (2026-07-30):** 13-48 e2e multi-section wizard fixture (`ready-for-dev`, POST-LAUNCH). Answers *why no test caught 13-47* — and the answer is not a missing assertion. The e2e environment serves **no pinned public form** (`wizard-registration.spec.ts:140`: *the form fetch 404s → survey skipped → 4-step head model*), so CI drives a **4-step** wizard while production runs **10**. The `.max(5)` cap was unreachable by construction, and GP-6 — the one draft-persistence e2e — is `test.fixme`. Scoped as a **fixture** story: seed a representative ≥3-section public form, derive `N` from the pinned form rather than hardcoding it, and assert the **draft row** at the final step (not that submit succeeded — submit worked throughout the outage). ⚠️ Known limit stated in-story: CI guards only the code side; a prod re-pin is data, so `registration.draft_rejected` routed into 13-42 is the only prod-facing signal.
- **The 2026-08 post-launch wave — draft adoption, and the identity-chokepoint class (indexed 2026-08-08, John/PM):** 13-49 draft-adoption programme D1-D6 (**done+deployed** — registry 145 → 310; R21 found and fixed the identity guard that never ran on the public wizard, verified by an OBSERVED attach not by a deploy. ⚠️ Its own lesson outranks its output: **a batch job against a live system races the people in it** — 7 of 174 adopted people ended up with two records, because `findOrCreateRespondent` dedupes on the INCOMING NIN and a no-NIN self-registration matches nothing; 24% of the pending-NIN cohort within 90 minutes. **The copy was a cause, not a comment** — D1 and D3 got identical text saying "your record is active" and "add what is missing", which is an invitation to do the one thing that duplicates them) · 13-50 `/check-registration` dead-end links + `wizard_resume` audit blindspot + phantom drafts (backlog — 244 sends/month; degrades WITH engagement, so it worsens exactly when the campaign works) · 13-51 operator contact-correction + suppression lift (backlog — validate email at capture so we stop manufacturing bounces) · 13-52 deploy resilience, open tabs survive a release (**done** — hotfixed as `aac6153` BEFORE the story existed, recorded as process; ⚠️ **its TESTS were never written**, which is the open half) · 13-53 identity guard covers the NIN-arrival journey (**done+deployed** — the seam between "register without a NIN" and "come back with it", found on a REAL citizen: `56C9PG`/`W1PS38`, same phone, one person, two records. Ships on 3603 tests and THREE RED-verifies. ⚠️ **Read HOW it closed:** R2 asked for a prod observation that does not exist — the event needs a pending-NIN person to re-register via the front page, and it was NOT manufactured because the smoke refuses non-test DBs by design. Closed with the gap STATED, watch handed to 13-44 AC-T4 as a digest PAIR. **Reopen on any NIN-arrival duplicate pair on prod, or the cohort climbing while promotes stay at zero**) · 13-54 respondent-write chokepoint CI guard (**done+deployed 2026-08-08** — adjudicated and closed the same day it was scaffolded; R1 discharged by READING the CI step list (step 12 `success`, above `Lint`) rather than the job badge. ⚠️ **Known limit #6, found at adjudication:** the allowlist is PER-FILE, so it would NOT have caught R21 or 13-53 — both created inside files now allow-listed. It stops a FIFTH ingestion path; fewer sanctioned creators is 13-55, which this ARGUES FOR SEQUENCING SOONER — ⭐ the highest-evidence item on the list: the same bug has been hand-fixed THREE times (R13 → R21 → 13-53), each fix protecting one more caller, twice costing a named citizen two records. Makes un-guarded respondent **creation** a build failure via a sibling of the proven `lint-registry-read-drift.ts`. ⚠️ **Scope, because the title overclaims:** it catches `insert(respondents)` only — `update(respondents)` spans 12 files and belongs to 13-55) · 13-55 unify the three promote-to-active paths (backlog — three paths, three identity keys, three status scopes, all writing `PENDING_NIN_PROMOTED`. **Goal is ONE CODE PATH WITH EXPLICIT PARAMETERS, NOT one policy** — a magic-link token is stronger evidence than a fuzzy name match and should keep a wider scope. Sequences AFTER 13-54: the guard stops the bleeding, this stops the scarring).

  _(⚠️ **Index-lapse note, 2026-08-08.** This bullet was added when a 13-54 parity sweep found the index eight days and seven stories stale. The lapse is itself the finding: emergent stories reach `sprint-status.yaml` and their own `13-*.md` immediately, and this index only when someone remembers. Two stale labels in the bullets ABOVE were corrected in the same pass — 13-37 and 13-47 both still read `review` here long after closing. **`sprint-status.yaml` is canonical; this index is a convenience view and should be read as potentially behind it.**)_
- **Field-day + hotfix wave (13-56 … 13-62) — INDEXED 2026-08-21 (John/PM), and the lapse is the finding again.** These seven reached `sprint-status.yaml` and their own `13-*.md` between 2026-08-08 and 2026-08-10 and were never indexed here; `sprint-status.yaml` is canonical and is the source of every status word below. Index depth only — full ACs and the (substantial) adjudication records live in the story files, deliberately NOT duplicated here: 13-56 commit–story coverage audit + guard (backlog) · 13-57 the ingestion boundary must fail loudly (**done+deployed 2026-08-15 `4e36a92`**) · 13-58 marketplace "association-confirmed" badge (backlog) · 13-59 staff activation-confirmation email (**done+deployed 2026-08-17 `63088f8`** — FIELD-DAY GATE 3 of 3; its review also found `packages/*` had NEVER been linted, so a green `pnpm lint` was a FALSE REPORT rather than missing coverage) · 13-60 a failed selfie must not be swallowed (**done+deployed 2026-08-14 `6876b9f`** — FIELD-DAY GATE 1 of 3) · 13-61 staff list returns citizens (**done** — emergent hotfix 2026-08-09, reported by Awwal from the live Super Admin UI) · 13-62 a query parameter the server never read (ready-for-dev — generalises 13-61 from one call site to the CLASS; ⛔ NOT a field-day gate).

- **Radio attribution + registry honesty — the 13-46 residual carve (authored 2026-08-21 by Bob/SM from 13-46's residual ledger; sequenced 2026-08-21 by John/PM):** **13-63** station-level attribution — vanity paths that survive the landing, and a number someone can read (`ready-for-dev`; carries residuals **R3** + **R9** + **R10**, and **R2** as a LOCKED AC7) · **13-64** data-loss marker tail — six respondents in the wrong bucket (`ready-for-dev`; carries residual **R7**).
  - 🚦 **SEQUENCING RULING 1 (John/PM, 2026-08-21) — 13-63 AC1–AC3 GATE PAID SPEND; AC4–AC7 do not.** Recorded as item 6 of the `roadmap-to-launch.md` pre-flight gate, as a §0 go/no-go row in `pre-viral-push-checklist.md`, and as row 6 of `enumerator-prod-smoke-and-golive-gate.md`. **Why:** this is the only gate input that is PERISHABLE — 11 stations are on the buy (`roadmap-to-launch.md:107`), the URL becomes physical the moment the jingle is recorded and read on air, and there is no backfill for a signal that was never emitted; miss the first spot and the station dimension is unrecoverable for the whole flight, leaving "radio worked" as the only available finding when the decision the money actually buys is *which of the 11 to renew and which to kill*. The cost of holding is the movable 24–48h radio date this gate already assumes, against a discharge that is **zero code** (Cloudflare Redirect Rules + a committed slug registry + one prod dry run) — a bounded, reversible delay against an unbounded, irreversible loss of measurement. Capture is perishable, reporting is not: if AC4's per-station read slips, the rows still exist and SQL recovers the answer afterwards.
  - 🔴 **THE REDIRECT TARGET IS `/register?ref=<slug>`, NOT `oyoskills.com/?ref=<slug>`** — verified by grep 2026-08-21 and propagated to every artifact that stated the apex form (`13-3-cutover-and-failover.md:84` corrected in the same pass). `parseUtm` is called from exactly ONE place (`WizardPage.tsx:172`, mounted at `/register`) and every CTA is a bare `<Link to="/register">` that discards the query string, so an apex `?ref` dies at the first click. Built from 13-46 R3's original wording the rules would have been live, verified-as-redirecting, and attributing **nothing** — [[pattern-ship-a-fix-that-never-fires]] at the exact moment the money is spent. **Do not restate the apex form anywhere.**
  - 🚦 **SEQUENCING RULING 2 (John/PM, 2026-08-21) — a WRITE-PATH capacity check is now pre-flight gate item 7**, and it belongs to no story yet (SM to author). 13-3's AC1.1 asked for "the wizard entry + draft-save + submit path (the real registration hot path)"; the run that produced the green verdict measured `GET /api/v1/forms/public-active` (`13-3-launch-capacity-and-static-fallback.md:31,51`). Measured on prod 2026-08-21, **one public registration fires THREE synchronous outbound emails** (magic link + reference confirmation + thank-you outreach — 13-46 R6), so the load is **~3N per registration on a 2GB VPS with no swap**, where the failure mode is an **OOM kill, not a graceful degrade**. Neither existing mitigation covers it: 13-3's static fallback protects the READ (and a jingle landing on a static page has already failed), and 13-46's burst breaker **alerts rather than throttles**, deliberately. Discharge by ANY ONE of: a bounded write-path test at a stated peak, same rig as 13-3 and the correct endpoint; **queueing the thank-you outreach** so the in-request cost falls to the transactional sends (⚠️ all 10 BullMQ workers run IN the API process — that buys shedding and retry, **not** CPU isolation); or a **written** peak estimate, with its basis, that 13-3's measured headroom bounds. ⛔ An unstated estimate is not a discharge.
  - ✍️ **RULING 2 IS NOW ANSWERED — `13-65-registration-sends-off-the-request-path` (authored 2026-08-22 by Bob/SM, `ready-for-dev`).** It takes **(a) AND (b) together as ONE story**, on Awwal's explicit instruction (2026-08-22) not to slice it into gating and non-gating halves, and it queues **all three** registration-triggered sends onto the **existing** `email-notification` BullMQ queue — not only the thank-you. **The failure mode is MEMORY, not CPU:** the fan-out is unbounded today, so a burst holds ~2N–3N concurrent TLS sockets and rendered HTML bodies in a 2GB heap with no swap; queued, at most **5** run at once (`email.worker.ts:248`) and the rest are Redis-resident job records. It also closes two gaps nothing else does — today a transient provider 5xx **silently loses** a citizen's email (counted, logged, never re-sent) and a restart mid-burst drops every in-flight send with no record. ⚠️ **The claim is bounded and AC9 enforces it: bounded concurrency, durability, retry and backpressure — NOT CPU reduction and NOT event-loop isolation**, because all 10 workers run in the API process. ⛔ **PM2 topology is a constraint, not a question** — battle-tested and hardened, not changing (Awwal, 2026-08-22); story **11-8** stays in backlog and the story forbids "process isolation" appearing as a follow-up in any artefact. ✅ **Option (c) — a written peak estimate — is deliberately left standing** as a legitimate discharge if 13-65 slips. **Three drafting-brief corrections that changed an AC:** (i) 🔴 the registration journey *does* carry a user-blocking send — `WizardPage.tsx:559` awaits `POST /auth/public/magic-link` before painting the success screen — which is out of scope by instruction, recorded with its one-keyword fix and a reopen trigger rather than lost; (ii) 🔴 queueing **creates** a failure mode that does not exist today — `email.worker.ts:92` pauses the WHOLE queue and throws on budget exhaustion, so naively queued magic links and reference codes would be stopped by an exhausted *marketing* budget (AC4 exempts `critical`); (iii) the produce-side dedup key (300s TTL, skipped for `critical`) cannot cover a retry that lands at 10 min, so the **whole guard block** moves into the worker and the send-once marker is named as the only load-bearing mechanism. 9 ACs / 8 Tasks. Depends on 13-46 (`in-progress`).
  - 🔻 **SEQUENCING RULING 3 (John/PM, 2026-08-21) — 13-64 rides IMMEDIATELY BEFORE 12-6, not to general backlog.** It does **not** gate paid spend and must not compete with 13-63 for the pre-jingle window. **Why not backlog:** 12-6 Data Health exists to RENDER the `deriveDataStatus` taxonomy, so shipping it over six knowingly mis-bucketed rows publishes the exact defect that story is built to cure — and the fix is one constant plus one predicate in the existing 9-26 Part B script, not new machinery. ⚠️ **State the impact accurately in any close-out:** the six do NOT move a published number today (13-46 AC5 put the *verification* axis on `/insights`, not the data-status axis). What 13-46 changed is the STAKES — the same per-person pass now feeds an unauthenticated, government-facing page. A missing marker is not a neutral absence: `no_submission` positively asserts the person never completed a questionnaire, when the pre-9-26 handler in fact dropped their answers. ⚠️ **The obvious fix is wrong, and 13-64 AC2 says so:** the cohort predicate does not check whether a respondent HAS answers, so simply widening the window to `--until 2026-05-21` would stamp `questionnaire_data_lost` on people whose data is intact — published raw as `Yes` by the export, while `deriveDataStatus` HIDES it (`hasSubmissionData` outranks the marker). A wrong marker is worse than the missing one this story exists to fix.

- **Field-integrity wave (13-66 … 13-69) — INDEXED 2026-09-17 (13-69/Amelia), and the lapse is the finding for the THIRD time.** ⚠️ **Measured before writing this bullet: FOUR stories were on `sprint-status.yaml` and absent from this index** — 13-66 cohort guard that sees one route (`ready-for-dev`, 2026-08-22), 13-67 association identity as structured data (`done`, closed on prod 2026-09-08), 13-68 login rate limiting by person not by proxy (`review`, 2026-09-16), and 13-69 below. Only 13-69 is described here; the other three are NAMED WITH THEIR STATUS rather than silently back-filled, because a piecemeal patch by whoever happens to notice is what produced the 2026-08-08 eight-day lapse and the 2026-08-21 one after it. A real sweep of the three is owed ([[feedback_planning_artifact_parity_sweep]]). · **13-69 the fraud engine is dark** (`review`, authored + built 2026-09-17) — fraud detection had scored NOTHING since 2026-09-05 and four of its five detectors had NEVER fired. Root cause was a chain of three defensible decisions, not a bug: 13-34 correctly removed the geopoint question from the PUBLIC form (conversion tax), neither published form then carried one, and `submission-processing.service.ts` gated the whole enqueue on `if (args.gps)` — so no GPS meant no job for **any** component, on either channel. ⭐ The correspondence is exact and is what makes it a diagnosis: **3** submissions carry GPS and exactly **3** were ever scored, out of **8,282** detections in which `gps`/`timing`/`straightline` had never been non-zero and `speed` had fired once. The fix is the ungate. **13-27 AC4 (the 2026-07-12 product ruling that the skip was correct) is SUPERSEDED** — it was right that the GPS engine is meaningless for an anonymous public registration, and did not reach the fact that the gate was not public-specific and that four detectors never needed coordinates. ⚠️ Honest coverage, measured not claimed: ungating buys the ENUMERATOR channel all four non-GPS detectors and the PUBLIC channel only timing + speed (Public Core has no ≥5-`select_one` battery for straight-lining; the wizard writes `enumerator_id` AND `submitter_id` null so duplicate has no history to key on). Thresholds stay untouched — R-A8 tunes what the detectors fire at, and tuning one that never ran would be tuning against nothing. ⛔ **SEQUENCING: `13-69 → back-score → R-A8`.** Back-scoring the unscored 2026-09-05→deploy window (13-69 residual R1) is R-A8's **PRECONDITION, not a nicety**: that window is the trial enumerator cohort's own traffic and the only field evidence this project has, so calibrating before it is scored tunes against the held import rows plus a hole exactly where field behaviour lives. The gap GROWS daily until the ungate deploys and the back-score runs.

- **Field-readiness wave (13-69 → 13-71 → 13-72 → 13-73) — INDEXED 2026-09-18 at authorship, not at close.** ⛔ **The order is a DEPENDENCY CHAIN, not a preference:** 13-69 (`review`, COMMITTED `e7417f9` 2026-09-19 — adjudicated; `done` awaits its two DISCHARGE-ON-DEPLOY rows) must DEPLOY before 13-72 can score anything; 13-72 PASS 1 must finish before R-A8 can calibrate; 13-72 PASS 2 is blocked on 13-73; 13-71 is independent of all three and is the one whose data loss is PERMANENT (a location never captured cannot be back-filled, unlike a fraud score). `13-69-fraud-engine-is-dark` (`review`, committed `e7417f9` — the fraud engine was dark for the life of the project; ungated, and the honest coverage is 3 detectors for enumerators, 2 for public/clerk) and `13-71-enumerator-gps-auto-capture-and-required` (ready-for-dev — 4 of 36 trial submissions carry a location; auto-capture + a code-enforced requirement). ⚠️ Indexed on the DAY each was authored, because this index has now lapsed twice and been caught twice by the story that happened to notice [[feedback_planning_artifact_parity_sweep]].

- **Limiter-hygiene debt closure (13-70) — INDEXED 2026-09-20, AT DEVELOPMENT, and the lapse is this story's own subject.** ⚠️ `13-70-limiter-hygiene-and-dated-residuals` existed as a brief (2026-09-17) and a `sprint-status.yaml` row (2026-09-19) with **no entry here at all** — which is the exact failure it was written to stop: the residual sweep after 13-68 found SIX rows open and **FIVE handed to stories nobody had written**. Indexed here as Task 6.3 of its own development, not back-filled later by whoever noticed [[feedback_planning_artifact_parity_sweep]]. ⚠️ **TIER: debt closure, NOT field-blocking** — do not sequence it ahead of 13-71 or the nine locked-out enumerators. **THREE INCIDENTS, ONE HABIT:** registration's per-IP limiter was fixed 2026-08-07 and its siblings were "handed to a sweep"; activation carried the identical defect for five more weeks, was fixed 2026-09-15, and its siblings were handed to a sweep again; password-reset surfaced the next day. **Scope:** FR1 *a refusal is not a request* — `loginIpFloodLimit` counted the 429s its own downstream limiters emitted, so one person spending 5 real attempts plus 95 of their own refusals reached the 100 ceiling and refused **their whole proxy** for 15 minutes (every incident in this family was a legitimate person, never an attacker); FR2 disjoint Redis keyspaces; FR3 hash the email keys through the ONE shipped builder; FR4 make a `DATED` deferral fail the build once its date passes; FR5 close R2a (~480 guesses/day accepted) and R8 (disclosure boundary is response-side) BY DECISION. ⭐ **Two of the four findings were bigger than the brief said, and enumerating is what found them:** the brief named TWO proper-prefix pairs, the tree had **FIVE** (and two of them were LIVE collisions, not latent — a 20-per-token budget and a 300-per-IP ceiling sharing one counter); and the guard's id pattern matched `R<digits>` only, so 13-68's own `A1`/`A2`/`A3` rows — **including A1, the residual FR1 closes** — were invisible to it. ⭐ **AND BOTH FINDINGS WERE THEN CLOSED AS CLASSES, not as instances** (Awwal, 2026-09-20): every rate-limit keyspace now lives in one registry that asserts disjointness AT MODULE LOAD — a colliding prefix stops the API booting, and a test fails if an `'rl:…'` literal appears anywhere outside it; and the residual guard now decides what a residual row IS from its SECTION rather than its id letter, so a ledger written with `D1`/`A1` ids is seen while the review-finding tables that share the shape are not (measured: 0 new hits, 0 lost, 0 verdicts changed). The third finding, AC1's remaining leg, was ruled and fixed the same day: **every limiter on the login routes stamps its own name when it refuses, and no limiter counts a request a different limiter refused** — which also reaches `mfaRateLimit`, mounted behind all three login limiters where mount-order reasoning could not go. ⭐ **And R1 — the flood ceiling's headroom — was decided HERE rather than handed back to 13-68**, because that story is closing and parking a live question in a closing story is the habit this one ends. `max: 100` ACCEPTED (Awwal, 2026-09-20): until FR1 **one person** could reach 100 alone — five real attempts plus 95 of their own refusals — and shut their whole proxy for fifteen minutes; it now takes ~17 simultaneous people against a measured concurrency of three. The harmonise-to-300 question was already ruled by the PRD (2026-09-17) and already carries a deadline, so it became **R5**, `DATED 2026-10-15`, alongside **R6** — the cost FR1 itself introduced, found by checking that ruling's justification instead of assuming it: the ceiling now bounds bcrypt BETTER and the hCaptcha `siteverify` call WORSE (measured: 40 captcha-passing refused requests cost 40 siteverify calls while the counter peaked at 6). 13-68 is thereby emptied to a single correctly-open row. ⚠️ **Expect FR4 to red real work on 2026-10-16** — verified against the real corpus, it names THREE rows that day (13-68 R2b, 13-70 R5, 13-70 R6) and none on 2026-10-15. That is the deliverable, not a defect.
...
| 2 | Story 11-1 schema + Akintola-risk composite indexes (AC#11) merged | 11-1 | ✅ Done 2026-05-03 |
...
| **9-63** | **Notification Usage Observability & Send-Credential Isolation** (counted email/SMS chokepoint so EmailBudgetService sees 100% of sends; per-category usage dashboard; Telegram daily-usage + abuse alerts; non-prod must NOT use the prod Resend key — fail-closed) — ops/security hygiene, not a gate | done (all 9 ACs deployed 2026-06-22 `86c33a3`: credential isolation + counted chokepoint + budget + usage dashboard + Telegram digest/abuse + recipient hygiene + SMS meter) | Emergent 2026-06-21 (Resend quota-exhaustion investigation) |
...
Split BullMQ workers into a **second PM2 process** (`oslsr-worker`) behind a `PROCESS_ROLE` env flag (default `all` = today's single-process behavior, backward-compatible) so a CPU-heavy background job stops degrading live API latency and the API/worker tiers scale independently — verified 2026-07-22 that all 10 workers currently run IN the API process (`app.ts:103`; one shared event loop; BullMQ here = decouple/retry/cron, NOT CPU isolation). Paired second lever (volume-gated): move respondent import to an async `respondent-import` BullMQ job (NOT the taken `staff-import`). **Emergent** from an 11-2 architecture review; 11-2's in-request bounding stays correct at current volume — this is the next rung, not a fix. **Depends:** 11-2 (done), Redis. **Status:** backlog (POST-LAUNCH, NON-GATING, trigger-deferred — pick up when a job class raises p95 API latency / imports outgrow in-request / api-vs-worker need independent scaling). **SM+PM reviewed 2026-07-22:** approved for backlog; at promotion, split the async-import half into its own story (11-9) + sweep the 11-3 admin-UI `/confirm` consumer. Story file `11-8-worker-process-isolation-and-async-import.md`.
