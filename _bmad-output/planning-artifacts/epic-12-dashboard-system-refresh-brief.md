...
- **12-8 — Export data-health preview.** Pre-download preview/summary on the Export page; the wire-in point for 9-59's unified mode. **Also absorbs the resolved-by-design 9-59 finding:** surface `data_status` + `reference_code` on the lightweight/Summary export so the legibility columns don't require a published form (the 9-59 unified mode requires a form for answer columns — correct by design; the form-free legibility export belongs here). Consumes 9-59's `deriveDataStatus` atom — no new taxonomy.
