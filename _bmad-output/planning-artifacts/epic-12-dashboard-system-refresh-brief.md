...
- **12-8 — Export data-health preview.** Pre-download preview/summary on the Export page; the wire-in point for 9-59's unified mode. **Resolves the deferred 9-59 no-published-form finding (Option B, Bob/SM 2026-06-16):** the preview detects the zero-published-form state and **signposts Summary mode** (the existing form-free identity export) instead of adding a throwaway identity-only unified path — consistent with the 9-59 by-design closure. Consumes 9-59's `deriveDataStatus` atom + 12-4's totals — no new taxonomy.
