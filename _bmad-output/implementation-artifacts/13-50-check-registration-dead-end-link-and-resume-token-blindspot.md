...
|---|---|---|---|---|---|
| **R1** | AC3.1 — the dead-end stock must be **0 and stay 0** across a day of jingle traffic | **✅ DISCHARGED 2026-09-06** — second reading, 4 days and 6 jingle sessions after deploy `f881577` (2026-09-02 14:10 UTC). Strict **0**, wider-by-email **0**, `wizard_resume` minted since deploy **0**. Controls prove the predicate could have matched: 172 adopted respondents, 159 with an email in the IN-list, 163 `wizard_resume` tokens minted historically. ⭐ **AND THE ENDPOINT WAS EXERCISED** — the first reading could not separate "13-50 stopped the minting" from "nothing was tried"; this one can. Post-deploy traffic: 12 magic-link tokens, 7 public/enumerator registrations, and one confirmed `/check-registration` call at 2026-09-02 16:36:57 UTC that minted **`login`**, not `wizard_resume`. That single row is the whole discharge. | `docs/runbooks/13-50-check-registration-verification.md` §R1 (both the strict and the wider by-email form) | Operator, post-deploy | Either query > 0. A non-zero on the WIDER form with a zero on the strict one is a **different** defect — a producer minting resume links without binding a `respondent_id`; find the producer, do not just expire the stock |
| **R2** | AC2 — the branch is executing **on prod**, not merely in tests | **✅ DISCHARGED 2026-09-06** — `branch_not_firing` = **0**, and not vacuously: all 12 post-deploy `magic_link.issued` rows carry `respondentId`, so the join in that query resolves. Post-deploy mints break down `login/public_magic_link_request` 10, `login/check_registration_status` 1, `pending_nin_complete/reminder_worker` 1 — the AC2 picture on one screen, which is what the AC asked for. ⚠️ **A SCARE WORTH RECORDING:** 12 `wizard_resume` tokens were minted 08-24 → 08-28 with **zero** audit rows, which looked like R2 failing. They all PREDATE the 09-02 deploy — they are the blind spot itself, caught in the wild, not evidence against the fix. **Verified structurally rather than waiting for a post-deploy `wizard_resume` mint** (there have been none): the audit row is written INSIDE `issueToken` unconditionally for every caller and purpose, and `insert(magicLinkTokens)` appears at exactly ONE site in the codebase — inside that same primitive. So "a mint that is not audited" is not expressible, and `magic-link.audit.test.ts` (33 tests) pins it. | §R2 `branch_not_firing` query: rows where `trigger='check_registration_status'` AND `purpose='wizard_resume'` | Operator, post-deploy | Any row. That combination means AC1 shipped and never fired ([[pattern-ship-a-fix-that-never-fires]]) |
| **R3** | AC3.2 — the reopen trigger's **baseline is unknowable before deploy** | **✅ DISCHARGED 2026-09-06** — adopted-cohort hits **0**, and this zero is REAL where the pre-deploy one was not. The control is the point: `registration.nin_duplicate_blocked` now has **2 rows** in the whole population, both since deploy, and it is the only `registration.*` action in the audit vocabulary. So the recorder demonstrably fires; the adopted slice is genuinely empty rather than unobserved. The two hits are the general population — already-registered people correctly blocked WITH an audit row, which is the instrumentation working. | §R3. `registration.nin_duplicate_blocked` did not exist until this story; the story's "currently 0" came from a table that could not hold the row | Operator, post-deploy | Any row joined to `metadata->>'adopted_by'='13-49'`. ⛔ Do NOT quote the pre-13-50 zero as a baseline — the true baseline starts at deploy |
| **R4** | AC4.2 — the phantom draft rows are not deleted | **✅ RULED + RE-MEASURED 2026-09-06 — LEAVE THEM.** The ruling (LEAVE, no prod deletion) was already recorded below and stands; the ledger state simply never caught up with it. ⚠️ **The count is now FIVE, not four** — `gregtop4christ@gmail.co` appeared **2026-08-28**, and they expire on **two** dates (2026-11-26 and 2026-12-28), not the single date the ruling text stated. That 5th is PRE-deploy (13-50 shipped 09-02), so it does not falsify "the producer is closed" — and post-deploy the claim holds so far: **4 drafts created since deploy, 0 new phantoms**. ⚠️ Thin sample; 40 drafts in the last 30 days, so re-check after the next blast. All five are `@gmail.co` → `@gmail.com` mid-typing, all still at `current_step` 2, and all self-remove at expiry. | `SELECT a.email AS phantom, b.email AS real_address FROM wizard_drafts a JOIN wizard_drafts b ON b.email LIKE a.email \|\| '%' AND b.email <> a.email;` | Awwal | Deletion is a prod row deletion against the stated keep-forever draft-retention ruling, so it is not mine to make. The **producer is closed** (AC4) and the **sweep excludes them** from every draft-derived cohort (AC5), so they are operationally inert either way |
| **R5** | `nin_unavailable` still receives `wizard_resume` from this surface | **✅ ACCEPTED AND CLOSED — Awwal, 2026-09-06.** Closed as an accepted risk, not as a fixed defect: the behaviour is unchanged and deliberate. The 9-12 ladder refuses that status (`allowedStatuses: ['pending_nin_capture']`) and no other surface exists for it today, so routing it anywhere else would be routing it somewhere WRONG. ⚠️ **The reopen trigger survives closure and is the point of this row:** reopen if a `nin_unavailable` person reports a dead end, or when 9-40 gains a self-serve NIN path — at which point there IS somewhere correct to send them. Pinned by a test (`statusLinkPurposeFor('nin_unavailable') === 'wizard_resume'`), so a silent change reddens CI. | `statusLinkPurposeFor('nin_unavailable') === 'wizard_resume'`, pinned by a test | — | The 9-12 ladder refuses that status (`allowedStatuses: ['pending_nin_capture']`) and no other surface exists for it today, so this story left it UNCHANGED rather than routing it somewhere wrong. Reopen if a `nin_unavailable` person reports a dead end, or when 9-40 gains a self-serve NIN path |
| **R6** | The `login` magic link carries a **15-minute TTL** | **✅ ACCEPTED AND CLOSED — Awwal, 2026-09-06.** The most user-facing of the three, and closed with that stated rather than glossed: a status email opened 16 minutes after it lands shows "This link has expired — request a new one". Honest and graceful, but a wasted round trip for a real person. NOT changed because `TTL_MS_BY_PURPOSE.login` is a **security constant shared with `/login`** — lengthening it to save a round trip widens the window on every staff sign-in too, which is a worse trade than the inconvenience it fixes. ⚠️ **Reopen** if status-email recipients report expiry at volume; the fix then is a SEPARATE TTL for the status-email purpose, never a longer shared `login` TTL. | `TTL_MS_BY_PURPOSE.login` in `magic-link.service.ts` | — | Accepted risk, closed with its reopen trigger intact. |
| **R7** | The 26 other `targetResource: 'users'` (plural) sites inherited from 13-51 | **✅ ACCEPTED AND CLOSED — Awwal, 2026-09-06.** Not this story's to fix and never was: 13-50 added `MAGIC_LINK_TOKEN` singular-canonical alongside them and touched none of the 26. Historical `magic_link_tokens` (plural) rows stay exactly as written because **`audit_logs` is append-only by trigger** — rewriting them is not available even if it were desirable. ⚠️ **Reopen as its own story** if the plural sites start breaking an audit query; the canonical-singular convention is recorded in the 13-51 census comment in `AUDIT_TARGETS`. | The 13-51 census comment in `AUDIT_TARGETS` | — | Untouched by this story; `MAGIC_LINK_TOKEN` was added singular-canonical alongside them. Historical `magic_link_tokens` (plural) rows stay as written — `audit_logs` is append-only |
| **R8** | C1 — a completed registrant whose address carries a **non-`public_user`** account now gets a LINKLESS status email. Safe, but they still cannot reach their record self-serve | **✅ CLOSED as MEASURED-ZERO, re-measured 2026-09-06.** `r8_non_public_user` = **0**, and NOT vacuously: the intersecting `public_user` population grew **127 → 176** as the registry went 400 → 8,662, so the join is live and finding people — it simply finds no staff/suspended account behind a registrant's address. `registration_status.account_not_redeemable` events: **0**. C1 stays regardless of population: it asks the only correct question (*will `loginByMagicLinkToken` accept this account for this address*) and costs nothing at zero volume. **Reopen** on any `account_not_redeemable` with `reason=not_public_user`. | `SELECT count(*) FROM respondents r JOIN submissions s ON s.respondent_id=r.id JOIN users u ON lower(u.email)=lower(s.raw_data->>'email') JOIN roles ro ON ro.id=u.role_id WHERE r.status IN ('active','imported_unverified') AND ro.name <> 'public_user';` — plus `registration_status.account_not_redeemable` in the logs, which names the reason (`locked` / `suspended` / `not_public_user`) | Operator, post-deploy | Any volume of `registration_status.account_not_redeemable` with `reason=not_public_user`. The pre-C1 behaviour was WORSE (a burnt token and "Let's get you registered first"), so this is strictly an improvement — but if the count is non-trivial these people need a real destination, which is 9-40 work, not this story's |
| **R9** | AC5's prefix rule can in principle drop a LEGITIMATE address whose longer sibling exists — `x@firm.com` is a strict prefix of `x@firm.com.ng` | **✅ CLOSED as ACCEPTED-and-measured, re-measured 2026-09-06.** The TLD-aware query returns **0 rows** across **310 live drafts** (was 284 — a larger corpus, same answer). All 5 prefix pairs are `@gmail.co` → `@gmail.com`, i.e. genuine mid-typing, not a real address with a longer sibling. No TLD refinement needed. **Re-run before each blast** — the corpus grows and the sweep prints every exclusion (AC5.2), so this fails loudly rather than silently. | `SELECT a.email AS would_be_dropped, b.email AS proving_address FROM wizard_drafts a JOIN wizard_drafts b ON b.email LIKE a.email \|\| '%' AND b.email <> a.email WHERE split_part(a.email,'.',array_length(string_to_array(a.email,'.'),1)) NOT IN ('c','co','cm','om','con','come');` — anything this returns is a candidate false positive rather than a phantom | Operator, next pre-blast dry-run | A row whose `would_be_dropped` address is a plausible real address (a ≥2-char TLD that is not a known typo). The sweep **logs and prints every exclusion** (AC5.2), so this fails loudly rather than silently — read the dry-run's `🫥` block before firing |
...
|---|---|---|
| **R4** | ✅ **LEAVE the 4 phantom drafts.** No prod deletion. | All five (⚠️ FOUR when this was written — a 5th, `gregtop4christ@gmail.co`, arrived 2026-08-28, still pre-deploy) expire on **two** dates, 2026-11-26 and 2026-12-28, created 2026-05-19 → 2026-08-28, all still at `current_step` 2. Deleting rows against a keep-forever retention ruling, to remove something that removes itself, is not a trade worth making. They are inert (producer closed by AC4, sweep excludes them by AC5) and are the only real specimens of the pattern — useful for validating the sweep at the next dry-run. |
