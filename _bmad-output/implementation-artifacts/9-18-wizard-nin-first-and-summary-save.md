...
Status: review — code complete + deployed 2026-06-11; reconciled 2026-06-15: Task 5.5 (name-canonicalization backfill) RUN on prod 2026-06-11, Task 5.8 MOOT. **SOLE remaining gate: AC#E9** — Step-4 stall ratio drops to <30% measured over 7 days post-deploy via the 9-19 dashboard; window closes **2026-06-18** (then flip → done if met).
...
- [x] **Task 5: Given-name / Family-name split + backfill (Part F — AC: #F1, #F2, #F3, #F4, #F5)** — write-path (5.1-5.3) done 2026-06-10; backfill + downstream (5.4-5.8) done 2026-06-11. Operator-gated 5.5 (run backfill) + visual 5.8 (ID-card spot-check) remain for the operator.
  ...
  - [x] 5.4: Author `apps/api/scripts/_backfill-name-canonicalization.ts` per AC#F5 ✅ 2026-06-11 — operator-gated, audit-logged (`OPERATOR_RESPONDENT_NAME_CANONICALIZED`, count 45→46), --dry-run/--apply(preview)/--confirm-i-am-not-dry-running, KNOWN_FLAGS, --help. **REWORKED 2026-06-11 to a SURNAME-DESIGNATION model** (operator fills one `surname` column; family=surname, given=name minus surname) — supersedes swap/keep, which couldn't express 3-token names + produced inconsistent decisions (Last-Name vs Surname confusion). `.xlsx` via exceljs (frozen header + amber flag on no-surname/unmatched). `--apply` w/o `--confirm` = no-write PREVIEW. `dbMatchesSnapshot` live-row guard preserved. Covers ALL rows with both names (no phone-only skip — respondents has no email column). 24 unit tests.
  - [x] 5.5: Run the backfill. ✅ **RUN ON PROD 2026-06-11** (139 rows → 128 canonicalized / 11 no-change / 0 unmatched; 128 `OPERATOR_RESPONDENT_NAME_CANONICALIZED` audit rows; 0 drift / 0 fail). **Runbook filed: `docs/runbooks/name-canonicalization-backfill.md`.** ⛔ POST-DEPLOY ONLY — the script + `exceljs` aren't on `main`/the VPS yet and prod Postgres is localhost-only, so it runs on the VPS after 9-18 merges + deploys. Reviewed file verified 2026-06-11 (139 rows → 128 change / 11 no-change / 0 unmatched). Preview (`--apply --file`) → write (`--apply --confirm-i-am-not-dry-running --file`).
  ...
  - [x] 5.8: ~~Visual-diff ID card pre-vs-post-backfill~~ — **MOOT (verified 2026-06-11).** AC#F4 assumed the name backfill touches the ID card, but ID cards are generated for **staff/`users`** (`id-card.service.ts` ← `user.controller.ts`: `user.fullName` / `role` / `staffId` / `verify-staff/…`), NOT public `respondents`. The backfill operates on `respondents`, which have **no ID-card path** at all. So there is nothing to spot-check and AC#F4's premise doesn't apply to the backfilled population. (No code change; recorded as moot rather than an open operator task.)
...
### Review Follow-ups (AI) — Part D e2e specs code-review (2026-06-11, Opus 4.8)

Part D (e2e refresh / Task 6.5 / AC#D5+E8) reviewed. The specs are clean — **every `data-testid` was verified to resolve to a real component** (active + skipped), NINs are correct (`61961438053` Modulus-11 valid, `12345678910` invalid), the flow mapping matches the redesign (NIN@Step1, given/family, Review-and-Save, section walk), and the completion-copy assertions match the actual `CompletionScreen` (active "one-click link" + pending "remind you in 2 days, 7 days, and 14 days"). eslint clean; `playwright test --list` → 15 tests. Active/skipped split is sound (active touches only no-form surfaces; full-stack flows `test.skip()` with correct preconditions). 1 finding + 1 observation, both addressed.

- [x] **[AI-Review][Low] Active `/register mounts…` test coupled to "CI has no pinned form"** [e2e/wizard-registration.spec.ts]. It asserted `wizard-step-0/1/2`, which render only in the **breadcrumb** indicator variant (≤6 steps). Seeding the multi-section form in CI — exactly the step that un-skips the full-stack flows — pushes the wizard to 11 steps → **compact** indicator → no `wizard-step-N`, silently breaking this active test. **Fix:** dropped the breadcrumb-only assertions; the indicator container + Step-1 content (already asserted) is the mode-agnostic "mounted at Step 1" check. Comment documents why.
- [x] **[AI-Review][Obs] `walkSectionsToReview` fills `'n/a'` into every field** — won't satisfy the real form's integer/select_one/geopoint/required questions, so the 40-iteration cap would exit before Review once un-skipped. It's gated behind the form seed (can't fail CI now). **Documented a CAVEAT in the helper's JSDoc** for the un-skip checklist (give it per-question-type handling).
...
| 2026-06-11 | **e2e specs (Task 6.5 / AC#D5 + E8)** — rewrote `wizard-registration.spec.ts` + `nin-validation.spec.ts` for the 9-18 flow (NIN@Step1, given/family, Review-and-Save, section-as-step). Active smoke: NIN-first Step 1 + given/family fields + client-side Continue gate + Modulus-11 reject/accept + pending-toggle. Skipped (need pinned multi-section form + email sink): happy-path, pending-NIN, **section walk + empty-section auto-skip (E8)**, duplicate-NIN block, existing-login. `playwright test --list` → 15 tests collect; eslint clean. Browsers not bootable in this dev env → specs run on CI full-stack. |
| 2026-06-11 | **Part F backfill REWORK — surname-designation model** (operator review surfaced that swap/keep couldn't handle 3-token names + was producing inconsistent decisions). Dropped swap/keep + the surname heuristic; the operator now fills ONE `surname` column and the script derives family=surname, given=name-minus-surname (handles surname-first + surname-as-last-token uniformly). Added a no-write `--apply` PREVIEW. `dbMatchesSnapshot` live-row guard preserved. exceljs retained for frozen-header + amber flagging of no-surname/unmatched rows (the dropdown rationale is moot; dep now justified by formatting). Test rewritten (24 tests). Regenerated the operator's real adjudication file preserving their 139 surnames → 128 change / 11 no-change / 0 unmatched (vs the swap/keep file's 59 marks — it was under-fixing ~69). api lint ✓, targeted suites green. Awaiting code-review + commit. |
...
| 2026-06-11 | Adversarial code-review on Part D e2e specs (Task 6.5 / AC#D5+E8): clean — every `data-testid` verified against real components, NINs checksum-correct, flow mapping matches the redesign, completion copy matches `CompletionScreen`; eslint + `playwright --list` (15) green. 1 Low (fixed) + 1 observation (documented). **Low**: the active `/register` test asserted `wizard-step-0/1/2` (breadcrumb-only) → would silently break the moment a multi-section form is seeded in CI to un-skip the full-stack flows (>6 steps → compact indicator) → dropped those assertions for the mode-agnostic container + Step-1 content. **Obs**: `walkSectionsToReview` fills `'n/a'` everywhere (won't satisfy real integer/select/geopoint/required questions) → CAVEAT documented in the helper JSDoc for the un-skip checklist. No runtime code touched (specs only). |
