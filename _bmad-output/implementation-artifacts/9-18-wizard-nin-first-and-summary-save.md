...
- [x] **Task 5: Given-name / Family-name split + backfill (Part F — AC: #F1, #F2, #F3, #F4, #F5)** — write-path (5.1-5.3) done 2026-06-10; backfill + downstream (5.4-5.8) done 2026-06-11. Operator-gated 5.5 (run backfill) + visual 5.8 (ID-card spot-check) remain for the operator.
  ...
  - [ ] 5.5: Operator runbook — Awwal runs `--dry-run` → fill the `surname` column in the `.xlsx` → `--apply --file <reviewed.xlsx>` (preview) → `--apply --confirm-i-am-not-dry-running --file <reviewed.xlsx>` (write). **OPERATOR-GATED (deferred to you).** Procedure in Dev Agent Record. (Real prod adjudication file already generated: 139 rows → 128 change / 11 no-change / 0 unmatched.)
