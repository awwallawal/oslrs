...
- **Code-review pass (2026-07-20 → 21):** re-ran `tsc --noEmit` = 0 after all
  fixes incl. the M1/M3 hardening; DB-free suites green locally = **66 tests**
  (ingest-plan 13, pdf-layout 8, normalise-row 5, csv 5 incl. row-cap, xlsx 4
  incl. row-cap, pdf round-trip + title-row + active-deadline + multi-page
  ITF-format 4, parse-limits 6, imports.routes 10 incl. CSV, imports.routes.authz
  3, buildLgaLabelResolver 7 — run separately). Integration additions (H1 prune,
  L3 invalid-status) + the DB-backed `canonicalizeLgaId` suite type-checked; run
  in CI via `db:push:full:force` against `app_test` (local box only carries the
  guard-protected `app_db`). eslint = 0 on all changed files.
...
**Added by code-review (2026-07-20 → 21):**
- `apps/api/src/routes/__tests__/imports.routes.authz.test.ts` (NEW — real super-admin gate coverage, M3)
- `apps/api/src/services/import/parse-limits.ts` (NEW — `MAX_IMPORT_ROWS`/`MAX_PDF_PAGES`/`PARSE_DEADLINE_MS`, `assertWithinLimit`/`assertBeforeDeadline`/`countLines`, typed `ParseLimitExceededError`/`ParseDeadlineExceededError`, M1)
...
- `apps/api/src/services/import.service.ts` — `pruneStaleDrafts` (H1), `withTimeout`+`deadlineAt`+`PARSE_LIMIT_EXCEEDED`/`PARSE_TIMEOUT` mapping (M1), `getFailureReportCsv` (M2), `23505`→409 in `confirm` (L2), `status` filter validation in `list` (L3), LGA resolution via shared `buildLgaLabelResolver` (L1)
- `apps/api/src/services/import/parsers/{csv,xlsx,pdf-tabular}.parser.ts` — row caps + PDF page cap + active deadline cancellation (M1)
- `apps/api/src/services/import/parsers/types.ts` — `ParserInput.deadlineAt`/`maxRows` (M1)
...
- `apps/api/src/db/schema/import-batch-drafts.ts` — corrected prune comment (H1); `apps/api/src/db/schema/lgas.ts` — corrected slug-format comment (L1)
- Tests: NEW `parse-limits.test.ts` (M1 pure); `csv`/`xlsx`/`pdf-tabular` parser tests (M1 caps + PDF active-deadline + M3 multi-page ITF-format fixture); `imports.routes.test.ts` (M2 CSV); `import.service.integration.test.ts` (H1 prune + L3); `lga-canonical.service.test.ts` (L1 resolver, 7 pure); `imports.routes.authz.test.ts` (M3 gate)

### Change Log

| Date | Change | Rationale |
|---|---|---|
| 2026-07-20 | **Adjudication (operator verify + commit + deploy) — DONE + LIVE.** Self-verified: full API 3213 green (dry-run→confirm→rollback integration exercised, 2000-row confirm ~1.3s), tsc/eslint 0, all 8 review findings confirmed fixed. Split commit `a7f0f1f` (code) + `ea75280` (Epic-11 planning cascade). **DEPLOY-BLOCKING FIX `394a8d0`:** the dev pinned `pdfjs-dist@6.1.200` (engine `>=22.13`) but CI + the VPS run **Node 20.20** → `pnpm install` failed at deploy — masked locally (my Node is 24) and the real-ITF PDF test is a **gitignored PII fixture** so CI never exercised extraction. Fixed → **pdfjs 5.6.205** (`>=20.19`, VPS-satisfied; 5.1/5.4 install on Node 20 but extract 0 rows — the fix landed between 5.4 and 5.6) + `standardFontDataUrl`/`cMapUrl` for robust text extraction. **VERIFIED LIVE:** VPS `394a8d0`/Node20, `respondents_status_check` permits `rolled_back`, `import_batch_drafts` present, 58 import tests green incl. real ITF extraction (>3000 rows). **Durable lesson:** pin deps to the CI/VPS Node (20); a newer local Node masks incompatibility; a gitignored PII fixture leaves that path CI-unvalidated — a small synthetic committed PDF fixture would have caught both. | Top defect class = works-on-my-Node + a CI-unproven feature path. |
...
| 2026-07-21 | **M1 + M3 hardened from "accepted residuals" to full resolutions (Awwal-directed — zero tolerance for deferred debt).** M1: added `import/parse-limits.ts` (deterministic row caps on all parsers, PDF page cap, and an active between-page wall-clock deadline that `destroy()`s the pdfjs task) so a parse is bounded AND actually cancellable — no more "Promise.race only stops waiting"; typed errors map to 413/408. Rejected a `worker_threads`+tsx bolt-on as a footgun in this BullMQ-isolation codebase. M3: added a full multi-page ITF-SUPA-format synthetic fixture (`buildItfFormatPdf`, fabricated data, real column mapping, title/repeated-header/footer quirks) so the riskiest parser's production format runs in CI without the gitignored PII file. Debugged a real fixture bug (overlapping long header labels merged → header mis-detected; fixed by column spacing). `tsc`=0, eslint=0, 66 DB-free tests green. **Accepted residuals now ZERO.** | Constraints are resolvable, not inherent — bound the work + model the format rather than defer. |
...
- [x] **[AI-Review][Medium→fully resolved] M1 — AC#3 parse timeout, hardened from "abandon the wait" to "bound + actively cancel the work".** Initial fix added `withTimeout()` (→ `PARSE_TIMEOUT` 408). Follow-up (per Awwal — no accepted residual): a `Promise.race` only stops *waiting*, so I added **deterministic bounds + active cancellation** in a shared `import/parse-limits.ts`: row caps on all three parsers (`MAX_IMPORT_ROWS`; CSV pre-counts lines before its un-interruptible sync parse; XLSX checks `rowCount` before the cell walk), a `MAX_PDF_PAGES` cap, and — for the slow async pdfjs path — a wall-clock deadline checked between pages that tears the pdfjs task down (`destroy()`), so the work actually STOPS. Typed `ParseLimitExceededError`/`ParseDeadlineExceededError` map to `PARSE_LIMIT_EXCEEDED` (413) / `PARSE_TIMEOUT` (408); `withTimeout` stays as an outer backstop. Combined with the 10 MB upload cap, unbounded/un-stoppable parses are eliminated without a fragile `worker_threads`+tsx bolt-on (this codebase's isolation primitive is BullMQ; in-request stays per Task 6). 9 tests (pure limits + per-parser wiring + active-deadline cancel). [`import/parse-limits.ts`; all three parsers; `import.service.ts`]
...
- [x] **[AI-Review][Medium→fully resolved] M3 — super-admin gate untested; real PDF format unproven in CI.** New `imports.routes.authz.test.ts` exercises the REAL rbac `authorize(SUPER_ADMIN)` (403 for enumerator + government-official, 200 for super-admin, service never reached when denied). For the parser: added a **full multi-page ITF-SUPA-format synthetic fixture** (per Awwal — no accepted residual) — `buildItfFormatPdf()` generates a 3-page PDF replicating the real layout (title/preamble rows, the real 6-column header repeated per page, footer noise) with FABRICATED data (committable — no PII), parsed via the **real `imported_itf_supa` column mapping**, asserting all 36 rows extract with clean email/phone/LGA/trade and no title/footer leakage. The production path is CSV/XLSX anyway (the real shortlist has redacted phones); the only thing a synthetic can't cover is an unknown quirk of the real producer's engine — not a residual, an inherent property of not having that exact file (which isn't import-viable). [`imports.routes.authz.test.ts`; `pdf-tabular.parser.test.ts`]
...
**Accepted residuals: NONE.** All 8 findings — including L1, M1, M3 — are fully resolved in code, not deferred:
- **M1** went beyond the timeout to deterministic bounds + active pdfjs cancellation (`parse-limits.ts`) — no "can't interrupt sync parse" gap remains.
- **M3** now has a full multi-page ITF-format synthetic fixture (fabricated data) running the real column mapping in CI — no "unproven format" gap remains.
- **L1** was upgraded from a band-aid to the shared robust resolver.

The only thing outside code is operational: the real unmasked ITF register (redacted-phone shortlist isn't import-viable → CSV/XLSX is the production path), which no test could substitute for and which the synthetic fixture faithfully models.
