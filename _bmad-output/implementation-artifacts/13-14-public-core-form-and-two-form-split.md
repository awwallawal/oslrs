...
**Root-cause flag (follow-up, NOT a 13-14 blocker):** the LGA UUID↔slug mismatch means the `lgaId` choice-dedup can NEVER succeed for ANY wizard-rendered form (it also affects the current FULL form while it's the pinned public form — public registrants have been asked LGA twice to date). Removing `lga_id` from the pinned Public Core resolves the public path. A proper root-cause fix (wizard `<option value={lga.code}>` + code→id resolution on submit, OR a UUID→slug resolver in the dedup) is a small code story — worth filing so a future wizard+form combo doesn't reintroduce it. Enumerators are unaffected (they fill the FULL form directly, no wizard).
