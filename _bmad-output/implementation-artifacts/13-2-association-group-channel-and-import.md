...
> ✅ **BOXES TICKED AT CLOSE-OUT, 2026-09-14 — read, not assumed.** Fifteen of these sixteen are the
> documented stale-checklist case (§2a0's known blind spot): the channel they describe is
> demonstrably live on prod — two batches confirmed, 8,278 respondents, every one carrying a
> `submissions` row and a canonical skill slug. **The sixteenth was not.** See AC5.2 below.

- [x] **Task 1 — Confirm the frozen sheet as the import contract (AC1)**
  - [x] Treat `docs/launch-campaign/association-condensed-sheet-spec.md` as FROZEN v1; do NOT re-author it. Confirm the 12-column order + header block are the column-mapping the importer implements (AC1.1) [Source: docs/launch-campaign/association-condensed-sheet-spec.md:12-37].
  - [x] Record the controlled-list validation contract: LGA → `lgas.code` (Appendix A), Trade → Appendix B (AC1.2). Note the operator pre-print inputs (Appendix B confirmation, Yoruba translation, print logistics) as PRINT-gating, not build-gating (AC1.3) [Source: docs/launch-campaign/association-condensed-sheet-spec.md:62-66].

- [x] **Task 2 — Add the `imported_association` source (AC2)**
  - [x] Add `imported_association` to `respondentSourceTypes` [Source: apps/api/src/db/schema/respondents.ts:21-27] and update the DB CHECK constraint in lockstep (parity with the 11-1 status-CHECK pattern) [Source: apps/api/src/db/schema/import-batches.ts:28-33].
  - [x] Add the association per-source config block in `import-sources.ts` (the 11-2 source registry) — the frozen 12-column mapping (AC2.2). No other schema change.

- [x] **Task 3 — Wire the association importer on the 11-2 backbone (AC3, AC4)**
  - [x] Wire the association source onto the EXISTING 11-2 `dry-run → confirm → rollback` service + endpoints [Source: docs/launch-campaign/association-condensed-sheet-spec.md:47] — reuse file-hash dedup, phone/email auto-skip, lawful-basis prompt, 14-day rollback; do NOT build a parallel pipeline (AC3.1, AC3.2).
  - [x] Map columns per the frozen spec; enforce phone-mandatory (+234 normalise), consent-Yes-only entry, trade-from-Appendix-B validation, optional NIN (AC4). Rows land `status = imported_unverified` (AC3.3) [Source: apps/api/src/db/schema/respondents.ts:9-13].

- [x] **Task 4 — Lawful basis, reconciliation, attribution-by-construction (AC5)**
  - [x] Record `ndpa_6_1_e` + per-member consent backstop; `lawful_basis_note` cites the sheet + meeting date (AC5.1) [Source: apps/api/src/db/schema/import-batches.ts:74-75].
  - [x] Surface `rows_inserted` vs the head's declared member count for reconciliation (AC5.2) ⭐ **THIS ONE WAS GENUINELY UNBUILT UNTIL 2026-09-13, and it was the debt gate that caught it.** `import_batches` had **no declared-member-count column at all** — AC5.2 never had storage, so it could not have been satisfied at any point since July. It is satisfied NOW by review proposal **P1** (`provenance_stats.declaredMembers`) plus **P2**'s `declaredGap` and the `received_exceeds_declared` / `received_below_declared` signals on the batch integrity reading. ⚠️ It was closed as a SIDE EFFECT of a review proposal, and nobody noticed it was an acceptance criterion — which is precisely why the other fifteen boxes were read rather than assumed. [Source: docs/launch-campaign/association-condensed-sheet-spec.md:20,49].
  - [x] Confirm `imported_association` is the by-construction channel (no 13-1 self-report question on imports) (AC5.3) [Source: docs/launch-campaign/attribution-spec.md:31-34].

- [x] **Task 5 — Tests (AC6)**
  - [x] Real-DB integration: valid sheet → `imported_association` / `imported_unverified` rows, consent mapped, trade validated, phone normalised (AC6.1); no-phone fail, consent=No/blank skip, phone/NIN-exists auto-skip (AC6.2); status-gate exclusion regression (AC6.3).
  - [x] Full `pnpm test` green; tsc + lint clean.
...
| Date | Change |
|------|--------|
| 2026-09-14 | ✅ **STATUS → `done`. Every residual resolved on substance, and the ledger got smaller on purpose.** **R-A2 → CLOSED** (its cell still read *"UNCOMMITTED"* / *"NOT RUN"*, both false since the runs — the §2w drift this ledger exists to catch). **R-A7 → DISCHARGED** (the whole runbook ran; every step hit its prediction). ⭐ **R-A3 / R-A5 / R-A6 / R-A10 PROMOTED OUT** to `docs/adjudication-agent-handoff.md` §3p as **standing properties of the association channel** — they are permanent facts (a public-by-design surface, a form that cannot distinguish two trades, genuinely ambiguous name order, imports belonging to no supervisor's team), and **a residual that can never be discharged does not belong in a ledger meaning "work not yet done"** — it would block `done` forever and be re-litigated at every sweep, which is the trap 13-58's R6 set. |
| 2026-09-14 | ⚖️ **R-A4 ACCEPTED — owner AWWAL, trigger: before any campaign targets this cohort.** ⭐ Re-measured rather than assumed: today's publish exposed 8,278 people on the public marketplace, but **0 of them carry `consent_enriched`**, so `revealContact` returns `not_found` for every one and no phone is obtainable through the product. Trade / LGA / association only. R-A4 stays an **operator-decision** hazard, not a product-surface one — the opposite of the natural assumption. |
| 2026-09-14 | ⚖️ **R-A8 ACCEPTED with its TRIGGER RE-CUT, because the original fired and produced nothing.** *"Read the distribution after the first fraud run"* → done: **8,278 of 8,278 `clean`, zero flags**, exactly as predicted, therefore **zero calibration information**. The consolidation stripped the duplicate evidence before import. The new trigger is the only cohort that can calibrate: **the 1,482 held rows** (1,329 flagged + 153 no usable phone), imported as their own batch and scored with `--batch-id` before anyone acts on a detection. **R-A9** given a named owner — *"whoever next wants fraud coverage"* names nobody. |
| 2026-09-14 | ✅ **The 16 task boxes ticked — READ, not assumed.** Fifteen are the documented stale-checklist case (the channel is demonstrably live: two batches, 8,278 respondents, each with a submissions row and a canonical slug). ⭐ **The sixteenth was real: AC5.2 had never been satisfiable** — `import_batches` had no declared-member-count column at all, so *"surface rows_inserted vs the declared member count"* was unbuildable from July until P1 shipped `provenance_stats.declaredMembers` + P2's `declaredGap` this week. **It was closed as a side effect of a review proposal and nobody noticed it was an AC.** That is what the §2a0 debt gate is for. |
