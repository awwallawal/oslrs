...
|---|---|---|---|---|
| ~~**R1**~~ | **RE-HOMED to 12-7 on 2026-08-11 — this row is CLOSED HERE.** Under 13-45 R3 (ruled same day) `RE-HOMED` is **not a ledger state**: a re-homed item leaves the ledger and is replaced by a pointer naming the receiving story, or it stays OPEN. It now has one. | **→ Story 12-7 (`registry-table-data-status-and-reference-code`)**, which owns the registry table this filter lives on. Deliberately NOT 12-4/12-5/12-6 — those are on the blast gate (R-F) and a critical-path story must not absorb unrelated scope. | 12-7 |
