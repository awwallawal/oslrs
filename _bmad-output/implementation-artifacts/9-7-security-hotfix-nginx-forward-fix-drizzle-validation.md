...
- [ ] **Task 4: Post-deploy verification** (AC: #1, #2, #9) — **4.1-4.3 complete, 4.4 pending browser smoke**
  ...
  - [x] 4.4 **3-of-5 browser smoke checks verified live; the final 2 deferred post-done by Awwal as residual checks.** Verified via Firefox 149 captures (Socket.IO wss upgrade, hCaptcha end-to-end POST to api.hcaptcha.com, Google OAuth library load via g_state cookie). Deferred: (a) activation wizard camera capture — the selfie flow is enumerator-only, requires provisioning a test enumerator account, and is an R6 mitigation check rather than a first-order AC verification. (b) DevTools Console full scan — belt-and-braces sweep, no specific signal to chase. If either residual surfaces a regression, open a hotfix story — the enforcing nginx config can be rolled back via the playbook's 2-minute recipe (edit `/etc/nginx/sites-available/oslsr` in place, nginx -t, reload). Ground-truth rerun procedure for both deferred checks is preserved in the checklist below for any future session that needs to execute them.
    - Enumerator camera check: super-admin Staff Management → Provision → role `Enumerator` → email-invite → walk activation wizard → DevTools Console + Network tab open → watch for `getUserMedia` prompt, camera stream render, and any CSP / Permissions-Policy violation.
    - Console scan procedure: F12 → Console tab → enable "Preserve log" → filter Errors+Warnings → walk feature matrix (home → register → login → dashboard → log out) → filter box searches: `csp`, `mixed content`, `blocked`, `CORS` → Network tab filter `Failed` for unexpected 4xx/5xx → Network filter `WS` to confirm Socket.IO upgrade shows 101. Verdict: zero new errors/warnings vs pre-deploy baseline. Browser smoke-test evidence from Awwal's Firefox 149, captured to `new_error.txt` over multiple sessions 2026-04-11 18:27 UTC: **(1) Socket.IO**: WebSocket upgrade to `wss://oyotradeministry.com.ng/socket.io/?EIO=4&transport=websocket` returned **HTTP 101 Switching Protocols** with a valid `Sec-WebSocket-Accept: eB079Ue6ehQpdYpMEhOv5p20A0M=` handshake and the **H2 review fix visible live**: `X-Proxy-Upstream: socketio` header is emitted by nginx on the upgrade response, confirming the dummy `add_header` is successfully breaking inheritance of server-level security headers on the `/socket.io/` location block. CORS correctly emits `Access-Control-Allow-Origin: https://oyotradeministry.com.ng` + `Access-Control-Allow-Credentials: true` + `Vary: Origin`. **(2) Google OAuth**: cookie jar included `g_state={"i_l":0,...}` (Google Sign-In client library state), indicating Google OAuth scripts loaded successfully and the client library initialized. **(3) hCaptcha**: captured a POST to `https://api.hcaptcha.com/checkcaptcha/9772af14-6b9c-42cc-8788-1334037aff04/...` returning **HTTP 200** via HTTP/3 + Cloudflare edge (cf-ray `9eac0b2378841d29-LHR`), with correct `Origin: https://newassets.hcaptcha.com` + same-site sec-fetch headers. Confirms the hCaptcha widget loaded, ran a challenge, posted the verification token, and got a valid response — end-to-end hCaptcha flow is unbroken by the new CSP-adjacent headers. **Still pending:** (a) activation-wizard camera capture — **this check requires provisioning a test enumerator account** because the selfie capture flow only fires for field roles (Enumerator/Supervisor/Clerk), not back-office roles. Awwal to create a test enumerator via super-admin Staff Management → Provision, email-invite himself, walk through the activation wizard, and observe whether `getUserMedia` prompts appear and the camera stream renders (R6 mitigation check for `Permissions-Policy: camera=(self)`). (b) Full DevTools Console scan with "Preserve log" enabled, filter set to Errors+Warnings, walking the feature matrix (home → register → login → dashboard → log out), searching for `csp` / `mixed content` / `blocked` / `CORS` keywords in the console filter, plus a Network-tab filter by "Failed" for unexpected 4xx/5xx. Verdict: if zero new errors/warnings appear that didn't exist pre-deploy, flip Status → `done`.
...
- AC#6 ✅ CI deploy wired with backup/test/reload (and now production-proven across TWO successive deploys: the initial 9-7 merge + the /api/v1/health drive-by redeploy)
- AC#7 ✅ drizzle upgrade runtime-validated pre-deploy
- AC#8 ✅ sprint-status.yaml traceability restored
- AC#9 ✅ 4,185 tests pass (1,808 API + 2,377 web, +29 above baseline), zero regressions
- AC#1 browser portion (Task 4.4) partial: ✅ Socket.IO + Google OAuth + hCaptcha verified, ⏳ activation camera + DevTools Console scan pending

### Drive-by `/api/v1/health` Live Verification (2026-04-11 19:05 UTC, commit `f5ed89d` + `8a91df8`)

Post-CI-deploy of the drive-by endpoint commit:

```
$ curl -s -D - https://oyotradeministry.com.ng/api/v1/health
HTTP/1.1 200 OK
Content-Type: application/json; charset=utf-8
Content-Length: 54
X-Proxy-Upstream: api                             ← H2 review fix visible live (second verification)
Access-Control-Allow-Origin: https://oyotradeministry.com.ng
Access-Control-Allow-Credentials: true
Reporting-Endpoints: csp-endpoint="/api/v1/csp-report"
(...full Helmet header set — see below)

{"status":"ok","timestamp":"2026-04-11T19:05:11.204Z"}
```

**Response time:** 946ms first hit (includes TLS handshake + cold-path), ~350ms warm. Well within uptime-monitor thresholds.

**External monitors can now use:** `https://oyotradeministry.com.ng/api/v1/health` as an unauthenticated liveness probe. DO Monitoring, UptimeRobot, and k8s readiness probes (if we ever containerize) can all ping it.

### Bonus Discovery: Full Helmet CSP IS live on `/api/*` (correcting earlier misread)

During Task 4.1, when I curled `/api/v1/health` and got 404 "Cannot GET", I observed the response header `Content-Security-Policy: default-src 'none'` and tentatively concluded that the sec2-3 hardened CSP from `apps/api/src/app.ts:103-156` was NOT live in production. **That conclusion was wrong.**

The drive-by commit added the route, and the new 200 response reveals the **full 17-directive Helmet CSP is enforcing in production** on `/api/*`:

```
Content-Security-Policy: default-src 'self';script-src 'self' https://accounts.google.com https://hcaptcha.com https://*.hcaptcha.com;style-src 'self' 'unsafe-inline' https://fonts.googleapis.com https://hcaptcha.com https://*.hcaptcha.com;img-src 'self' data: blob: https://*.tile.openstreetmap.org https://*.digitaloceanspaces.com;font-src 'self' https://fonts.gstatic.com;connect-src 'self' wss://oyotradeministry.com.ng https://accounts.google.com https://hcaptcha.com https://*.hcaptcha.com https://cdn.jsdelivr.net;frame-src https://accounts.google.com https://hcaptcha.com https://*.hcaptcha.com;worker-src 'self' blob:;media-src 'self' blob: mediastream:;object-src 'none';base-uri 'self';form-action 'self';frame-ancestors 'self';report-uri /api/v1/csp-report;report-to csp-endpoint;upgrade-insecure-requests;script-src-attr 'none'
```

**Why the earlier 404 showed `default-src 'none'` instead:** Express's default 404 handler renders HTML ("Cannot GET X"), and Helmet applies a hardened fallback CSP (`default-src 'none'`) on error responses that bypasses the user-configured directive list. This is a Helmet behavior (probably in its error-handling path), not a config gap. The practical implication: 200 responses on known routes get the full 17-directive enforcing CSP; 404/error responses get a more restrictive fallback. Both behaviors are correct.

**Why this matters for Story 9-8:** the 9-8 story already assumed "Helmet CSP is production-vetted" based on the source code at `app.ts:103-156`. This live-curl evidence **confirms** that assumption — the policy is not just defined in code, it's actively enforcing on every successful API response. Story 9-8's mirror-to-nginx work can proceed with full confidence.

**New gotcha for Story 9-8's parity test (AC#3/AC#8):** the live response includes `script-src-attr 'none'` which is an **implicit Helmet-added directive** — it's NOT in the source `directives: {...}` object at `app.ts:103-156`. Helmet adds it as a default. The csp-parity.test.ts normalization function must account for this: either (a) augment the parsed Helmet directive object with the known Helmet implicit defaults before comparing against nginx, or (b) exclude implicit directives from the comparison by matching only the user-configured ones. Option (a) is safer because it surfaces any future Helmet default-directive changes. **Logged as a known case in Story 9-8 Dev Notes.**
...
- ~~Tasks 4.1-4.4 (live post-deploy verification: curl headers, securityheaders.com grade, nmap TLS scan, browser smoke).~~ **Complete 2026-04-11 post-deploy.** Tasks 4.1-4.3 green via curl + securityheaders.com scan + curl TLS probes; Task 4.4 3-of-5 verified live (Socket.IO wss upgrade with H2 fix visible, hCaptcha end-to-end POST returning 200 via HTTP/3 Cloudflare, Google OAuth library load confirmed via g_state cookie). Residual 2 checks (enumerator activation camera + DevTools Console full scan) deferred post-done as low-risk follow-ups with rerun procedures preserved in the Task 4.4 checklist. Story status transitioned `in-progress` → `done` 2026-04-11.

**Final done-state summary:**
- Commit chain: `f3bd895` (main implementation + code review fixes) → `f5ed89d` (drive-by `/api/v1/health` liveness endpoint) → `8a91df8` (lint cleanup: removed dead `roles` import from auth.controller). All three merged to `origin/main`, all three deployed green via CI.
- 11 code review findings all resolved (2 HIGH, 4 MEDIUM, 5 LOW).
- Test suite: 4,185 total (1,808 API + 2,377 web), +29 above baseline, zero regressions, lint clean.
- securityheaders.com grade: A (CSP missing is the only red item — Story 9-8 scoped to bump to A+).
- 1 follow-up story drafted: Story 9-8 CSP nginx rollout (ready-for-dev, blocked on 9-7 done which has now flipped — 9-8 is now unblocked and can enter dev anytime).
- Operational win: external uptime monitors can now use `https://oyotradeministry.com.ng/api/v1/health` as a public unauthenticated liveness probe.
- Non-obvious discovery logged: Helmet returns `default-src 'none'` CSP fallback on HTML error responses regardless of user configuration; the full 17-directive sec2-3 CSP is only emitted on 200 responses. Documented in Dev Notes and propagated into Story 9-8's parity-test normalization plan + MEMORY.md Key Patterns for future sessions.
...
| 2026-04-11 | **Lint cleanup: removed dead `roles` import from `auth.controller.ts:23`** — pre-existing `@typescript-eslint/no-unused-vars` warning flagged by the pre-commit lint hook. Grep confirmed `roles` was genuinely unreferenced (the only other occurrences in the file are in English prose inside a docstring). 0 errors, 0 warnings on both API and web lint post-fix. 58 auth-related tests still pass. | Lint hygiene |
| 2026-04-11 | **Task 4.4 additional live verification — hCaptcha end-to-end.** Second `new_error.txt` capture showed a POST to `api.hcaptcha.com/checkcaptcha/9772af14-6b9c-42cc-8788-1334037aff04/...` returning HTTP 200 via HTTP/3 + Cloudflare edge, with correct Origin / Sec-Fetch headers. Confirms the hCaptcha widget loaded, ran a challenge, submitted the verification token, and got accepted — the new 6-header + Permissions-Policy config did not break the captcha integration. | Browser smoke (partial) |
| 2026-04-11 | **`/api/v1/health` live verification + bonus CSP discovery.** Curl against the new route (post-`f5ed89d`+`8a91df8` deploy) returned HTTP 200 + `{"status":"ok","timestamp":"..."}` in ~946ms. Response headers include the full 17-directive Helmet CSP enforcing (not Report-Only) — confirming sec2-3's CSP is live on production `/api/*`. This corrects an earlier mis-read where a 404 response showed `default-src 'none'` and led me to wrongly conclude sec2-3 hadn't deployed; Helmet actually serves a restrictive fallback CSP on HTML error responses that bypasses user config. The full-CSP behavior on 200s is the real production state. Captured as Key Patterns memory entry for future sessions. Also surfaced a `script-src-attr 'none'` implicit-Helmet-default-directive gotcha that Story 9-8's parity test must handle — logged in 9-8 Dev Notes with three handling options and a recommended fix (make it explicit in `app.ts` + allowlist backstop). | Post-deploy curl + discovery |
| 2026-04-11 | **Story 9-7 Status: `in-progress` → `done`.** All 11 ACs met (Tasks 4.1-4.3 green from curl + securityheaders.com + TLS probes; Task 4.4 3-of-5 browser smoke verified live with 2 residual checks deferred as low-risk spot-checks). 11 code review findings all resolved. 3 commits on main: `f3bd895` + `f5ed89d` + `8a91df8`. 1 follow-up story drafted and unblocked (Story 9-8 CSP nginx rollout). Sprint status synced. | Done transition |
