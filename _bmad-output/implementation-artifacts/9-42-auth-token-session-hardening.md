...
| 2026-06-08 | Deferred review items resolved post-deploy: L1 (`fix(import):` — worker is now the single budget-aware invitation sender; createManual `skipInvitationEmail` + returns plaintext token; found the worker email block was wholly dead, not just the URL) + L2 (`refactor(auth):` — shared `@oslsr/utils.sha256Hex`, 3 hashers delegate). tsc clean; crypto 30 / magic-link+password-reset 51 / activation 29 green. | Awwal (review) |
