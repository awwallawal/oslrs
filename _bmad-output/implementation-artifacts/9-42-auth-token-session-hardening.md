...
- [ ] [AI-Review][Low][DEFER→9-46] `processImportRow(): Promise<any>` masks the `import.worker.ts:69,94` double-email bug (`user.invitationToken` is `undefined` on the `{user,emailStatus}` wrapper → a second, broken `generateStaffActivationUrl(undefined)` email). **Confirmed NOT an OPS-2 regression** (undefined before & after; no hash leak) — left untouched to keep the OPS-2 commit atomic. Fix needs a design call (which path owns bulk-import email + tighten the `any` return type), already routed to 9-46. [import.worker.ts:69,94; staff.service.ts:738]
- [ ] [AI-Review][Low][DEFER→`refactor(auth):`] Third copy of the `sha256-hex` primitive (`hashInvitationToken` + password-reset's private `hashToken` + magic-link inline), each re-documenting "unsalted is correct." A shared `sha256Hex(secret)` util would centralize the rationale and prevent drift. **NOT folded into OPS-2** — would pull the already-committed F-011 (`4fee9b9`) and magic-link files into this atomic security commit; belongs in its own refactor commit. [crypto.ts; password-reset.service.ts; magic-link.service.ts]
...
- **Pre-existing hygiene find (NOT OPS-2 scope; logged for 9-46):** `import.worker.ts:69,94` reads `user.invitationToken` off `createManual`'s `{ user, emailStatus }` wrapper — that property is `undefined` (the wrapper has no `invitationToken`), so the bulk-import path already builds `generateStaffActivationUrl(undefined)` and queues a *second, broken* invitation email (createManual already queues the correct one). This is independent of hashing (undefined before and after OPS-2) — no hash leak — and was left untouched to keep the OPS-2 security commit atomic. Recommend fixing in 9-46 (thread the plaintext token through `createManual`'s return, or remove the worker's redundant queue block).
