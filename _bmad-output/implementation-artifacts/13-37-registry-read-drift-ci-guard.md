...
- [x] **Task 4 — Validate** (AC: #6)
  ...
  - [ ] **DISCHARGE-ON-PUSH:** confirm CI `lint-and-build` runs the new step green on `main`. Cannot be checked locally — dev-story does not commit or push (code-review runs on the uncommitted tree first).
...
- **Verdict:** ⏸️ **NOT CLOSED — `review`, closing on push.** Every acceptance criterion is satisfied
...
- **Deploy SHA:** ⏳ **PENDING — not yet committed or pushed.** To be filled in by the adjudication
