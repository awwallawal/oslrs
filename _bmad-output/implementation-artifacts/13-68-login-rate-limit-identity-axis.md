...
| R2a | **Escalating lockout duration** (30 min → 1 h → 2 h → 4 h, capped) | ✅ **CLOSED BY DECISION** (Awwal, Story 13-70 FR5, 2026-09-20) — we will not build it, and the number being accepted is written down: **~480 guesses/day per account**. Plain decay leaves 10 guesses per 30-minute lock window = 20/hour = ~480/day, against bcrypt at 10–12 rounds, the password policy, and MFA mandatory for super-admins. Escalation needs a lock-count column on `users`, a migration and its own security review, to add defence behind three controls that already hold. ⭐ "Won't do, and here is why" is a CLOSED residual, not an open one | Figure re-derived 2026-09-20: `AuthService` locks for 30 min after 10 failures, so 10 guesses per 30 min = 480/day. `grep -n "lock_count" apps/api/src/db/schema/users.ts` still returns nothing — that absent column is the migration this decision declines to write | Awwal (adjudication) — decided, not deferred | REOPEN TRIGGER: any weakening of the password policy, or MFA ceasing to be mandatory for super-admins. Either one removes a control this acceptance rests on, and ~480/day stops being immaterial. Also still worth watching: one `userId` producing several `account_locked` lines in a day. |
...
| R4 | Stale `googleAuthRateLimit` row in the `rate-limit-coverage.test.ts` comment table (identifier does not exist) | ✅ **CLOSED — already discharged, verified 2026-09-20 (Story 13-70).** The row was removed by the Lane C sweep on 2026-09-17; this residual simply outlived it, which is the class Story 13-70 exists to stop | `grep -rn googleAuthRateLimit apps/api/src` returns ONE line, and it is the epitaph: *"REMOVED 2026-09-17 (Lane C, R4)"* in `rate-limit-coverage.test.ts`. No identifier, no coverage row, nothing asserted against it | Lane C — done | — |
| R5 | Keyspace collision latent in the activation and reset-completion limiters (Completion Notes #3) | ✅ **CLOSED BY STORY 13-70 FR2** (2026-09-20) — and it was **LIVE, not latent**: `activationRateLimit` falls back to the key `ip:<addr>` under prefix `rl:activation:`, spelling `rl:activation:ip:<addr>` byte for byte, which is what `activationIpFloodLimit` writes. A 20-per-token budget and a 300-per-IP ceiling shared ONE counter on every tokenless request. Renamed to `rl:activation:token:` and `rl:password-reset-complete:token:`. ⭐ Enumerating the tree found THREE more pairs this row never named (`rl:login:`, `rl:register:`, `rl:wizard-draft:`); all five are closed | ⛔ **The enforcement is now structural, not a scan.** All **31** keyspaces live in `apps/api/src/lib/rate-limit-prefixes.ts`, which asserts the invariant AT MODULE LOAD — a colliding prefix throws on import, naming the pair, so the API cannot boot with one. `rate-limit-prefix-disjointness.test.ts` additionally fails if an `'rl:…'` literal appears anywhere in `apps/api/src` outside that registry, which is what makes "add it here" enforceable. ⚠️ The first version of that test scanned `middleware/*.ts` for `prefix:` and passed cleanly over 24 of 31 — it could not see the two factory-built limiters or the one in `services/` [[pattern-census-counts-sites-not-callers]] | Story 13-70 | REOPEN TRIGGER: that disjointness test being deleted, or narrowed back to a fixed list of prefixes. |
| R7 | Raw, unbounded email as a Redis key in `registrationEmailRateLimit` and `wizardDraftEmailRateLimit` (the H2 pattern, which this story copied from them) | ✅ **CLOSED BY STORY 13-70 FR3** (2026-09-20) — both now delegate to `buildLoginRateLimitKey`, the builder H2 shipped, so the key is `e:<sha256>` at a fixed 66 characters and no address sits in the shared Redis in plaintext. ONE hasher, not a third copy: deleting the digest from the shared builder reds 8 tests across all three limiters | `pnpm --filter @oslsr/api exec vitest run src/middleware/__tests__/registration-email-rate-limit-key.test.ts src/middleware/__tests__/wizard-draft-rate-limit-key.test.ts` — each asserts a megabyte-long address yields a 66-character key, and that it equals the key the login builder produces | Story 13-70 | REOPEN TRIGGER: any per-email limiter added without going through `buildLoginRateLimitKey`. |
| R8 | Account existence still revealed by the login error CODES for locked (`AUTH_ACCOUNT_LOCKED`), invited (`AUTH_ACCOUNT_NOT_ACTIVATED`) and suspended (`AUTH_ACCOUNT_SUSPENDED`) accounts, and by "use the public/staff login" after a correct password | ✅ **CLOSED BY DECISION** — pre-authorised by Story 13-70 **AC9**, which mandated this closure and named **John (PM)** as owner, consistent with this story's own §825 disposition (*"R8 to John (PM), Lane C — it is product copy"*); executed by 13-70 FR5, ratified at adjudication 2026-09-20. ⚠️ First recorded as "(Awwal, …)", which was the wrong signature on a row AC9 had already assigned — corrected at adjudication; two-part attribution per handoff §2al. **The disclosure boundary is RESPONSE-SIDE, not log-side.** The responses stay: they tell a real person what to do next, which is why an invited user stops re-typing a password that was never going to work. What the LOG says is a separate decision and it is the opposite one — `auth.login_failed` distinguishes the reasons deliberately, and that is exactly what makes R2b's cardinality monitor possible at all. The TIMING channel, the part an attacker could not have asked for, was closed by this story (L2) | `grep -n "AUTH_ACCOUNT_NOT_ACTIVATED" apps/api/src/services/auth.service.ts` shows the codes; the R2b row's own grep shows `email` on both failure branches, i.e. the log-side distinction the monitor reads | John (PM) — accepted as product copy | REOPEN TRIGGER: an enumeration pattern in `auth.login_failed` (many distinct `email`s per `ipAddress`) that the R2b monitor attributes to the error codes rather than to timing or volume. |
...
|---|---|---|---|
| **A1** | **The accidental proxy block.** `loginIpFloodLimit` is mounted first with no `skipFailedRequests`, so it counts the 429s the downstream limiters produce. One person: 5 failures (email budget spent) plus 95 of their OWN 429s = 100, and **their whole proxy is refused for 15 minutes**. About 7 req/min by hand; seconds for a buggy client retry loop. **M1 surviving one layer down.** | ✅ **CLOSED BY STORY 13-70 FR1** (2026-09-20). ⛔ **AND THIS CELL'S REASONING IS CORRECTED, not overruled.** It read *"`skipFailedRequests` would skip 401s and gut the ceiling"* — TRUE of `skipFailedRequests` with the library's DEFAULT `requestWasSuccessful` (`statusCode < 400`), under which a 401 is a "failed" request and gets decremented, disarming the ceiling on the one axis that bounds a credential spray. Option B (Awwal, adjudication 2026-09-20) pairs the flag with a MARKER predicate instead — `(_req, res) => !res.locals.rateLimitRefusedBy` — so "failed" means "refused by another limiter" and nothing else; a genuine 401 still counts. ⚠️ The flag is not optional decoration: `requestWasSuccessful` is consulted ONLY when `skipFailedRequests` or `skipSuccessfulRequests` is set (express-rate-limit 8.3.0), so the predicate alone registers no listener and never runs — the ruled mechanism without it is a fix that cannot fire | Both halves RED-VERIFIED 2026-09-20 in `login-rate-limit.binding.test.ts`, reading the ceiling's counter value rather than whether a request was served: delete the stamp and the "does not advance" leg reds `[6,7,8]` vs `[6,6,6]`; force the predicate false and **this cell's own fear reproduces exactly** — 8 tests red, the ceiling serving request 101 instead of refusing it. Drop `skipFailedRequests` and leave the predicate and the file goes green on the marker while counting exactly as before. REOPEN TRIGGER: any `auth.login_ip_flood_limit_exceeded` where one email dominates the window, or whose IP does not resolve to a proxy/CGNAT range |
| **A2** | hCaptcha **siteverify volume per IP rises from 60/hour to 400/hour** (the ceiling now sits ahead of the captcha by design — it must, to bound the call at all). Whether hCaptcha rate-limits at that volume is **unverified**. Also unmeasured: bcrypt p95 on the 2 GB VPS where BullMQ workers share the libuv threadpool. | ✅ **SUPERSEDED BY STORY 13-70 R5 + R6 (2026-09-20), which carry a DATE.** These are the same two measurements the PRD's flood-ceiling divergence ruling requires by **2026-10-15**, and they had no owner and no deadline here. ⛔ **AND ONE OF A2's OWN NUMBERS NO LONGER HOLDS:** *"400/hour"* was 100/15min read as an upper bound, and it was the figure that made the divergence defensible — but Story 13-70 FR1 stopped the ceiling counting downstream 429s, and a request the burst limiter refuses has ALREADY passed the captcha. Measured: **40 captcha-passing requests refused downstream cost 40 siteverify calls while the counter peaked at 6.** For that path the ceiling is no longer a bound at all, so the measurement A2 asked for is now more load-bearing, not less. Scoped honestly in R6: a production token is single-use, so the cost is one solved captcha per call, not free. A2's own exposure note stands — peak organic traffic is about 19 registrations/day | any hCaptcha quota or error response, or a login p95 latency regression — **now owned by 13-70 R5/R6** |
| **A3** | **Flood-ceiling inconsistency.** `activationIpFloodLimit` and `passwordResetCompletionIpFloodLimit` are both **300**/15min; `loginIpFloodLimit` is **100**/15min — same shared-proxy population, 3× tighter | ✅ **SUPERSEDED BY STORY 13-70 R5 (2026-09-20), which carries a DATE.** This row raised the right question and had no owner and no deadline, so it would have sat here until someone happened to look. The PRD's flood-ceiling divergence ruling (2026-09-17) permits the divergence *only while* the cost login's ceiling bounds is recorded, and requires hCaptcha's per-IP limit and bcrypt p95 to be measured by **2026-10-15**, harmonising to 300 if neither constrains. 13-70 R5 is that row, dated, and 13-70's own guard now fails the build on 2026-10-16 if the date passes unanswered. ⛔ Moved OUT of this story deliberately rather than left in it: parking a live question in a closing story is the habit 13-70 exists to end | **And 13-70 R6 records that the justification itself shifted:** FR1 stopped the login ceiling counting downstream 429s, so it bounds bcrypt better and `siteverify` worse — measured at 40 captcha-passing refused requests costing 40 siteverify calls while the counter peaked at 6. ⭐ 13-70 R1 separately CLOSED the headroom question by decision: `max: 100` accepted, because FR1 changed the binding constraint from one person to ~17 simultaneous people |
