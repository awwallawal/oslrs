...
|---|---|---|---|---|---|
| **R1** | ◨ **HALF CLOSED, HALF IMPOSSIBLE — run on prod 2026-08-22 by adjudication.** ✅ **`--normalise-keys` APPLIED and independently verified.** Predicted first (1 non-bare suppression of 12, 1 non-bare recipient of 302, bare key `aqeemakolade@gmail.com` unsuppressed so an UPDATE not a merge); the dry-run matched exactly; the apply reported `rewritten: 1  merged: 0  events: 1`, and a direct query confirms **0 non-bare suppressions, 0 non-bare events, total still 12** (nothing lost or duplicated) plus **1 audit row** — proof `logActionTx` fired inside the transaction rather than being lost at exit (§2j). **Citizen effect predicted and correct:** severity NULL reads SOFT and the row is past the 72 h window, so normalising made the suppression *correctly tracked* rather than *permanently excluding* — which is exactly why severity had to ship first. ⛔ **`_ops-migrate-audit-target-users-to-user.ts` CANNOT RUN — and should be RETIRED, not fixed.** See R13. | The apply output, plus `SELECT count(*) FROM email_suppressions WHERE email ~ '"'"'[<>[:space:]]'"'"'` → **0** | adjudication (done) | A non-bare key reappearing — that would mean the webhook inlet stopped canonicalising on write |
| **R14** (new, adjudication 2026-08-22) | ✅ **CLOSED HERE — HANDED OFF to Story `13-66-a-cohort-guard-that-sees-one-route`, registered `ready-for-dev`.** 13-51's severity work made all 12 suppressions retry-eligible exactly as R3 predicted — and a live blast segmentation then showed what that means downstream: `suppressed=0` in the dry-run is CORRECT, and 9 of the 12 suppressed addresses are in the cohort, 4 of them `.co` typos that will bounce forever. ⚠️ **All four typos have `.com` twins ALREADY REGISTERED**, so the remedy is suppress-and-leave, NOT the address correction this story shipped — repairing them would open a second route to a registered person. Recorded so 13-51's own correction tool is not pointed at them. Two defects found and handed on: Cat1 sees one of four contact routes (misses 112 of 185), and `filterMarketingCohort()` has no campaign-history filter (52 re-invitees). | The segmentation table in 13-66, reproducible from `_reengagement-email-blast.ts --dry-run` plus the four-route query | Awwal / 13-66 | A marketing blast firing before 13-66 is ruled on |
| **R13** (new, adjudication 2026-08-22) | ✅ **CLOSED — RETIRED ON AWWAL'S RULING 2026-08-22. The script is deleted.** ⛔ **THE AUDIT-TARGET MIGRATION IS STRUCTURALLY IMPOSSIBLE, AND THAT IS THE GUARANTEE WORKING.** `_ops-migrate-audit-target-users-to-user.ts` UPDATEs `audit_logs` to rewrite 4 historical rows from `target_resource='users'` to `'user'`. **`audit_logs` is append-only:** `trg_audit_logs_immutable` blocks UPDATE **and** DELETE. **PROVEN BY EXECUTION, not predicted** — `--apply` failed with `P0001` raised from `audit_logs_immutable()` line 3, and the rollback left all 4 rows untouched. ⭐ **The dry-run could never have caught this** (it printed a confident "PREDICTION: 4 row(s) will change") because it reads the input and never attempts the write — §2p exactly. **➜ RECOMMENDATION: delete the script rather than repair it.** Rewriting audit history to look tidy is precisely what immutability exists to prevent; those rows are a TRUE record of what the code wrote at the time. The drift belongs to the WRITERS (26 live sites still emit `'users'` — see R5) and to any reader that groups on `target_resource`, which should normalise on READ. | `SELECT target_resource, count(*) FROM audit_logs WHERE action IN ('"'"'email.suppression_lifted'"'"','"'"'user.email_corrected'"'"') GROUP BY 1` → `users|4`, unchanged after a failed `--apply` | Awwal (RULED: retire) — the remedy the project had ALREADY chosen: the audit-target convention says migration is a constant swap at each SITE with **no data backfill**, and a plural-outlier site with live rows carries an inline cutover comment instead. That comment now lives in `audit.service.ts` beside `AUDIT_TARGETS.USER`. | Anyone attempting the migration again and reading the P0001 as a permissions problem rather than a design guarantee | | Neither backfill has been RUN. `_ops-contact-remediation.ts --normalise-keys` (the wrapped suppression + event rows) and `_ops-migrate-audit-target-users-to-user.ts` (4 audit rows) are written, dry-run-by-default, predict-then-compare, with read-backs — but both are operator actions against prod and nothing is committed. ⛔ `--normalise-keys` MUST NOT run until severity is **deployed**: normalising the key is what makes that row *function*, and a functioning un-severitied row is a permanent exclusion of a live citizen. | `tsx scripts/_ops-contact-remediation.ts --normalise-keys --dry-run` and `tsx scripts/_ops-migrate-audit-target-users-to-user.ts --dry-run` — both print a prediction and write nothing | Awwal (operator) | Runs on prod → compare printed prediction to the read-back. Any mismatch = reopen. |
| **R2** | ✅ **CLOSED 2026-08-22 — BOTH `_ops-*` entry points have now been EXECUTED END TO END on prod.** That was this row's entire content, and running them is the only proof available because `apps/api/scripts/` sits outside tsconfig (Pitfall #41 — RUN scripts, don't trust tsc). `_ops-contact-remediation.ts --normalise-keys` ran dry then applied cleanly. `_ops-migrate-audit-target-users-to-user.ts` ran dry, then `--apply` **executed and failed against the append-only trigger** — which is still an end-to-end execution, and it surfaced R13, a defect no dry-run could ever have shown. ⭐ **The narrowing recorded 2026-08-20 also holds:** `resolveRespondentContactEmail` is proven on prod and pinned by a real-DB test. | Both runs are in the session transcript; re-runnable with `--dry-run` on either script | adjudication (done) | A future `_ops-*` script shipped without a prod dry-run |
| ~~**R2-orig**~~ | *(superseded — was: OPEN, BLOCKS `done`, NARROWED at adjudication 2026-08-20.)* The `_ops-*` entry points are still unrun. **What is no longer unproven is the resolver underneath them:** `resolveRespondentContactEmail` had never succeeded once — bare `ORDER BY … LIMIT` on `UNION` branches threw `42601` on every call since `9d33b94`, invisible because all three callers are scripts outside tsconfig and a mocked `db.execute` never parses SQL. It is now **executed against a real Postgres** by `respondent-contact.service.integration.test.ts` (new, 4 tests), and **RED-verified**: removing one branch's parentheses reproduces `42601 syntax error at or near "UNION"` and reds 3 of them. Also confirmed on PROD read-only — the fixed query returns a real contact by `magic_link_token` rank 2. ⚠️ Fail-safe checked: `nin-reconfirm` resolves contact at :89 and clears the NIN at :117, so it **threw BEFORE clearing** — total failure, zero harm. | The two `_ops-*` scripts have never been executed end-to-end, not even against the test DB. `apps/api/scripts/` is outside tsconfig, so per Pitfall #41 they are proven by RUNNING, not by `tsc` — and eslint is the only compile-time signal they get. The correction script's *service* path IS exercised (the integration suite drives the same service through the same transaction shape); the two `_ops-*` entry points are not. | run each with `--dry-run` against `app_test` and read the output | dev | A first prod run that errors on a path a dry run would have caught. |
