...
- 11.1 `sprint-status.yaml`: 9-9 entry remains `in-progress` until all subtasks except AC#10 (domain-gated, already done) are done. Comment refreshed per subtask completion. Done: AC#1 ✅ AC#2 ✅ AC#3 ✅ AC#5 ✅ AC#6 ✅ AC#8 ✅ AC#10 ✅. Pending: AC#4 ⏳ AC#7 ⏳ AC#9 ⏳. **7 of 10 entries refreshed (all done).** Story remains `in-progress` until AC#4 + AC#7 + AC#9 land — none are FRC-blocking; field-readiness gate is closed.
...
**Subtasks 4-5, 7, 9 (forward-planned) — expected created/modified:**
