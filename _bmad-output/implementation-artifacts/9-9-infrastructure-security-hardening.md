...
- 11.1 `sprint-status.yaml`: 9-9 entry remains `in-progress` until all subtasks except AC#10 (domain-gated, already done) are done. Comment refreshed per subtask completion. Done: AC#1 ✅ AC#2 ✅ AC#3 ✅ AC#6 ✅ AC#8 ✅ AC#10 ✅. Pending: AC#4 ⏳ AC#5 ⏳ AC#7 ⏳ AC#9 ⏳. **6 of 10 entries refreshed (all done).** Story remains `in-progress` until AC#5 (FRC #5 backup encryption) lands — AC#4/7/9 are non-FRC and parallelisable.
- 11.2 Field Readiness Certificate (epics.md): item #1 (Tailscale) already ✅ done in the FRC table. Per 2026-04-27 FRC revision, item #5 is now backup encryption (AC#5), NOT alerting (AC#6) — to be flipped when AC#5 ships, not by AC#6. **Status: no FRC table edits needed from this story until AC#5 lands.**
