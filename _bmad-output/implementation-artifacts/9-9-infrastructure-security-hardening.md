...
| 3 | Public port audit (`ss -tlnp`); close/restrict Portainer | 🟡 **Partial 2026-05-08** — audit + F2 fix shipped; F1 Portainer rebind pending operator action | `docs/port-audit-2026-05-08.md` + AC#3 below |
...
| AC#3 (Port audit) | **Partial 2026-05-08** | P1 | ~3 hours actual + operator F1 follow-up pending | — | Audit + F2 fix shipped; F1 Portainer rebind requires brief downtime, operator-scheduled |
...
### Subtask 3: Public port audit & Portainer hardening (AC#3) — PARTIAL 2026-05-08 (audit + F2 fix shipped; F1 Portainer rebind pending operator action)

- [x] 3.1 `ss -tlnp` captured + 15 listening sockets enumerated. See `docs/port-audit-2026-05-08.md` § "Listening port table".
- [x] 3.2 `iptables -L -n` + `iptables -t nat -L -n` captured. Docker DNAT chains analysed; Postgres + Redis correctly destination-locked to `127.0.0.1`, Portainer destination-permissive (F1 finding).
- [x] 3.3 Cross-reference complete. DO Cloud Firewall posture verified via external Test-NetConnection probe (operator laptop public IP → 159.89.146.93): 22/80/443 ALLOW, 3000/8000/9443 DENY. Two-gate architecture intact.
- [~] 3.4 Portainer binding identified — actual is `0.0.0.0:8000` + `0.0.0.0:9443` (story expected `0.0.0.0:9000`; 9000 is container-internal only). **Decision: Option A (rebind to 127.0.0.1).** Recipe + rollback documented in `docs/port-audit-2026-05-08.md` § F1. **Operator action required, not yet executed** — recreating the Portainer container is a brief downtime + admin-login verification that should be done at a quiet operator window, not by the workflow automatically. Until the recreate lands, Portainer is still publicly bound at the kernel level — DO Cloud Firewall is the only thing standing between the public internet and a Portainer admin UI that holds Docker socket access. AC#3 closure is gated on this. **F2 sub-finding (API process bound to `*:3000`) FIXED in this PR** via new pure-function helper `apps/api/src/lib/listen-address.ts` (8 unit tests) imported in `apps/api/src/index.ts`, plus `infra/nginx/oslsr.conf` upstream made explicit (`http://127.0.0.1:3000` instead of `http://localhost:3000`) so the host binding and the upstream resolution agree without depending on `/etc/hosts` order. F2 severity bumped LOW → MEDIUM in third-pass review (defence-in-depth on the public-facing API ingress is not "low").
- [x] 3.5 Audit document authored: `docs/port-audit-2026-05-08.md` (date-stamped today). Covers listening port table + DO firewall posture + 4 findings (F1 Portainer, F2 API, F3 stale UFW rule, F4 informational scope mismatch) + Postgres/Redis/SSH ✅ verifications.
- [x] 3.6 Runbook §1.4 updated with Listening port table + Pending changes subsection + cross-reference rule. The "Current state" snapshot reflects what's actually deployed at audit time (port 3000 still on `*`, ports 8000/9443 still on `0.0.0.0`); the "Pending changes" table separately lists what'll change after the deploy + the operator Portainer recreate. Re-audit and collapse the pending rows once both have landed.

**Audit shipped, hardening half-shipped.** F2 (the API binding) is the cheap fix and shipped in code. F1 (the Portainer rebind, the actual MEDIUM-severity gap that motivated the wider audit) is operator-scheduled. Subtask 3 is therefore PARTIAL, not DONE — AC#3 reopens to closed when the F1 recreate + UFW cleanup land. The two-gate architecture (DO Cloud Firewall on the cloud edge + UFW + 127.0.0.1 binding at the host) only fully closes when all three layers agree.
...
- 11.1 `sprint-status.yaml`: 9-9 entry remains `in-progress` until all subtasks except AC#10 (domain-gated) are done. Comment refreshed per subtask completion. Done: AC#1 ✅ AC#2 ✅ AC#6 ✅ AC#8 ✅ AC#10 ✅. Partial: AC#3 🟡 (audit + F2 done; F1 pending). Pending: AC#4 ⏳ AC#5 ⏳ AC#7 ⏳ AC#9 ⏳. **6 of 10 entries refreshed (5 done + 1 partial).**
...
- [x] [AI-Review][HIGH] **R3-H1: AC#3 was marked DONE while F1 (Portainer publicly bound) deferred to operator follow-up.** AC#3 (line 57) requires "either rebind to 127.0.0.1 OR keep public but add basic-auth via NGINX proxy" — neither has happened on prod. The audit is shipped and F2 is fixed in code, but the actual hardening (the MEDIUM-severity gap that motivated the audit) is still pending. Subtask 3 reframed PARTIAL not DONE; Status Matrix + Priority Map + sprint-status.yaml comment + Subtask 3 §header all updated; AC#3 closure deferred until F1 operator action lands. [`9-9-infrastructure-security-hardening.md:36,82,112-119,266` + `sprint-status.yaml:335`]
...
**F1 operator follow-up (NOT in this PR — requires brief Portainer downtime; AC#3 closure gated on this):**

- VPS-side: `docker stop portainer && docker rm portainer && docker run -d --name=portainer --restart=always -p 127.0.0.1:8000:8000 -p 127.0.0.1:9443:9443 -v /var/run/docker.sock:/var/run/docker.sock -v portainer_data:/data portainer/portainer-ce:latest`
- VPS-side: `ufw delete allow 9443/tcp` (F3 stale-rule cleanup)
- After F1 + UFW cleanup: refresh `docs/emergency-recovery-runbook.md` §1.4 ("Current state" rows for 8000 + 9443 → `127.0.0.1`; collapse those rows out of "Pending changes"); flip Status Matrix L36 + Priority Map AC#3 from 🟡 Partial → ✅ Done; flip sprint-status.yaml comment from "PARTIAL 1/10" → "DONE 6/10" with AC#3 close-out evidence.
