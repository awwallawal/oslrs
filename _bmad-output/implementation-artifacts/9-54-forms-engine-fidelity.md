...
- [x] **[Infra][Med] NG2 — pre-push full-suite flakiness (discovered during the 9-54 push cycle, 2026-06-14) — fixed at root cause.** The local pre-push gate failed intermittently on `auth.activation.test.ts` (real-DB integration) and `Header.test.tsx` (jsdom) — each cost a ~6-min retry. **Root cause (not the tests):** the hook ran `pnpm test` (= `turbo run test`), executing api + web + utils + testing **concurrently on one machine**, oversubscribing CPU and the Postgres connection pool → timeout/starvation. Proof it's contention, not a logic bug: each suite is green **in isolation** (`pnpm --filter @oslsr/api test` → 2452 pass; web alone → 2590 pass) **and in CI**, which runs each package as its **own job on a dedicated runner** (`ci-cd.yml` `pnpm --filter @oslsr/<pkg> test`) — so CI never had mutual contention and never flaked these. **Fix:** `.husky/pre-push` now runs `pnpm exec turbo run test --concurrency=1` — each suite gets the whole machine (the proven-green condition), making the local gate deterministic. **CI is intentionally untouched** (it never used turbo-all; serializing api there would tax every run to fix a problem CI doesn't have). No test code changed — the tests were never the defect. Trade-off: pre-push wall-time ~+1 min, but zero retries (net faster than flaky + 2-3 retries).
