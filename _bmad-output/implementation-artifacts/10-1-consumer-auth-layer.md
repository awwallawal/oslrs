...
| 2026-05-04 | **Risk #7 + Task 1.1 closed by Story 9-11 Session 2.** Story 9-11's Schema Down Payment (Session 1, 2026-05-03 — landed in `apps/api/scripts/migrate-audit-principal-dualism-init.ts`) shipped both `api_consumers` (production-shape, 8 cols matching this story's AC#1: id / name / status / organisation_type / contact_email / dsa_signed_at / dsa_pdf_url / created_at / updated_at) AND `audit_logs.consumer_id` (FK ON DELETE SET NULL + principal-exclusive CHECK). When this story (10-1) is implemented, Task 1.1 is reduced from "create api_consumers" to "extend api_consumers with `last_used_at` if not present + add api_keys + api_key_scopes". Risk #7 ("11-1 audit_logs.consumer_id missing at impl time") is closed definitively — both columns are live in production after `fc38d33` (11-1 deploy) + the 9-11 Session 1 deploy chain. Existing test factory `apps/api/src/test/factories/api-consumer.factory.ts` (created in 9-11 Session 1) is the canonical fixture for consumer-auth tests in this story. | Cross-story dependency closure: capturing the schema inheritance now so the eventual 10-1 dev agent doesn't re-create `api_consumers` and instead extends. |
