...
- **2026-04-06 (Pre-Field Security Sweep):** Full 3-layer security sweep before field deployment. Results:
  - **Dependencies:** 0 critical, 0 high. 2 known/accepted: `qs@6.14.1` (1 moderate + 1 low) via Express 4 → body-parser. No fix without Express 5 migration. All 6 pnpm overrides valid. Security-critical packages current.
  - **Secrets:** Clean. No leaked credentials in source. `.env` gitignored. CI uses GitHub Actions secrets.
  - **Configuration:** CSP enforcing in production (no unsafe-eval), CORS strict, all auth/public endpoints rate-limited, JWT 15-min access + opaque refresh in Redis, 100% Zod validation, all routes auth-gated.
  - **One finding fixed:** Activation endpoints (`GET/POST /auth/activate/:token`) had no rate limiting. Added `activationRateLimit` (10 req/15min/IP, Redis-backed) in commit `7fe1192`.
  - **`qs` accepted risk detail:** `qs` is a query string parser bundled inside Express 4. GHSA-w7fw-mjwx-w883 (moderate) allows `arrayLimit` bypass via crafted query strings, potentially causing higher memory usage. Express 4.22.1 ships `qs@6.14.1`; fix requires `qs >= 6.14.2` which Express 4 hasn't released. Risk is low: OSLRS uses JSON POST bodies (not query string arrays) for all data mutations, and rate limiting on all endpoints mitigates resource exhaustion. Only fix is Express 5 migration (future epic). Accepted.
