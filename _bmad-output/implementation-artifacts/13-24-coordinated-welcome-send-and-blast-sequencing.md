...
- [~] **Task 3 (AC5) — docs:** ✅ `docs/runbooks/pre-blast-dry-run.md` CREATED 2026-07-23 (codified from the proven 13-34 dry-run + teardown; its §1 dedupe-verify step is a MANUAL cross-check until the code lands, then enforced). ✅ `re-engagement-campaign-launch.md` STALE-SUPERSEDED banner added. ⏳ Remaining: fold the new ordered sequence INTO the campaign runbook body + counts-honesty pass + flip `backfill-operator-residuals.md:56` once the dedupe ships (these track the dedupe code).
