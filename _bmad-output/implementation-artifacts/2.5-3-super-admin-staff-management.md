...
**Status:** complete
**Created:** 2026-02-02
**Updated:** 2026-02-08 (Code Review #3 - 9 issues found: 7 auto-fixed, 2 action items)
...
- `/dashboard/super-admin/staff` → PlaceholderPage (needs StaffManagementPage)
...
### AC8: Professional Government-Grade ID Card Design
**Given** the ID card PDF generation
**When** a staff ID card is generated (via self-download or Super Admin download)
**Then** the card uses professional government branding with:

**Front Side:**
- Brand maroon header (#9C1E23) with Oyo State coat of arms logo + "OYO STATE LABOUR & SKILLS REGISTRY"
- Staff photo with thin maroon border frame
- Full name (bold), role title, LGA, phone number
- Staff ID in format `OSLSR-XXXXXXXX` (from UUID first 8 chars)
- Issue date (formatted DD/MM/YYYY)
- Bottom accent line with "PROPERTY OF OYO STATE GOVERNMENT"
- Subtle diagonal watermark pattern across card body

**Back Side:**
- Brand maroon header with "OYO STATE GOVERNMENT"
- "SCAN TO VERIFY" with centered QR code
- Return instructions: "If found, please return to: Office of the Commissioner, Ministry of Trade, Industry & Cooperatives, Secretariat Ibadan, Oyo State."
- Website URL at bottom
- Thin maroon border around card edge

**Front Side Wireframe:**
```
┌─────────────────────────────────────────────┐
│ [MAROON HEADER]                             │
│ [Logo] OYO STATE LABOUR & SKILLS REGISTRY   │
├─────────────────────────────────────────────┤
│                                             │
│  ┌──────────┐   FULL NAME (bold)            │
│  │          │   Role Title                  │
...
│                  Issued: DD/MM/YYYY          │
│                                             │
│  ─────── PROPERTY OF OYO STATE GOVERNMENT   │
└─────────────────────────────────────────────┘
```
...
│  Cooperatives, Secretariat Ibadan,          │
│  Oyo State.                                 │
│                                             │
│          oslrs.oyostate.gov.ng              │
└─────────────────────────────────────────────┘
```
...
apps/api/src/
├── routes/
│   └── staff.routes.ts                    # MODIFY - Add GET /, PATCH /:userId/role, POST /:userId/deactivate, GET /:userId/id-card
├── controllers/
│   └── staff.controller.ts                # MODIFY - Add list, updateRole, deactivate, downloadIdCard methods
├── services/
│   └── staff.service.ts                   # MODIFY - Add listUsers, updateRole, deactivateUser methods
│
apps/web/src/
├── features/
│   └── staff/                             # NEW - Entire feature module
│       ├── api/
│       │   └── staff.api.ts               # NEW - API client functions
│       ├── hooks/
│       │   └── useStaff.ts                # NEW - TanStack Query hooks
│       ├── components/
│       │   ├── StaffTable.tsx             # NEW - Data table with columns & actions
│       │   ├── StaffStatusBadge.tsx       # NEW - Status badge component
│       │   ├── StaffActionsMenu.tsx       # NEW - Row actions dropdown
│       │   ├── BulkImportModal.tsx        # NEW - CSV upload modal
│       │   ├── AddStaffModal.tsx          # NEW - Single staff creation modal
│       │   ├── RoleChangeDialog.tsx       # NEW - Role change AlertDialog
│       │   ├── DeactivateDialog.tsx       # NEW - Deactivate confirmation AlertDialog
│       │   └── index.ts                   # NEW - Component exports
│       ├── pages/
│       │   └── StaffManagementPage.tsx    # NEW - Main page
│       └── types.ts                       # NEW - TypeScript types
├── App.tsx                                # MODIFY - Add StaffManagementPage route
```
...
- [ ] E2E test: Create staff → Edit role → Verify session invalidation (deferred)
- [ ] E2E test: Bulk import → List refresh (deferred)
- [ ] E2E test: Deactivate → Verify user cannot log in (deferred)

### Task 30: ID Card Redesign — Professional Government-Grade Design (AC8)

#### 30a. Asset Preparation
- [x] Copy `apps/web/public/images/oyo-coat-of-arms.png` to `apps/api/assets/oyo-coat-of-arms.png`
- [x] Update `IDCardData` interface in `id-card.service.ts` with new fields: `phone`, `staffId`, `logoBuffer`
- [x] Update `user.controller.ts` `downloadIDCard()` to pass `phone`, `staffId` (user.id), and `logoBuffer` (loaded from filesystem)
- [x] Update `staff.service.ts` `downloadIdCard()` to pass `phone`, `staffId`, and `logoBuffer`

#### 30b. Front Side Redesign
- [x] Brand maroon header bar (#9C1E23) replacing green (#006400)
- [x] Embed Oyo State coat of arms PNG in header (left side)
- [x] Header text: "OYO STATE LABOUR & SKILLS REGISTRY" (right of logo)
- [x] Staff photo with thin maroon border frame (left side of body)
- [x] Text block (right of photo): full name (bold, larger), role title, LGA, phone number
- [x] Staff ID in format `OSLSR-XXXXXXXX` (first 8 chars of UUID, uppercase)
- [x] Issue date formatted as DD/MM/YYYY
- [x] Bottom accent line (thin maroon rule) + "PROPERTY OF OYO STATE GOVERNMENT" in small caps
- [x] Subtle diagonal watermark "OSLRS" repeating across card body (light opacity)
- [x] Thin maroon border around entire card edge

#### 30c. Back Side Redesign
- [x] Brand maroon header bar with "OYO STATE GOVERNMENT"
- [x] "SCAN TO VERIFY" centered below header
- [x] QR code centered (existing functionality preserved)
- [x] Return instructions text block: "If found, please return to: Office of the Commissioner, Ministry of Trade, Industry & Cooperatives, Secretariat Ibadan, Oyo State."
- [x] Website URL at bottom: "oslrs.oyostate.gov.ng"
- [x] Thin maroon border around entire card edge

#### 30d. Color Scheme
- [x] Primary header: `#9C1E23` (brand maroon/burgundy)
- [x] Header text: `#FFFFFF` (white)
- [x] Body text: `#1a1a1a` (near black)
- [x] Subtitle/label text: `#4a4a4a` (dark gray)
- [x] Photo border: `#9C1E23` (maroon frame)
- [x] Bottom accent line: `#9C1E23`
- [x] Watermark: `#9C1E23` at 4% opacity
- [x] Back disclaimer: `#666666` (medium gray)

#### 30e. Testing & Verification
- [x] Update `apps/api/src/services/__tests__/id-card.service.test.ts` for new interface fields (6 tests passing)
- [ ] Verify PDF opens correctly in viewer
- [ ] Verify both front and back pages render
- [ ] Verify logo displays correctly in header
- [ ] Verify all text fields readable (name, role, LGA, phone, ID, date)
- [ ] Verify QR code still scannable
- [ ] Run performance test: generation must complete in < 2 seconds
- [ ] Manual test: Download ID card from Staff Management page for active user
- [ ] Manual test: Download ID card from user self-service endpoint
- [x] All API tests pass: `pnpm vitest run apps/api` (252/252 passed)

### Review Follow-ups (AI) — Code Review 2026-02-06
- [x] [AI-Review][HIGH] Fix useDebounce using useMemo instead of useEffect — timers never cleaned up [StaffManagementPage.tsx:21-30]
- [x] [AI-Review][HIGH] Add auth null-guard (req.user?.sub) to createManual, importCsv, resendInvitation [staff.controller.ts:132,149,213]
- [x] [AI-Review][MEDIUM] Escape ILIKE wildcards (%, _) in search input before DB query [staff.service.ts:91-98]
- [x] [AI-Review][MEDIUM] Centralize auth headers in shared apiClient, remove duplicate getAuthHeaders from staff.api.ts [api-client.ts, staff.api.ts]
- [x] [AI-Review][MEDIUM] Add UUID format validation for :userId route params [staff.routes.ts]
- [x] [AI-Review][MEDIUM] Standardize API response format: removed `status: 'success'` wrapper from all 5 backend controllers (16 endpoints), updated 10 frontend response type interfaces, updated all test assertions
- [x] [AI-Review][LOW] Centralize auth headers in questionnaire.api.ts: removed local `getAuthHeaders()`/`API_BASE_URL`, imported from shared `api-client.ts`, removed redundant manual headers from 5 `apiClient` calls
- [x] [AI-Review][LOW] Tasks 19-29 (Activation Wizard) scope creep documented — added retrospective note to Story 1-4
...
```
┌─────────────────────────────────────────────────────────────────┐
│ Staff Management                    [Add Staff] [Bulk Import]   │
├─────────────────────────────────────────────────────────────────┤
│ [🔍 Search...          ] [Status ▼] [Role ▼] [LGA ▼] [↻ Refresh]│
├─────────────────────────────────────────────────────────────────┤
│ Name          │ Email              │ Role        │ LGA    │ Status   │ Actions │
│───────────────┼────────────────────┼─────────────┼────────┼──────────┼─────────│
│ John Doe      │ john@example.com   │ Enumerator  │ Ibadan │ ●Active  │ [⋮]     │
│ Jane Smith    │ jane@example.com   │ Supervisor  │ Oyo    │ ●Invited │ [⋮]     │
│ ...           │ ...                │ ...         │ ...    │ ...      │ ...     │
├─────────────────────────────────────────────────────────────────┤
...
│ Change Role                            │
├────────────────────────────────────────┤
│ Select a new role for John Doe:        │
│                                        │
│ [Enumerator          ▼]                │
│                                        │
│ ⚠️ This will invalidate all active     │
│ sessions and require the user to       │
│ log in again.                          │
│                                        │
│            [Cancel]  [Change Role]     │
└────────────────────────────────────────┘
...
│ [Select a role                     ▼]  │
│                                        │
│ LGA * (shown for enumerator/supervisor)│
│ [Select an LGA                     ▼]  │
│ Required for Enumerator role           │
│                                        │
│ ┌────────────────────────────────────┐ │
│ │ An invitation email will be sent   │ │
│ │ to the staff member to complete    │ │
│ │ their account setup.               │ │
│ └────────────────────────────────────┘ │
│                                        │
│            [Cancel]  [Add Staff]       │
...
| 2026-02-04 | Completed Task 28: Integrated ActivationWizard into ActivationPage.tsx. Added token validation endpoint `GET /auth/activate/:token/validate`. Page now validates token on mount showing loading→valid/invalid/expired states. Backend: AuthService.validateActivationToken(), AuthController.validateActivationToken(). API: 321 tests pass, Web: 745 tests pass. | Dev Session (Amelia) |
...
| 2026-02-06 | **Activation Wizard UX Fixes**: (1) Fixed bank dropdown selection UX in BankDetailsStep, (2) Fixed selfie capture camera resolution errors and Human.js model loading timeout in LiveSelfieCapture, (3) Added NIN checksum validation with real-time visual feedback in PersonalInfoStep + packages/types validation, (4) Added DOB age validation (15-70) with real-time feedback in PersonalInfoStep. | Dev Session (Claude Opus 4.6) |
| 2026-02-06 | **Staff Reactivation Feature (Full Stack)**: Added `reactivateUser()` to StaffService, controller endpoint, POST route. Created `reactivateStaff()` API call, `useReactivateStaff()` TanStack Query mutation. Created ReactivateDialog.tsx (green theme, UserCheck icon). Wired through StaffActionsMenu → StaffTable → StaffManagementPage. Updated StaffTable + StaffManagementPage tests. | Dev Session (Claude Opus 4.6) |
| 2026-02-06 | **Task 30 — ID Card Redesign Complete**: Full rewrite of id-card.service.ts with professional government-grade design. Front: brand maroon (#9C1E23) header with gradient effect (#7A171B), embedded coat of arms logo, centered "Oyo State Labour & Skills Registry" title, vertically centered photo with drop shadow + rounded corners, labeled fields (Name/Role/Location/ID No/Date Issued) with hairline separators, diagonal OSLRS watermark, bottom accent bar. Back: maroon header with gradient, QR verification code, return instructions to Commissioner's office. Added sentence case for roles, copied oyo-coat-of-arms.png to api/assets, updated user.controller.ts + staff.service.ts callers with new IDCardData fields (phone, staffId, logoBuffer). 8 test cases passing. 252 API tests pass. | Dev Session (Claude Opus 4.6) |
| 2026-02-06 | **Code Review #2 (Adversarial)**: Found and resolved ALL 8 issues (2H, 4M, 2L). Fixed: (H1) useDebounce useMemo→useEffect bug, (H2) missing auth guards in 3 controller methods, (M1) ILIKE wildcard injection, (M2) centralized auth headers in apiClient, (M3) UUID param validation, (M4) removed `status: 'success'` from all 16 API endpoints across 5 controllers + updated 10 frontend types + all test assertions, (L1) centralized auth headers in questionnaire.api.ts, (L2) documented Tasks 19-29 scope creep in Story 1-4. Zero technical debt. | Code Review (Claude Opus 4.6) |
| 2026-02-08 | **Code Review #3 (Adversarial)**: Found 9 issues (2H, 4M, 3L). Auto-fixed 7: (H1) Error code mismatch RESEND_LIMIT_EXCEEDED→RATE_LIMIT_EXCEEDED in useStaff.ts, (M1) Migrated StaffActionsMenu from custom dropdown to shadcn DropdownMenu for WCAG a11y, (M2) Fixed ActivationPage setTimeout leak with useRef cleanup, (M3) Implemented two-tier reactivation: quick restore + re-onboard with invitation email, (M4) Added Nigerian phone format validation to AddStaffModal, (L1) Refactored activation wizard to use apiClient via auth.api.ts, (L2) Added status enum validation in StaffController.list. Action items: (H2) ID Card website URL deferred until domain finalized, (L3) Radix forwardRef warning tracked for future version upgrade. | Code Review (Claude Opus 4.6) |
