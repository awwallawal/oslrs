...
5 findings fixed in-pass — **M1** (stale dashboard) + **M2** (NIN-dedupe TOCTOU backstop → 409, parity with the public submit; re-graded from "acknowledged" once I confirmed the public path already handles it and 9-60 didn't) + **L1/L2/L3**; the **operator manual app-run gate** is carried to HANDOFF.md as a hard pre-merge check (the new DB write paths are CI-real-DB-tested but not run here). See **Review Follow-ups (AI)**.
