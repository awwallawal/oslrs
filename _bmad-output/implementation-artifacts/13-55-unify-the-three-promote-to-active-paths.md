...
| **R2** | ✅ **CLOSED — the inventory is an assertion now, not a note.** "Informational" is how a measurement rots: 13-54's amendment already named this file "the working reference" for both spellings, while the six-file raw-SQL figure it cites lived only in markdown. `respondent-promotion-census.test.ts` now pins all 6 raw `UPDATE "respondents"` sites **with a reason each**, 13-54-allowlist-style, plus the six-file figure itself. A 7th site reds and someone reads it. **↪ It did, on 2026-08-23: Story 13-50 added `registration-status.service.ts` (a `user_id` stamp after provisioning a sign-in account), the assertion went red, and the site was entered with a reason. Figure is now 7.** | Nothing outstanding. A future update-guard inherits the reasons instead of re-deriving them; a new raw writer is *unreviewed*, not forbidden — add it with a reason. |
