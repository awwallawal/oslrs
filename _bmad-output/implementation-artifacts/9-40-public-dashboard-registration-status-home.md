...
- [~] [AI-Review][Med] **M1 — AC#3 deviation: re-entry reuses the magic-link channel, not a direct session-authed wizard resume.** ACCEPTED BY-DESIGN. Verified against reality: `/register` is `PublicOnlyRoute`-gated (relied on by 9-39 wrong-door recovery), `CompleteNinPage` is magic-link-token-authed, and all `registration.routes` are unauthenticated — a logged-in user cannot reach the wizard/NIN flows by session today. Building session-authed equivalents + an authenticated `WizardPage` edit-mode blind (no app run) is exactly the un-exercised-risk class to avoid. **Reviewer verdict: magic-link re-entry is the RIGHT call for launch** — anti-enumeration-safe, zero new risky surface, works end-to-end now; the full session-authed wizard edit is a documented post-launch enhancement (Dev Agent Record). AC#3's literal "no magic-link round-trip" is the conscious trade. [PublicUserHome.tsx `EmailLinkButton`] — **✅ CLOSED 2026-06-20 by Story 9-61** (in-session `/registration/manage` replaces the magic-link re-entry).
- [~] [AI-Review][Med] **M2 — AC#4 deviation: only the marketplace-consent flag is editable, not full identity/NIN/survey-answer editing.** ACCEPTED BY-DESIGN (AC#4 "inline edit surface — dev judgment"). Consent is the safe, low-blast-radius field and the one AC#5 already surfaces; full edit needs the heavier session-authed wizard edit-mode (designed in the Dev Agent Record, deferred to avoid blind un-exercised writes). **Reviewer verdict: acceptable for launch.** [me.service.ts `updateMarketplaceConsent`; PublicUserHome.tsx `MarketplaceCard`] — **✅ CLOSED 2026-06-20 by Story 9-61** (full session-authed wizard edit via `/registration/manage`; the consent toggle remains as the lightweight path).
...
- [x] [AI-Review][Low] **L1 — Completed-summary shows raw `status` enum + raw `lgaId` slug.** ✅ CLOSED 2026-06-21: lgaId→lgaName resolved server-side (9-61, `5574ae5`); `status` now mapped to friendly labels via `STATUS_LABEL` in `PublicUserHome.tsx` (Active / Pending NIN / NIN unavailable / Imported (unverified)), raw fallback for any future enum. Test: `summary-status` → "Active".
- [x] [AI-Review][Low] **L2 — Draft card shows "Step X", not "Step X of N"** ✅ CLOSED 2026-06-21: new `useWizardStepCount` hook derives N from the SAME pinned-form query the wizard uses (shared cache key, `enabled` only for the draft state), mirroring `WizardPage.buildSteps` (3 head + unique sections + review) — honoring the read-model's deliberate server-side omission (client owns "of N"). Draft card now renders "Step 3 of 11"; falls back to "Step 3" if the form is unresolved. Test: `reg-state-draft` → /step 3 of 11/i.
- [x] [AI-Review][Low] **L3 — Verification gap: `me.service.test` is real-DB / CI-only.** ✅ CLOSED 2026-06-21: confirmed green in CI across the integration AND now **runnable locally** — `DATABASE_URL=<app_test> pnpm --filter @oslsr/api exec vitest run src/services/__tests__/me.service.test.ts` → **10/10**. The "can't run locally" gap is resolved by Story 9-62's scratch-DB pattern + test-DB guard (point `DATABASE_URL` at a `…/app_test` in the docker `oslsr_postgres` container; UAT `app_db` is protected by the guard).
...
- **→ Now authored as Story 9-61** ("Authenticated registration edit + true session resume", Bob/SM 2026-06-19; scope reconciled by John/PM as a standalone Epic 9 post-launch enabler, sequenced before 9-32). This section is 9-61's source-of-truth design; **9-61 closes this story's review findings M1 (magic-link re-entry) + M2 (consent-only edit)** when built. [Source: 9-61-authenticated-registration-edit-and-session-resume.md]
