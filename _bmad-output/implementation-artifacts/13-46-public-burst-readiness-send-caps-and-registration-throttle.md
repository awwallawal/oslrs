...
| **R2** — The post-submit ask (attribution on the confirmation screen) is the stronger response-rate instrument and is NOT built. | Med | **CLOSED HERE — HANDED OFF to Story `13-63-station-level-attribution-vanity-paths-and-a-readable-number` §8, registered `ready-for-dev`. ⚠️ **POINTER CORRECTED AT ADJUDICATION 2026-08-22 — it named `13-65` (AC7), which is the `void` fan-out at the call sites and has nothing to do with the post-submit ask.** The work IS carried, in 13-63 §8 (*"R2 — the post-submit ask, and why it is NOT being built yet"*) and §9 (*"Where the confirmation screen actually is"*) — so nothing was dropped. But a reader following the old pointer would have landed on an unrelated AC and concluded R2 was lost. **A hand-off is only as good as its pointer** (§2ad).** 13-46's obligation was to discover, root-cause, file and register it; that is done. ⚠️ The trigger travels WITH the story and is written into it as a LOCK: it is EVIDENCE-TRIGGERED, not taste-triggered — Trigger: measure the public-rows-only `answered_rate` (query in `13-3-cutover-and-failover.md`) after AC10 deploys. If prominence does not move it, that justifies the ask; if it does, the funnel change is unnecessary. **Do NOT build it before that measurement exists.** | Runbook carries the exact query, scoped `r.source='public'` so staff captures cannot dilute the denominator | SM (Bob) to author · PM (John) to sequence |
