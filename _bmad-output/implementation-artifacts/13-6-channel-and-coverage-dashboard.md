...
- **⚠️ 13-33 harmonization (John/PM 2026-07-19) — read the CANONICAL source, and mind the scope you inherit.** The existing `getOverviewStats`/`getTrends`/`sourceBreakdown` are **submission-scoped** (`FROM submissions`, `s.source = …`). 13-33 shipped the canonical respondent-anchored `registryUnifiedSource`/`registry_unified` read (exposes `ru.source`, `ru.raw_data->>'campaign_source'`, `ru.lga_id`). When this story is drafted for dev: **coverage/completions DENOMINATORS must come from 12-4's `getRegistryTotals()` (already a HARD dep)** AND the **channel/LGA-coverage COUNTS should be respondent-scoped via `registryUnifiedSource`** (one row per person, latest non-empty submission) so channel totals reconcile with the 139 headline — OR explicitly document why a submission-scoped channel count is acceptable here (e.g. a channel = a submission-time attribution). Do NOT silently ship submission-scoped channel counts against a respondent-scoped headline (the 76-vs-139 class this epic exists to kill). Decide + record at draft time.
...
| 2026-07-19 | **13-33 harmonization note (John/PM).** Added a DEPENDENCY-discipline note: 13-33 shipped the canonical respondent-anchored `registryUnifiedSource`/`registry_unified` read; when this backlog story is drafted, channel/LGA-coverage COUNTS should be respondent-scoped via that read (denominators already ride 12-4) so channel totals reconcile with the 139 headline — or explicitly document a submission-scoped attribution. Prevents shipping submission-scoped channel counts against a respondent-scoped headline. No AC/scope change; found by the post-13-33 backlog sweep. |
