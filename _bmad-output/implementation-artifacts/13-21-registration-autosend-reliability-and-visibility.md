...
4. **AC5 — Backfill the affected.** ⚠️ **SCOPE SUPERSEDED by 13-24 AC1 (operator decision, Awwal 2026-07-10; reconciled in code 2026-07-23).** The original "Modupe + any post-13-12 public completers" framing was a *date-gated* reading that the prod dry-run disproved: the pool is **141 public respondents missing ≥1 marker, 116 with a resolvable email** (135 of them May-2026 soft-launch), not a handful. A thank-you/referral is promotional and NOT time-bound, so **no date filter is applied** — the backfill sends to ALL 116 emailable real registrants. The script's behaviour was already send-to-all (no code change was needed for this correction); what changed is the documented scope + the guardrails around it, which 13-24 owns: Resend-Pro gating (13-24 AC2), the inherited cross-system contact-dedupe (13-24 AC3/AC6), and Dry-run #2 (13-24 AC4). Unchanged here: honour suppression + the send-once markers (idempotent), never re-send to test rows.
...
| 2026-07-23 | **AC5 scope reconciled with 13-24 AC1** (dev-story 13-24, Task 1). AC5 text + the operator-residual line no longer say "Modupe + any post-13-12 completers" — the prod dry-run showed 141 missing / 116 emailable / 135 of them May-2026, and the operator decision is send-to-all (promotional email is not time-bound). No code change: `_backfill-registration-autosends.ts` was already send-to-all. The guardrails (Pro gating, inherited contact-dedupe, Dry-run #2) live in 13-24. | Amelia (Dev) |
