# Story 9.61: Authenticated registration edit + true session resume
...
Adversarial code review 2026-06-20 (paired code-review CLI, Senior-Dev workflow). 2 Medium + 3 Low fixed in-pass; 1 operator-gate carried to HANDOFF. Re-validated: API tsc/lint 0 + `me.routes` 10/10; web tsc/lint 0 + targeted 19/19; full web suite green (capped). Verified against reality: `submissions.questionnaireFormId` is plain `text` (no FK — `'self-edit'` safe); `respondents.nin` partial unique index prevents duplicate-NIN corruption; the public submit uses the IDENTICAL out-of-txn NIN pre-check (9-61 matches, not regresses); shared `submitWizardSchema` extraction keeps public submit byte-identical; AC#7 genuinely removes the magic-link re-entry.
...
- [ ] [AI-Review][Operator-Gate] **MANUAL APP-RUN before merge (NOT fixable in code).** The new DB write paths (`updateRegistrationFromWizard`, `completeNinAuthenticated`, respondent→wizard mapper) are verified by CI-only real-DB tests + tsc/lint/mocked routes — NOT exercised against a running app in this sandbox. **An operator MUST, before merge:** (1) run the API integration suite on a real DB (the 9-61 `me.service.test` block); (2) manually exercise `/registration/manage` end-to-end — edit an active registration, complete a pending NIN, confirm the audit rows + a fresh `submissions` row, and confirm Story 9-39's wrong-door recovery still redirects off `/register`. **Carried to HANDOFF.md.**
- [x] [AI-Review][Med] **M2 — NIN-dedupe TOCTOU backstop (FIXED — re-graded from the initial "acknowledged").** Closer inspection overturned the first read: the public submit ALREADY maps a concurrent-NIN partial-unique-index violation to a clean **409** (`registration.controller` :887, tested by `registration.routes.test.ts` :987), but 9-61's `updateRegistrationFromWizard` + `completeNinAuthenticated` did NOT — so on the rare race they returned a raw **500**, making 9-61 *less* robust than the public path (not "consistent"). Fix: both transactions are wrapped to detect `respondents_nin_unique_when_present` (shared `isNinUniqueRace` helper, identical detection to the public handler) and throw `NIN_DUPLICATE` 409. The partial unique index still prevents corruption; this only corrects the error shape on the race → parity with the public submit. [me.service.ts] _Test note: the rare race is not unit-reproducible (the pre-check covers the deterministic duplicate, which IS tested — "another-NIN 409"); the backstop mirrors the public handler, which is itself tested via a mocked constraint error, and runs through the same CI real-DB suite._
...
- **HARD deps:** Story 9-38 (`respondents.user_id` link + `me` read-model), 9-18/9-12 (wizard + draft hydration), 6-1 (audit). **Consumer to rewire:** 9-40 (`PublicUserHome`). **Downstream consumer:** 9-32 (NDPA self-service rectification — sequence 9-61 before 9-32).
...
**Validation run locally:** API `tsc` + `lint` clean; `me.routes` mocked tests **10/10**; web `tsc`(build) + `lint` clean; web targeted suites (PublicUserHome, WizardPage ×2, route-resolution) green; full web suite (see board). The 9-61 **real-DB integration tests are CI-only** (no local `DATABASE_URL`) — see the manual-run gate above.
...
- `apps/api/src/services/me.service.ts` (modified) — `getEditableRegistration`, `updateRegistrationFromWizard`, `completeNinAuthenticated`, mapper + types. _Review M2: `isNinUniqueRace` helper + both write transactions map a `respondents_nin_unique_when_present` race to 409 (parity with the public submit); L2: added `me.registration_edit.completeness_skipped` warn; L3: dropped `email` from the self-edit audit details._
...
Working tree uncommitted, ready for the paired code-review CLI. **The review-pass also requires the manual app-run gate above.** After both, commit on `track/journey-9-39-40-21` (no push) flipping `9-61-...` in `sprint-status.yaml` in the same commit; mark 9-40 review M1/M2 closed.
...
5 findings fixed in-pass — **M1** (stale dashboard) + **M2** (NIN-dedupe TOCTOU backstop → 409, parity with the public submit; re-graded from "acknowledged" once I confirmed the public path already handles it and 9-61 didn't) + **L1/L2/L3**; the **operator manual app-run gate** is carried to HANDOFF.md as a hard pre-merge check (the new DB write paths are CI-real-DB-tested but not run here). See **Review Follow-ups (AI)**.

Validation: API tsc/lint 0 + `me.routes` 10/10; web tsc/lint 0 + targeted 19/19; full web suite green (capped). **This closes Story 9-40 review findings M1 + M2.** Handed back for commit on `track/journey-9-39-40-21` (no push); `sprint-status.yaml` 9-61 → done.
...
| 2026-06-20 | Senior-Dev code review: M1 (stale dashboard → invalidate `me`) + M2 (NIN-dedupe TOCTOU backstop → 409, parity with public submit) + L1 (success toast) + L2 (completeness-skip log parity) + L3 (audit PII minimisation) fixed in-pass; operator manual app-run gate carried to HANDOFF. Status `review` → `done`. Closes 9-40 M1/M2. | Paired code-review CLI; review-before-commit discipline. |
