...
- [x] [Self-flag][Med] CONFIRMED 2026-06-22 under a real run: `alert.service.test.ts` + `backup.worker.test.ts` + `super-admin-recipients.test.ts` all GREEN (4 files / 55 pass) — `admin@test.com` survives the new undeliverable filter (`test.com` ≠ `test`/`*.test`), and `example.com` IS dropped (asserted in super-admin-recipients.test).
- [x] [Self-flag][Low] CONFIRMED 2026-06-22 under a real run: `notification-meter.service.test.ts` GREEN — `readUsage` byCategory slicing + the `:bounced`/`:complained` suffix split verified. Enum categories carry no `:`; the only `:` in key-position is the event suffix, which `readUsage` handles (real-delivery byCategory vs negative-event buckets).
