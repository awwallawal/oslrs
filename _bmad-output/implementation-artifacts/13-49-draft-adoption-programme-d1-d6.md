...
⚠️ **NOT DONE — this touches 37 citizens' live links, so it is Awwal's call, not mine.** The better version routes an adopted-draft resume to `/check-registration` instead of expiring it, but that is code and belongs in a story.
**REOPEN TRIGGER: any `NIN_DUPLICATE` in the audit log from an adopted person — currently 0.** | Awwal (decision) + dev |
