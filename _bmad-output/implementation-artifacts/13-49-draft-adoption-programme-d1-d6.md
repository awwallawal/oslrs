...
held — instead of deleting that data when the drafts expire on 2026-08-31.**
...
The drafts **expire 2026-08-31**. Data that 203 people consented to give is deleted on that date.

## Acceptance Criteria

1. **AC1 — Expiry extended FIRST, before anything else.** All live `wizard_drafts` extended by **3 months**.
   One `UPDATE`, reversible, and it removes deadline pressure from every subsequent decision. Record the
   row count. *(Do this even if the rest of the story slips.)*
...
7. **AC7 — D5 Exclude (8) is a HARD guard in code.** `consent_basic = no` must be un-actionable: the script
   refuses to adopt or contact them even if the spreadsheet says otherwise. A spreadsheet is editable; a
   guard is not.
...
| **R2** — resume-path identity prefill unverified | Medium | **OPEN** | The authenticated-EDIT path maps head-step fields only (`WizardPage:186`); the anonymous RESUME path was never checked. If it also ignores `questionnaireResponses.firstname`, D4 invitees re-enter name/DOB (~30s, rest prefilled) — acceptable, but the copy must say "finish in 2 minutes", not "pick up where you left off". Verify by resuming ONE real draft. | dev |
