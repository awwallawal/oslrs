...
| **OSL-2026-RRCHDX** | **Dasola Zainab Aminu** | ⚠️ **`1589857782` — 10 DIGITS, fails `^\d{11}$`** |
...
Consequently: **do the 9 clean promotions; do NOT chase the 10th.** The 10-digit value needs no manual
confirmation campaign — leave that person `nin_unavailable`, which costs them nothing. Same reasoning
...
set it and promote `nin_unavailable → active`, writing an audit row. **9 promote cleanly; the 10th must NOT
be silently padded or dropped** — a 10-digit value is most plausibly a **dropped leading zero**
(`01589857782`), which is a data-entry question for the operator, not a guess for a script. Route it to
manual confirmation.
...
with these exact 10 values and asserts 9 `promote` + 1 `manual_review_bad_shape`, named as
`OSL-2026-RRCHDX`. So the story's claim and the code's behaviour are checked against each other in CI
rather than in an operator's terminal on the live run. ⏳ **NOT YET EXECUTED against prod — see R9.**

⚠️ **Effect on the numbers, CORRECTED 2026-08-02.** The original read "the 35 becomes **25**". That
assumed all 10 clear, but **only 9 clear without a human** — `OSL-2026-RRCHDX` is held for manual
confirmation by design. The honest figures are: the 35 becomes **26**, and post-adoption
`nin_unavailable` is **26 + 24 (D3) = 50** (D3 is 24, not 20 — see the cohort note). It drops to 25/49
once RRCHDX's NIN is confirmed with the person. Asserted in the test so the arithmetic cannot drift back.
...
|---|---|---|---|---|
| **R1** — AC10's single-record LIVE apply | **High** | **OPEN — blocks `done`.** Gated on Resend Pro (unpaid), so it could not be discharged this session. **Half done:** the dry-run leg is complete and re-runnable; the live leg is not started. | ✅ **Dry-run leg DONE** — `pnpm --filter @oslsr/api draft:adopt -- --dry-run [--explain <id>]` printed per-cohort counts, validated every adopting row, and rendered the exact mutations for a D1 and a D3 record (`nin key present: false` / `_pendingNin: true` / `status: pending_nin_capture`). ⏳ **Live leg OUTSTANDING:** after Resend Pro, run `--apply --confirm-i-am-not-dry-running --max 1` against ONE named prod record and verify, in order: (1) `respondents` row exists with `metadata->>'adopted_by' = '13-49'`; (2) a reference code was minted; (3) the confirmation and the 13-12 thank-you both arrive; (4) the `/check-registration` link resolves that person — this is the leg that proves the email re-attach worked, and it is the step most likely to fail. THEN the batch. **Until this row carries a real result, `Status:` must not read `done`.** | Awwal + adjudicator |
...
- **R9** — AC14's 9 promotions: implemented and tested, **blocked on a DEPLOY, not on Resend**.
  No 13-49 source is on `origin/main`; SSH to the VPS times out; `prod-verify.yml` is read-only
  by construction. Sequence is commit → push → CI deploy → run on the box.
