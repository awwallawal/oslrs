...
⚠️ **ONE HONEST LIMIT ON THIS CLOSURE.** It is closed on *deploy + unit tests*, **not** on an observed live attach. `no_nin_identity_match` is still **0**, and the whole point of R21 is that a zero there is ambiguous — it reads the same whether the guard is working and unneeded, or not running at all. **The zero only becomes evidence after a no-NIN re-registration is attempted against someone already in the register.** Awwal is running exactly that. **REOPEN TRIGGER: if that run mints a NEW reference code instead of returning the existing one, R21 is not fixed — reopen it.** Fail-open makes silent non-firing the *expected* failure mode here, which is precisely §2t's trap. | adjudicator |
