...
- **Commit plan:** four atomic per-F-ID commits at the pause (after review) — `git add -p` splits the shared `export.service.ts` (F-008 sanitize / F-009 stream) and `export.controller.ts` (F-009 cap+stream / F-013 audit) hunks.
