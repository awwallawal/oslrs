...
- The query already produces ONE row per respondent via `DISTINCT ON (r.id) … ORDER BY r.id, s.submitted_at DESC NULLS LAST`. **⚠️ 13-33 harmonization (John/PM 2026-07-19) — CORRECTNESS, not just style: the per-row `data_status` MUST derive `hasSubmissionData` from the latest NON-EMPTY submission, EXACTLY as the canonical `registryUnifiedSource`/`REGISTRY_UNIFIED_SQL_TEXT` LATERAL does** (`raw_data IS NOT NULL AND <> '{}'::jsonb ORDER BY submitted_at DESC LIMIT 1`) — **NOT** the plain `DISTINCT ON` latest submission. The two DIFFER when a respondent's newest submission is empty but an earlier one had answers: the DISTINCT-ON-latest badge reads `no_submission`/`data_lost` while 12-4 + the analytics strip (canonical latest-non-empty) read `completed` — precisely the forked-derivation drift the note above forbids. **RULING: 12-7 stays an INTENTIONALLY-SCOPED consumer** — the filtered/paginated/searchable list is a legitimately different query SHAPE and is NOT re-pointed wholesale onto `registryUnifiedSource` — **but it MUST match the "which submission" choice** by either (a) composing `registryUnifiedSource` for the `raw_data`/`status`/`source` derivation inputs, or (b) changing its own submission selection to latest-NON-EMPTY. Add a **parity test**: the table's `dataStatus` for a seeded respondent equals what the canonical read + `deriveDataStatus` produce.
