...
2. **AC2 — Tier-2 upgrade.** ⚖️ **DESCOPED 2026-09-12 (Awwal) — MOVED TO STORY 13-40, not left
   open inside a `done` story.**

   ~~On member-side confirmation the card renders **"Member-verified"**. The tier derives from the
   taxonomy's verification substrate — **not** a badge-local re-derivation.~~

   ⭐ **Why the AC MOVES rather than closing as a residual.** A story is `done` when its acceptance
   criteria are met. An unbuilt AC left sitting inside a `done` story makes the record claim
   something the artefact does not support — and **the AC wins, because the AC is the checkable
   artefact**. That is exactly how 13-2's AC3.3 went wrong (it instructed a dev to violate the
   ruling printed at the top of its own story). So AC2 is struck here and owned elsewhere.

   **The owner already existed — it was not carved for this.** `13-40-assessor-verify-imported-rows-queue`
   (backlog, spun up 2026-07-20) already specifies *"a callback outcome action per row: confirmed
   (**promote tier-1→tier-2** via the 12-4-aligned marker)"*. Tier-2 needs three things that do not
   exist — a `member_confirmed` marker on the taxonomy substrate, an SMS confirmation loop (blocked
   on **Termii not being cleared**, 13-2) and the Assessor callback queue (**13-40**) — and all
   three already have homes. Carving a fourth story would have duplicated 13-40.
   → [[feedback_canonical_primitive_backlog_sweep]] applied to STORIES rather than code.

   ⛔ **The guard that holds the line stays:** `WorkerCard.test.tsx` asserts "Member-verified" NEVER
   renders. Do not ship that copy before the substrate exists — a tier that cannot be earned is a
   badge that never changes.
...
6. **AC6 — Tests.** ⚖️ **AMENDED 2026-09-11 (Awwal's ruling on R3) — the old wording
   ...
   Tier-1 with name; ~~tier-2 on confirmation~~ (struck 2026-09-12 with AC2, descoped to 13-40);
   tooltip/aria present; never emits bare "Verified" for an import; and **both directions of AC4's
   predicate**:
...
| ~~**R4**~~ | ✅ **RESOLVED 2026-09-12 BY MOVING THE AC, not by closing a row.** AC2 is **struck from this story and owned by 13-40**, which already specifies *"promote tier-1→tier-2"* — so nothing was carved and nothing was duplicated. The other two substrates also already have homes: the SMS loop is 13-2 (blocked on Termii), the Assessor queue is 13-40. **An unbuilt AC inside a `done` story is the 13-2 AC3.3 failure mode**, so the honest move is to relocate the criterion rather than annotate it. The `WorkerCard` guard asserting "Member-verified" never renders STAYS, and is the thing holding the line. | **CLOSED 2026-09-12 — descoped to 13-40** | ~~ORIGINAL: **AC2 (tier-2) has no substrate and stays open.** Carve the member-side check as its own story before any "Member-verified" copy ships. ⚖️ **CARVE TRIGGER recorded 2026-09-11 — so this is a scoped story when someone picks it up, not a rediscovery.** Tier-2 needs THREE things that do not exist: a `member_confirmed` column on the taxonomy substrate (never a badge-local re-derivation, per AC2); an SMS confirmation loop, which is blocked on **Termii not being cleared**; and an Assessor callback queue. ⛔ Do not ship "Member-verified" copy before all three — a tier that cannot be earned is a badge that never changes, and the existing `WorkerCard` test asserting it NEVER renders is the guard holding that line.~~ |
...
| ~~**R6**~~ | ✅ **HANDED TO 13-2 R-A2 (which is `review`, i.e. still open and owns it).** ⭐ **This was never 13-58's debt.** It records that *another* story has not run yet — so it could never be discharged here, and left OPEN it would hold this story at `review` forever, waiting on work this story is explicitly forbidden to do. The residual guard's own failure message names this exit: *"or hand it to a named story — then say so in the state cell."* 8,278 consenting people remain invisible to the marketplace until R-A2 lands; that is tracked there, with both halves and a predict-then-compare. | **CLOSED 2026-09-12 — handed to 13-2 R-A2** |
| ~~**R7**~~ | ✅ **DISCHARGED 2026-09-12 — the finding's whole job was to correct the NEXT story's plan, and it did.** Verified by reading 13-2's R-A2 rather than trusting a note: it now reads explicitly **(a)** the gate change **and (b)** widening `_backfill-marketplace-extraction.ts`'s `source` predicate, carrying Awwal's mechanism ruling and a predict-then-compare. ⭐ **AND THE WARNING WAS MOVED TO WHERE THE EDIT HAPPENS.** A dev opening the gate is editing `PIPELINE_EXCLUDED_STATUSES` — whose comment described the by-construction gate but never said *"removing a status from this list creates ZERO profiles on its own."* That sentence now sits directly above the constant. A warning that lives only in a story file depends on someone choosing to read it; this one cannot be missed while making the exact change it warns about. Same lesson as **M5** — a guard wired at the site of an instance is not a guard on the class. | **CLOSED 2026-09-12** | ~~ORIGINAL: ⛔ **Layer 4 is a gate change PLUS an enqueue path — opening the constant alone creates ZERO profiles.** [AI-Review][High] 2026-09-08. The import service holds no reference to the marketplace-extraction queue; the only production enqueue is `submission-processing.service.ts:1349-1353`, which the importer never calls; and the operator's create-profiles script `_backfill-marketplace-extraction.ts:126-140` is scoped `WHERE r.source = 'public'`, so it skips every imported row. The raw material is there (13-2 AC3.4 gives imported respondents a `submissions` row), so the fix is small, but it must be **named in 13-2 R-A2's plan**, or the gate gets opened, nothing happens, and the cause is hunted in the badge. ⚖️ **MECHANISM RULED 2026-09-09 (Awwal): widen `_backfill-marketplace-extraction.ts`'s `source` predicate — NOT an import-time enqueue.** It stays an operator one-shot with a dry-run checkpoint, and it does not push 8,278 jobs through BullMQ workers that run IN the API process on a 2 GB box. Deliberately NOT implemented here: layer 4 is 13-2 R-A2, and widening the predicate is inert until the gate opens anyway (the worker still refuses on status). **PREDICT BEFORE RUNNING:** profiles created should equal imported respondents with `consent_marketplace = true`; measure from prod first.~~ |
...
| Date | Change |
|---|---|
| 2026-09-12 | ✅ **STATUS → `done`. The last three residuals resolved on substance, not by decorating the ledger.** ⚠️ Worth knowing while doing it: `isOpenState` clears a row that merely CONTAINS `CLOSED`/`✅`/`RESOLVED`/`DISCHARGED`, so the guard cannot tell a real discharge from a tick — the discipline has to come from the writer. **R6 → handed to 13-2 R-A2** (it was never this story's debt: it records that ANOTHER story has not run, so it could never be discharged here and would have held `done` hostage forever). **R7 → discharged**, its job being to correct the next story's plan — verified by READING 13-2's R-A2, which now carries both (a) and (b). **R4 → the AC MOVED rather than the row closing.** |
| 2026-09-12 | ⭐ **AC2 DESCOPED TO 13-40 — and no new story was carved, because the owner already existed.** `13-40-assessor-verify-imported-rows-queue` has specified *"promote tier-1→tier-2"* since 2026-07-20. A story is `done` when its ACs are met, so an unbuilt AC inside a `done` story makes the record claim what the artefact cannot support — the 13-2 AC3.3 failure mode, where the AC wins because the AC is the checkable artefact. 13-40 now records that **the render half is already live** and that the missing piece is the `member_confirmed` marker; the `WorkerCard` guard asserting "Member-verified" never renders stays as the tripwire. |
| 2026-09-12 | 🛡️ **R7's warning MOVED INTO THE CODE.** `PIPELINE_EXCLUDED_STATUSES` now carries, directly above the array, the fact that removing a status from it creates **ZERO** profiles on its own, with the three upstream reasons and the (b) half. A dev opening the gate is editing that constant; until now the instruction lived only in a story file, and a warning you must choose to read is the same class as a fix that never fires. |
