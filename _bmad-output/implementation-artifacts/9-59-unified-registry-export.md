...
- [ ] [AI-Review][Low][Accepted] Unified mode is blocked when no form is published (answer columns require a schema). Accepted by design — identity-only fallback would be a separate enhancement; not worth the branch now.
