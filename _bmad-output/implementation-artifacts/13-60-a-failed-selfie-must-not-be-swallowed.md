...
| **R6** | 🔎 The contention behind R4. | ✅ **IDENTIFIED 2026-08-14 — it is LOCAL MEMORY HEADROOM, not a suite or code defect.** | **The decisive evidence is that it has never happened in CI:** across the last 20 `CI/CD Pipeline` runs on `main`, the only failure is `31737577114` — `test-api`, my fresh-DB defect. **`test-web` has never failed.** Locally: **4 physical cores, 15.8 GB RAM, and free RAM falls from 3.7 GB to 0.95 GB during a SINGLE-file run** (peak node RSS across all processes is only 0.92 GB — node is not the hog; the machine simply has ~1 GB of headroom while testing). Memory pressure produces exactly the observed **nonlinear cliff** (1.7s → 20s); CPU starvation would degrade linearly. ⛔ **BOTH observed failures happened while I was running a SECOND vitest process concurrently** — the "flake" was substantially self-inflicted by the verification method. **Operational rule: never run two vitest processes at once on this machine.** If it recurs anyway, `VITEST_MAX_THREADS=1` is the sanctioned lever for a contention/timeout flake (Pitfall #37b permits it for exactly this, unlike the segfault it never fixed). **Do NOT raise the timeout a fourth time.** | adjudication |
...
| **R4** | `route-resolution.integration.test.tsx > resolves '/login'` fails intermittently under load (`expected 16 to be greater than 50`). | ✅ **CLOSED 2026-08-14 in `3e6c68b`** — fixed on Awwal's instruction rather than left accepted. `waitFor` 5s→**15s**, per-test 10s→**20s**, and the failure now names the route and character count instead of reading as a mystery. Verified: the six-file combination that originally failed passes **twice at 92/92** (`/login` 1744ms, 1826ms); full file **twice at 57/57**. ⚠️ **A MITIGATION, NOT A CURE — see R6.** ⛔ **A semantic predicate was tried and REJECTED ON EVIDENCE**: waiting for `<PageSkeleton>` (`aria-label="Loading page"`) to disappear reads better and is wrong — under vitest the lazy import resolves within a microtask, so the fallback is gone before `waitFor`'s first poll. Probed by inverting to `.toBeInTheDocument()`: **never matched once in 15s.** It would have waited for nothing and passed over a blank page. The probe is recorded in the file so it is not re-derived and re-introduced. | web/test-arch | Measurement: `/login` is untouched by 13-60 and `/profile-completion` (the route this story adds) passes every run; isolated it passes 57/57, and it flaked identically on 2026-08-11 **before** this work. Mechanism: a raw content-length threshold firing before a lazy chunk resolves — fails at >5s, passes at ~3s. **Trigger:** it has now cost two sessions; next occurrence, replace the threshold with a `findBy*` on a specific element. | web/test-arch |
