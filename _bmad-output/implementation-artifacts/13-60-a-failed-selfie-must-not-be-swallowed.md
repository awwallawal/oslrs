...
| **R6** | 🔎 **The contention behind R4 is UNIDENTIFIED.** `resolves '/login'` completes in **~1.7s** on a quiet machine and once exhausted a **15s** budget (20,282ms) immediately after another vitest process. A **10× spread is pathological, not "slow"**. | **OPEN — under investigation** | R4's timeout has now been raised **three times** (1s → 5s → 15s). ⛔ **Do NOT raise it a fourth time**: that would be treating the symptom for the third time in a row. **Trigger:** the next red on this test means the budget was never the answer and the contention is the defect. Being chased 2026-08-14 — findings land in the handoff, not here, because the cause is a suite-wide property rather than 13-60's. | adjudication |
