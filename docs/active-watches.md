...
|---|---|---|---|
<!-- Story 9-8 watch fully closed 2026-05-04 13:35 WAT: 48h Report-Only window CLEAN, enforcing flip pushed (commit ac903c8 + recovery 1e0173e), 85-min post-push soak CLEAN (0 csp_violation in pino + 0 nginx /csp-report POSTs + PM2 stable + 9-11 endpoint HTTP 401 as designed + dual-domain CSP parity holds). Story 9-8 status: done. -->
<!-- 2026-05-04 lessons surfaced and tracked separately:
     1. Pitfall #23 variant — VITE_HCAPTCHA_SITE_KEY + VITE_GOOGLE_CLIENT_ID stored as Secrets but workflow reads as vars.X → empty bake → hCaptcha test-key fallback → 35-min login outage (11:11→11:46 UTC). Fixed via gh variable set + empty-commit re-deploy. Playbook Pitfall #23 needs an addendum on vars-vs-secrets ambiguity. Auto-memory line about VITE_* storage scope was wrong; needs correction.
     2. PM2 restart counter at 38 cumulative — long-tail Story 9-10 tracking item, not field-blocking. -->
...
| 6 | Operations Manual enumerator-section drafted (`.md` + print-ready HTML/PDF) **[REVISED 2026-05-04: Ministry office-printer print run downstream, NOT field-blocking; closure-gate relaxed per Awwal]** | `prep-operations-manual-enumerator-section` | ✅ Done 2026-05-05 (Wave 1 satellite — drafted + rendered + AI-persona-ratified + adversarial code review pass clean; real-Iris + real-Gabe ratification + Awwal placeholder fills tracked as v1.1 follow-ups OUTSIDE this story's scope per Awwal directive 2026-05-05) — *deliverables: `docs/operations-manual/enumerator-v1.{md,html,pdf}` + `enumerator-quick-ref-v1.{md,html,pdf}` + `iris-signoff-v1.md` + `gabe-signoff-v1.md`; v1.1 hard-gates before printable: M1 (Section 5 naira amounts) + M2 (vehicle accident escalation) + M3 (Section 6.3 phone numbers) + code-review M3 (AC#3 A5 page-size build-script fork)* |

**Score: 4/6 done. 2 outstanding (engineering).** Item #6 flipped `review → done` 2026-05-05 per Awwal directive ("written process is done; all other issues will be resolved outside here"). Remaining outstanding: items #3 (Story 9-12 Public Wizard) + #5 (9-9 AC#5 backup encryption) — both engineering work.
