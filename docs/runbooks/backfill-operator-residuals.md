...
| `_backfill-marketplace-card-fields.ts` | 13-38 | Re-derives `marketplace_profiles.experience_level` + the NEW `business_name` from each respondent's latest submission answers (falls back to `metadata->'adopted_draft_answers'` for the 13-49 D2 rows that have no submission) | ⬜ **not run — REQUIRED for 13-38 to be visible** | ⚠️ **ORDER MATTERS — two preconditions, both BEFORE `--apply`:** (1) `db:push` must land the new `business_name` column or the marketplace search SQL 500s; (2) the marketplace FTS trigger must already carry `business_name` (deploy runs `scripts/migrate-custom-sql-init.ts`; check with `SELECT pg_get_functiondef(oid) FROM pg_proc WHERE proname='update_marketplace_search_vector';`). **The trigger recomputes `search_vector` only on INSERT/UPDATE and this backfill is idempotent** — so any row written while the trigger was stale keeps a stale vector *permanently*, and a re-run reports `needsUpdate=0` and touches nothing. Those workers would then be unfindable by their trading name, because search matches on `search_vector @@ plainto_tsquery` as a hard WHERE filter. **Recovery if it did run out of order:** force a no-op update to refire the trigger — `UPDATE marketplace_profiles SET business_name = business_name WHERE business_name IS NOT NULL;` — then spot-check with `SELECT id FROM marketplace_profiles WHERE search_vector @@ plainto_tsquery('english','<a known trading name>');`. Then `--dry-run` (the count IS the evidence), `--apply --confirm-i-am-not-dry-running`, re-run for `needsUpdate=0`. Two things it repairs: (1) the pre-13-38 normaliser stored **NULL for `less_1` and `over_10`** and collapsed **`7_10` into `4-7`** — the questionnaire's real values were never in its canon (`docs/questionnaire_schema.md:134-141`); (2) `business_name` is new, so every existing row is NULL. Probe before/after: `SELECT COALESCE(experience_level,'NULL'), count(*) FROM marketplace_profiles GROUP BY 1 ORDER BY 2 DESC;` |
...
| `_backfill-association-identity.ts` | 13-67 | Writes the **vouching body** onto the two live association batches — `01a071c8…` → **ASNAT** (56 rows), `01a072ae…` → **AFAN** (8,222 rows) — setting `import_batches.association_name` and MERGING `metadata.association_name` into each respondent. Awwal's ruling 2026-09-05; names are NOT re-derived from the sheets (those name NCARES / L-PRES, which are programme registers, not associations). | ⬜ **not run — REQUIRED before 13-58 renders anything** | ⚠️ **Deploy first** (`db:push` must land `import_batches.association_name`). Then `--dry-run`: the count IS the evidence — expect **56 / 8,222** and `alreadyTagged: 0`. Apply with `--apply --confirm-i-am-not-dry-running`; re-run `--dry-run` and read `alreadyTagged` = 8,278 ⇒ done. **It REFUSES to write** if either batch id is absent (wrong database), if a batch's row count disagrees with 56 / 8,222 (deliberate: override with `--accept-count-drift` only after reconciling), or if any row holds non-object `metadata` (no override — the JSONB concat would ARRAY-WRAP the document and orphan `normalisation_warnings` / `import_extra.full_name`). `--dry-run` + `--apply` together are refused, not resolved. Idempotent (JSONB merge). Probe: `SELECT count(*) FROM respondents WHERE import_batch_id IN ('01a071c8-709f-73a3-9e31-eb0e8cedf01a','01a072ae-83e7-7e8f-902d-590f0c589c74') AND metadata ->> 'association_name' IS NOT NULL;` — **0 ⇒ not run, 8,278 ⇒ done.** ⭐ **TWO PHASES in one run.** Phase 2 (13-67 R2, Awwal 2026-09-07) additionally attaches the vouch to the **12 people the AFAN import MATCHED** to an existing self-registration — they keep their own `import_batch_id`, so phase 1's predicate cannot see them, and the only surviving link is the batch's own `failure_report` match hashes (`sha256(respondent.id)`, resolved by hashing forward over the register). Phase 2 writes `association_vouched_by_batch_id` alongside the name and **one `operator.association_vouch_attached` audit row per person** — these are live citizens' records being annotated with a third party's claim. It refuses on a hash that resolves to nobody, on a target already carrying a DIFFERENT body's name, and on a matched count that disagrees with the ledger's 12. Phase-2 probe: `SELECT count(*) FROM respondents WHERE metadata ->> 'association_vouched_by_batch_id' IS NOT NULL;` — **0 ⇒ not run, 12 ⇒ done.** |
...
| `_thankyou-referral-blast.ts` | 13-11 / 13-12 | Evergreen thank-you/referral blast (Cohort C completers) | ⬜ **not fired — double-send risk CLOSED 2026-07-23 (13-24)** | The old `❓ check for double-send` flag is resolved **in code**, not by a manual check: (1) the cohort SQL now excludes `metadata->>'thankyou_referral_sent_at' IS NOT NULL`, so anyone the 13-21/13-12 auto-send already thanked is out; (2) the shared `filterMarketingCohort()` additionally drops any address contacted within `MARKETING_CONTACT_GAP_DAYS` (5) per the `campaign_sends` ledger. Regression-proven (`blast-cohort-dedupe.integration.test.ts`, revert-to-RED). ⚠️ Needs the 13-24 code DEPLOYED before firing. Re-run `--dry-run` at send time and read its `excluded:` line. |

| `_backfill-registration-autosends.ts` | 13-21 AC5 / 13-24 | **The coordinated WELCOME send** — confirmation + thank-you/referral to ALL emailable real registrants (116 at the 2026-07-10 dry-run; NOT date-gated). Idempotent via the send-once markers, honours suppression, excludes test rows, and (13-24) inherits the contact-dedupe. | ⬜ **not fired — gated on Resend Pro** | Sequence: Pro live → Dry-run #2 → this → 5-day gap → blasts (`docs/runbooks/re-engagement-campaign-launch.md` §2). Dry-run first, then `--apply --confirm-i-am-not-dry-running --rate-per-minute 10`. 116 × 2 = 232 emails ⇒ **the free 100/day tier would silently truncate it**. |

> Dedupe note (13-24): the three blasts + this welcome all share ONE filter
> (`filterMarketingCohort`, `apps/api/src/services/campaign-contact.service.ts`). "Did I already contact
> this person?" is answered by the `campaign_sends` ledger, not by cross-reading these rows.
