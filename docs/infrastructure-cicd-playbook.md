...
| 18 | Local tests pass but CI's `pnpm build` fails on TS errors | vitest uses tsx (transpile-only, no strict type check); CI runs `tsc` which IS strict. Type errors slip past local test runs. | Always run `pnpm --filter @oslsr/api build && pnpm --filter @oslsr/web build` before commit when touching `.ts` files. (Two CI failure rounds caught this 2026-04-27 with `proxy-addr` import: missing `@types/`-package + 2-arg trust fn signature.) Future improvement: wire `tsc --noEmit` into pre-commit hook. |
