...
- **✅ DONE (2026-06-16):** Bob (SM) + John (PM) authored **Epic 12 — Dashboard System Refresh** (brief: `_bmad-output/planning-artifacts/epic-12-measurement-honesty-and-dashboard-refresh-brief.md`) — Tracks A (analytics honesty) + B (design-system enforcement) + C (role-dashboard completeness), foundation-first; 11 stories `ready-for-dev` (12-1…12-11 + 12-9) + 7 backlog shells (12-12…12-18). **9-59 shipped** (PR #4 merged + deployed; it DEFINES the canonical `data_status` taxonomy the epic consumes — sequencing inverted, see brief §6). **Next dev step:** dev-story the Tier-0 foundation in order **12-1 → 12-2 → 12-3 → 12-4**.
