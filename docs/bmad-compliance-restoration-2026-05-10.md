...
| Last verified | 2026-05-10 ~16:00 UTC (R1-R5+F5a `c3db684`, F1 `729b83b`, F2 this commit; <2-week field-survey runway active) |
| Status | OPEN — **§A R1-R5 closed via `c3db684`; F5 closed `609e06e`; F5a closed `c3db684`; F1 closed `729b83b`; F2 closed by this commit.** Remaining: F3 (AC#9 carry), F6 (deferred wait-and-see), F7 (Operate-phase Story 9-14), F8 (alert ergonomics nice-to-have). F4 dissolved. **Story 9-12 + VPS resize are the only outstanding Tier 1 items.** |
...
- **2026-05-10 ~15:30 UTC** — **F1 (AC#7 logrotate) closed** by previous commit `729b83b`. VPS-side: pm2-logrotate v3.0.0 installed + configured; journald `SystemMaxUse=2G` + `MaxRetentionSec=30d`. Disk recovered 2.2 GB (2.5 GB → 266 MB). Runbook §1.4 "Log retention policy" subsection authored. Story 9-9 Subtask 7 marked DONE; Status Matrix + Priority Map + Subtask 11.1 progress (`7/10 → 8/10`) all refreshed; Change Log entry appended. sprint-status.yaml comment updated `DONE 7/10 → 8/10`. AC#9 carry-clause acknowledgement added. **Tier 1 Critical Path to Field complete except Story 9-12 (FRC #3 blocker, the only thing standing between current state and FRC complete) + VPS resize.**
- **2026-05-10 ~16:00 UTC** — **F2 (AC#4 auth rate-limit audit) closed** by this commit. 22-endpoint inventory in `apps/api/src/routes/auth.routes.ts`; 12 distinct rate-limit middleware modules mapped to NFR4.4. Findings: 0 HIGH / 1 MEDIUM (auto-fixed) / 3 exempt-with-rationale. AC#4-M1: `POST /auth/reauth` had no rate limit — accepts password and bcrypt-verifies to bump 5-min `reauth:<userId>` Redis key; without per-IP limiter, stolen-token holder could brute-force the password without tripping `loginRateLimit`. **Fix**: new `apps/api/src/middleware/reauth-rate-limit.ts` (5/IP/15min, matches `loginRateLimit` threshold); wired into `auth.routes.ts:108`. **Coverage suite**: new `apps/api/src/middleware/__tests__/rate-limit-coverage.test.ts` (6 tests + 22-entry coverage map source-of-truth + threshold contract documentation in test header) — drift detector for future route changes. **NFR4.4 "API Endpoints (General): 100/user/min" not implemented as global limiter** — deferred to future `prep-global-api-rate-limit` story when partner-API consumers come online (Epic 10) and `req.consumer` makes per-user keying meaningful. Story 9-9 Subtask 4 marked DONE; Status Matrix L37 + Priority Map AC#4 + Subtask 11.1 progress (`8/10 → 9/10`); Change Log entry appended. sprint-status.yaml comment updated `DONE 8/10 → 9/10`. **Tier 2 Critical Path to Field — F2 complete. Tier 1 + Tier 2 Critical Path now near-complete; only Story 9-12 (FRC #3 field-blocker) + VPS resize remain.**
...
### F2 — AC#4 App-layer rate-limit audit on `/auth/*` endpoints (CLOSED 2026-05-10)

- [x] **Task**: Inventory every `/api/v1/auth/*` and `/api/v1/staff/activate/*` endpoint, verify per-IP rate limits configured per PRD NFR4.4 (login: 5/15min, password reset: 3/hour, profile edit token: 3/NIN/day), add `apps/api/src/middleware/__tests__/rate-limit-coverage.test.ts` that asserts each endpoint enforces its documented limit. **CLOSED 2026-05-10 ~15:30 UTC** — 22-endpoint inventory; 12 limiters mapped; NFR4.4 Login (5/IP/15min) ✅ and Password Reset (3/email/hour) ✅ verified (latter at service layer in `PasswordResetService.checkRateLimit`); 1 MEDIUM gap (`/auth/reauth` brute-force surface) auto-fixed via new `reauth-rate-limit.ts` (5/IP/15min); 3 endpoints exempt-with-rationale (logout, /me, /mfa/enroll); 6-test coverage suite with 22-entry source-of-truth coverage map + threshold contract documentation + drift detector. **Closing SHA**: this commit.
