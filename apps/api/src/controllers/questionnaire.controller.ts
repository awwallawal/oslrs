...
export class QuestionnaireController {
  ...
  /**
   * PATCH /api/v1/questionnaires/:id/unpublish
   * Unpublish a form from ODK Central (Story 2-5, AC: 3)
   *
   * Sets form state to 'closing' in ODK Central and local database.
   */
  static async unpublishFromOdk(req: Request, res: Response, next: NextFunction) {
    try {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      const userId = (req as any).user?.sub;
      if (!userId) throw new AppError('UNAUTHORIZED', 'User not authenticated', 401);
      ...
      const result = await QuestionnaireService.unpublishFromOdk(id, userId, {
        ipAddress: req.ip,
